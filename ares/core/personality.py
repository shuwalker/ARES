...
class ExpressionLayer:
    ...
    def __post_init__(self):
        for attr in ["sarcasm", "warmth", "verbosity", "formality",
                     "directness", "humor", "empathy", "aggression"]:
            setattr(self, attr, max(0.0, min(1.0, float(getattr(self, attr)))))
...
class DomainsLayer:
    ...
    def __post_init__(self):
        for attr in ["science", "philosophy", "combat", "art",
                     "politics", "technology", "nature", "psychology"]:
            setattr(self, attr, max(0.0, min(1.0, float(getattr(self, attr)))))
