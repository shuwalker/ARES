...
- `apps/macos/Sources/ARES` owns the native lifecycle and WKWebView shell.
- `apps/macos/Sources/ARESCore` owns native services and MCP tools.
- `apps/macos/Sources/ARESNativeMCP` exposes those tools to the web runtime.
- `services/controller/fastapi_app` owns HTTP/WebSocket transport.
- `services/controller/api` owns server-side product services and runtime adapters.
- `apps/web/src` owns the visible product experience.
...
The same guard rejects the retired `services/controller/api/langgraph_study`, `evolution`,
...
product entry point. The reachable `services/controller/api/research` service remains because
it is owned by `fastapi_app/routers/research.py`.

It also rejects Python modules directly under `services/controller/api/adapters/`. That
...
`services/controller/api/adapters/safari_mcp` with its own executable integration boundary.
