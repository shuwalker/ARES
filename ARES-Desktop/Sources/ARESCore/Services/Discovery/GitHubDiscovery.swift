...
public struct GitHubRepo: Identifiable, Equatable, Sendable {
    public let id: String                  // "owner/name"
    public let name: String                // "ARES"
    public let owner: String               // "shuwalker"
    public let localPath: String?          // absolute path if cloned
    public let cloneStatus: CloneStatus
    public let description: String?
    public let language: String?
    public let stars: Int
    public let forks: Int
    public let lastActivity: Date?         // last commit timestamp
    public let isPrivate: Bool
    public let defaultBranch: String
    public let openPRs: Int
...
@MainActor
public final class GitHubDiscovery: ObservableObject {
    public init() {}

    @Published public var repos: [GitHubRepo] = []
    ...
    #else
    // Non-macOS stubs — Process and gh are unavailable on iOS/visionOS
    nonisolated private func checkGhAvailable() -> Bool { false }
    nonisolated private func ghAPI(_ endpoint: String) -> [String: Any]? { nil }
    nonisolated private func ghAPIArray(_ endpoint: String) -> [[String: Any]]? { nil }
    nonisolated private func ghAPICount(_ endpoint: String) -> Int? { nil }
    nonisolated private func enrichFromGh(owner: String, name: String, local: LocalClone) -> GitHubRepo {
        GitHubRepo(
            id: "\(owner)/\(name)", name: name, owner: owner,
            localPath: local.path, cloneStatus: .cloned,
            description: nil, language: nil, stars: 0, forks: 0,
            lastActivity: local.lastMtime, isPrivate: false,
            defaultBranch: "main", openPRs: 0, remoteURL: local.remoteURL
        )
    }
    nonisolated private func fetchStarredRepos(excludingLocal: [GitHubRepo]) -> [GitHubRepo] { [] }
    #endif
