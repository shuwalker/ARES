...
struct UsageView: View {
    ...
    var body: some View {
        ARESPageContainer(width: .analytics) {
            VStack(alignment: .leading, spacing: 24) {
                ARESPageHeader(
                    title: "Usage",
                    subtitle: "The main cards and charts show input/output tokens for the active ARES profile. When more than one profile is discovered, the host-wide panel shows all-categories tokens across readable profiles."
                )
                        ...
                        ContentUnavailableView(
                            ...
                            description: Text(
                                usageSummary.message ??
                                    "No readable ARES session database is currently available for the active host."
                            )
                        )
                        .frame(maxWidth: .infinity, minHeight: 320)
                    }
            ...
            ViewThatFits(in: .horizontal) {
                HStack(alignment: .firstTextBaseline, spacing: 18) {
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Text(appState.activeConnection?.resolvedARESProfileName ?? "default")
    ...
    private func usageHighlightsPanel(summary: UsageSummary) -> some View {
        ARESSurfacePanel(
            ...
            subtitle: "A compact summary of stored input and output tokens for the currently selected ARES profile."
        ) {
            ViewThatFits(in: .horizontal) {
                HStack(alignment: .top, spacing: 12) {
                    UsageMiniStat(
        ...
        ARESSurfacePanel(
            ...
            if summary.recentSessions.isEmpty {
                ContentUnavailableView(
                    ...
                    description: Text(L10n.string("Recent session usage will appear here once ARES has stored session data."))
                )
            ...
            VStack(alignment: .leading, spacing: 6) {
                HStack(spacing: 8) {
                    ...
                    if profile.isActiveProfile {
                        ARESBadge(text: "Active", tint: .accentColor)
                    }
                }
