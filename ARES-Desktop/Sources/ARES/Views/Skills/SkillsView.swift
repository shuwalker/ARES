...
struct SkillsView: View {
    ...
    var body: some View {
        ARESCollapsibleHSplitView(layout: $splitLayout, detailMinWidth: 420) {
            VStack(alignment: .leading, spacing: 18) {
                ARESPageHeader(
                    title: "Skills",
                    subtitle: "Browse the ARES skill library discovered on the active host."
                ) {
        ...
        if appState.isLoadingSkills && appState.skills.isEmpty {
            ...
            ARESSurfacePanel {
                ContentUnavailableView(
                    ...
                    description: Text(L10n.string("No readable SKILL.md files were discovered in the ARES skill roots for this SSH target."))
                )
                .frame(maxWidth: .infinity, minHeight: 300)
            }
        } else {
    ...
    private var skillsToolbar: some View {
        HStack(spacing: 10) {
            ARESCreateActionButton("New Skill") {
                startCreating()
            }
                    ...
                    VStack(alignment: .leading, spacing: 5) {
                        HStack(spacing: 8) {
                            ...
                            if !skill.source.isLocal {
                                ARESBadge(text: skill.sourceLabel, tint: .secondary)
                            }
                        }
