                        ...
                        VStack(alignment: .leading, spacing: 14) {
                            HStack(spacing: 8) {
                                ...
                                if !summary.source.isLocal {
                                    ARESBadge(text: summary.sourceLabel, tint: .secondary)
                                }
                            }
                    ...
                    ARESSurfacePanel {
                        VStack(alignment: .leading, spacing: 18) {
                            ContentUnavailableView(
                                ...
                                description: Text(L10n.string("Choose a ARES skill from the active host to inspect its metadata and full SKILL.md."))
                            )
        ...
        ARESSurfacePanel {
            VStack(alignment: .leading, spacing: 16) {
                HStack(alignment: .top, spacing: 12) {
                    ...
                    VStack(alignment: .trailing, spacing: 8) {
                        if let category = detail.category {
                            ARESBadge(text: category, tint: .secondary)
                        }
                ...
                if detail.isReadOnly {
                    Text(L10n.string("External skill directories are discovery-only in ARES. This skill is available to inspect here, but edits still belong in the local ARES skills store."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
...
struct SkillEditorView: View {
        ...
        ARESSurfacePanel(
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                SkillFormField(label: "Short Description") {
                    TextField(L10n.string("When ARES should use this skill and what it helps it do."), text: $draft.description, axis: .vertical)
                        .textFieldStyle(.roundedBorder)
                        .lineLimit(2...3)
                }
    ...
    private var createMetadataPanel: some View {
        ARESSurfacePanel(
            title: "Metadata",
            subtitle: "Optional tags and related skills help ARES and the user understand the role of the skill."
        ) {
            VStack(alignment: .leading, spacing: 14) {
    ...
    private var createInstructionsPanel: some View {
        ARESSurfacePanel(
            title: "Instructions",
            subtitle: "Write the actual guidance ARES should follow once the skill triggers."
        ) {
    ...
    private var generatedPreviewPanel: some View {
        ARESSurfacePanel(
            ...
            subtitle: "This is the SKILL.md the app will write on the remote ARES host."
        ) {
            ARESInsetSurface {
    ...
    private var headerSubtitle: String {
        switch mode {
        case .create:
            return "Create a new ARES skill from a guided form instead of writing YAML frontmatter and folder structure by hand."
        case .edit:
