...
struct ARESPageContainer<Content: View>: View {
    let width: ARESPageWidth
    let horizontalPadding: CGFloat
    let verticalPadding: CGFloat
    let content: Content

    init(
        width: ARESPageWidth = .standard,
        horizontalPadding: CGFloat = ARESTheme.pageHorizontalPadding,
        verticalPadding: CGFloat = ARESTheme.pageVerticalPadding,
...
struct ARESPageHeader<Accessory: View>: View {
    let title: String
    let subtitle: String
...
private struct ARESAdaptivePairLayout: Layout {
...
struct ARESSurfacePanel<Content: View>: View {
    let title: String?
    let subtitle: String?
    let content: Content
...
struct ARESInsetSurface<Content: View>: View {
    let content: Content
    ...
    var body: some View {
        content
            ...
            .frame(maxWidth: .infinity, alignment: .leading)
            .background(
                RoundedRectangle(cornerRadius: ARESTheme.insetCornerRadius, style: .continuous)
                    .fill(ARESTheme.insetFill)
            )
    }
}

struct ARESLoadingState: View {
    let label: String
...
struct ARESLoadingOverlay: View {
    var body: some View {
        ProgressView()
            .controlSize(.small)
            .padding(10)
            .background(.ultraThinMaterial, in: RoundedRectangle(cornerRadius: ARESTheme.insetCornerRadius, style: .continuous))
            .overlay {
                RoundedRectangle(cornerRadius: ARESTheme.insetCornerRadius, style: .continuous)
                    .strokeBorder(ARESTheme.subtleStroke, lineWidth: 1)
            }
...
struct ARESValidationMessage: View {
    let text: String
...
struct ARESCreateActionButton: View {
    let title: String
    let help: String?
    let action: () -> Void
...
struct ARESInspectorField: Identifiable, Hashable {
    let id: String
    let label: String
    let value: String
    var isMonospaced = false
...
struct ARESInspectorFieldList: View {
    let fields: [ARESInspectorField]
    var labelWidth: CGFloat = 108

    var body: some View {
        VStack(spacing: 0) {
            ForEach(Array(fields.enumerated()), id: \.element.id) { index, field in
                ARESInspectorFieldRow(field: field, labelWidth: labelWidth)
...
private struct ARESInspectorFieldRow: View {
    let field: ARESInspectorField
...
struct ARESExpandableSearchField: View {
...
struct ARESSearchActionBar<LeadingContent: View>: View {
    @Binding var text: String
...
struct ARESSplitLayout: Equatable {
...
struct ARESCollapseToolbarButton: View {
            ...
            Image(systemName: systemImage)
                ...
                .background {
                    RoundedRectangle(cornerRadius: 7, style: .continuous)
                        .fill(isActive ? ARESTheme.selectedFill : .clear)
                }
                .overlay {
                    RoundedRectangle(cornerRadius: 7, style: .continuous)
                        .strokeBorder(isActive ? ARESTheme.selectedStroke : .clear, lineWidth: 1)
                }
        }
...
struct ARESToolbarControlCluster<Content: View>: View {
    let content: Content
...
struct ARESToolbarPrincipalTitle: View {
    let title: String
...
struct ARESWindowTitleBarConfigurator: NSViewRepresentable {
...
struct ARESCollapsibleHSplitView<Primary: View, Detail: View>: View {
    @Binding var layout: ARESSplitLayout
    ...
    init(
        layout: Binding<ARESSplitLayout>,
    ...
    var body: some View {
        ZStack {
            if layout.isPrimaryCollapsed {
                ...
                ARESPersistentHSplitView(layout: $layout, detailMinWidth: detailMinWidth) {
                    primary
                } detail: {
                    detail
                }
...
struct ARESPersistentHSplitView<Primary: View, Detail: View>: NSViewRepresentable {
    @Binding var layout: ARESSplitLayout
    let detailMinWidth: CGFloat
    let primary: Primary
    let detail: Detail
    ...
    final class Coordinator: NSObject, NSSplitViewDelegate {
        ...
        var layout: Binding<ARESSplitLayout>?
...
struct ARESWrappingFlowLayout: Layout {
    let horizontalSpacing: CGFloat
    let verticalSpacing: CGFloat
    ...
    private func computeLines(for sizes: [CGSize], maxWidth: CGFloat?) -> [ARESFlowLine] {
        let availableWidth = maxWidth ?? .greatestFiniteMagnitude
        guard !sizes.isEmpty else { return [] }

        var lines: [ARESFlowLine] = []
        var currentItems: [ARESFlowLineItem] = []
        ...
        for (index, size) in sizes.enumerated() {
            ...
            if !currentItems.isEmpty && proposedWidth > availableWidth {
                ...
                currentItems = [ARESFlowLineItem(index: index)]
                currentWidth = size.width
                currentHeight = size.height
            } else {
                currentItems.append(ARESFlowLineItem(index: index))
...
private struct ARESFlowLine {
    let items: [ARESFlowLineItem]
    let width: CGFloat
    let height: CGFloat
}

private struct ARESFlowLineItem {
    let index: Int
}
