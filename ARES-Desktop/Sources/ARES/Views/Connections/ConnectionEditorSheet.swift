...
struct ConnectionEditorSheet: View {
    ...
    @State private var showsCustomARESHomeOptions: Bool
    ...
    init(connection: ConnectionProfile, isEditing: Bool, onSave: @escaping (ConnectionProfile) -> Void) {
        ...
        _showsCustomARESHomeOptions = State(initialValue: connection.customARESHomePath?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false)
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    ARESPageHeader(
                        ...
                        subtitle: "Set the SSH details ARES Desktop should use for discovery, file editing, sessions and terminal access."
                    )
                    ...
                    ARESSurfacePanel(
                        ...
                        VStack(alignment: .leading, spacing: 14) {
                            ...
                            EditorField(label: "ARES profile") {
                            ...
                            DisclosureGroup(
                                isExpanded: $showsCustomARESHomeOptions
                            ) {
                                VStack(alignment: .leading, spacing: 10) {
                                    EditorField(label: "Remote ARES home path") {
                                        TextField(L10n.string("~/.hermes-work or /opt/data"), text: customARESHomeBinding)
                                            .focused($focusedField, equals: .customARESHome)
                                            .textFieldStyle(.roundedBorder)
                                    }

                                    Text(L10n.string("Only use this if ARES lives outside the standard `~/.hermes` or `~/.hermes/profiles/<name>` layout on the remote host."))
                                        .font(.subheadline)
                                        .foregroundStyle(.secondary)
                                        .fixedSize(horizontal: false, vertical: true)

                                    Text(L10n.string("Leave this empty for the normal setup. When set, ARES Desktop uses this path as `HERMES_HOME` for Sessions, Files, Skills, Usage, Cron, chat, and Terminal."))
                                        .font(.subheadline)
                                        .foregroundStyle(.secondary)
                                        .fixedSize(horizontal: false, vertical: true)
                                }
                                ...
                                Text(L10n.string("Custom ARES home (advanced)"))
                                    .font(.headline)
                            }

                            if let validationMessage {
                                ARESValidationMessage(text: validationMessage)
                            }
                        }
                    }

                    ARESSurfacePanel(
                        title: "How ARES Connects",
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            ConnectionHintRow(
                                title: "Same Mac",
                                detail: "If ARES runs on this Mac, stay with the SSH model and use localhost, the local hostname, or a local SSH alias."
                            )

                            ConnectionHintRow(
                                title: "Authentication",
                                detail: "SSH must already work from this Mac without interactive prompts. Password login may still exist on the host, but ARES Desktop expects keys, an SSH agent, or another non-interactive SSH path for the actual connection it uses."
                            )

                            ConnectionHintRow(
                                title: "Network path",
                                detail: "The Mac and ARES host do not need to be on the same Wi-Fi. Local network, public IP, VPN, or Tailscale all work as long as standard ssh from this Mac reaches the host."
                            )
                        ...
                        ARESInsetSurface {
                            VStack(alignment: .leading, spacing: 6) {
                                Text(L10n.string("ARES profile"))
                                    .font(.headline)

                                Text(L10n.string("Leave it empty for the default ARES home at `~/.hermes`. Set a profile name like `researcher` to target `~/.hermes/profiles/researcher` on the same host."))
                                    .font(.subheadline)
                                    .foregroundStyle(.secondary)
                                    .fixedSize(horizontal: false, vertical: true)
                            }
                    ...
                    ARESSurfacePanel(
                        title: "Examples",
                        subtitle: "A few common patterns that work well with ARES Desktop."
                    ) {
                        VStack(alignment: .leading, spacing: 12) {
    ...
    private var customARESHomeBinding: Binding<String> {
        Binding {
            draft.customARESHomePath ?? ""
        } set: { newValue in
            draft.customARESHomePath = newValue
        }
    }
}
