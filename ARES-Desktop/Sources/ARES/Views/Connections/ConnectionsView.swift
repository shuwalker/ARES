...
struct ConnectionsView: View {
    ...
    var body: some View {
        ARESPageContainer(width: .standard) {
            VStack(alignment: .leading, spacing: 24) {
                ARESPageHeader(
                    title: "Hosts",
                    subtitle: "Alias-first SSH profiles for every ARES workspace, from a Raspberry Pi to another Mac or a remote VPS."
                ) {
                    Button {
                    ...
                    ARESSurfacePanel {
                        VStack(alignment: .leading, spacing: 18) {
                            ContentUnavailableView(
                                ...
                                description: Text(L10n.string("Create your first SSH profile to connect ARES Desktop to a Raspberry Pi, another Mac, a VPS, or this Mac via localhost."))
                            )
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                ARESInsetSurface {
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        Text(L10n.string("The Mac and ARES host do not need to share the same Wi-Fi. What matters is that normal ssh from this Mac can reach the host over LAN, public IP, VPN, or Tailscale."))
                            .font(.subheadline)
                            .fixedSize(horizontal: false, vertical: true)
                    }
                }
...
private struct ConnectionCard: View {
    ...
    private var hermesScopeLabel: String {
        connection.usesCustomARESHome ? "Custom ARES home" : "ARES profile"
    }

    private var hermesScopeValue: String {
        connection.trimmedCustomARESHomePath ?? connection.resolvedARESProfileName
    }
