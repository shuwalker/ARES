...
struct KanbanView: View {
    ...
    var body: some View {
        ARESCollapsibleHSplitView(layout: $splitLayout, detailMinWidth: 420) {
            primaryContent
        } detail: {
            detailContent
    ...
    private var primaryContent: some View {
        VStack(alignment: .leading, spacing: 18) {
            ARESPageHeader(
                title: "Kanban",
                subtitle: "Inspect and operate ARES Kanban projects over SSH."
            ) {
    ...
    private var kanbanToolbar: some View {
        ARESWrappingFlowLayout(horizontalSpacing: 10, verticalSpacing: 8) {
            HStack(spacing: 8) {
                boardPicker
                statusPicker
                advancedFilterMenu
                ...
                if !tasks.isEmpty {
                    VStack(alignment: .leading, spacing: 10) {
                        HStack(spacing: 8) {
                            ...
                            ARESBadge(text: "\(tasks.count)", tint: KanbanColors.tint(for: status))
                        }
    ...
    private func dispatcherWarning(for board: KanbanBoard) -> String? {
        ...
        return board.dispatcher?.message ?? "Ready tasks are waiting, but the remote ARES dispatcher does not appear to be active."
    }
...
private struct KanbanColumnView: View {
    ...
    var body: some View {
        ...
        .padding(12)
        .frame(maxWidth: .infinity, alignment: .topLeading)
        .background(Color.secondary.opacity(0.026), in: RoundedRectangle(cornerRadius: ARESTheme.panelCornerRadius, style: .continuous))
        .overlay {
            RoundedRectangle(cornerRadius: ARESTheme.panelCornerRadius, style: .continuous)
    ...
    private var emptyState: some View {
        Text(L10n.string("No tasks"))
            ...
            .padding(.vertical, 12)
            .frame(maxWidth: .infinity, alignment: .leading)
            .background(Color.secondary.opacity(0.035), in: RoundedRectangle(cornerRadius: ARESTheme.rowCornerRadius, style: .continuous))
    }
}
...
private struct KanbanTaskCard: View {
        ...
        Button(action: onSelect) {
            ...
            .background(
                RoundedRectangle(cornerRadius: ARESTheme.rowCornerRadius, style: .continuous)
                    .fill(isSelected ? ARESTheme.selectedFill : isHovering ? Color.secondary.opacity(0.07) : ARESTheme.rowFill)
            )
            .overlay {
                RoundedRectangle(cornerRadius: ARESTheme.rowCornerRadius, style: .continuous)
                    .strokeBorder(isSelected ? ARESTheme.selectedStroke : ARESTheme.subtleStroke, lineWidth: 1)
            }
    ...
    private var metadataStrip: some View {
        VStack(alignment: .leading, spacing: 7) {
            ARESWrappingFlowLayout(horizontalSpacing: 7, verticalSpacing: 5) {
                ...
                ARESSurfacePanel {
                    VStack(alignment: .leading, spacing: 16) {
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            Text(L10n.string("The board will be created on the active ARES host over SSH."))
                                .font(.subheadline)
                                .foregroundStyle(.secondary)
                        }
                        ...
                        if let validationError = draft.validationError {
                            ARESValidationMessage(text: validationError)
                        }
                    }
                }
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ARESSurfacePanel {
                    VStack(alignment: .leading, spacing: 16) {
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            Text(L10n.string("The task will be created in the selected ARES Kanban board over SSH."))
                                .font(.subheadline)
                                .foregroundStyle(.secondary)
                        }
                ...
                ARESSurfacePanel(
                    title: "Task",
                    subtitle: "Describe the work and optionally assign it to a ARES profile."
                ) {
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        HStack(alignment: .top, spacing: 16) {
                            KanbanFormField(label: "Priority") {
                                VStack(alignment: .leading, spacing: 5) {
                                    ...
                                    Text(L10n.string("Higher values sort first. 0 is the ARES default."))
                                        .font(.caption)
                                        .foregroundStyle(.secondary)
                                }
                            }
...
private struct KanbanTaskDetailView: View {
            ...
            VStack(alignment: .leading, spacing: 18) {
                if let task {
                    ...
                    if isLoading && detail == nil {
                        ARESSurfacePanel {
                            ARESLoadingState(label: "Loading task detail...", minHeight: 180)
                        }
                    } else {
                        metadataPanel(task)
                        actionPanel(task)
                        ...
                        if let result = task.trimmedResult {
                            ARESSurfacePanel(
                                title: "Result",
                                subtitle: "Completion handoff stored by ARES."
                            ) {
                                ARESInsetSurface {
                                    Text(result)
                                        .frame(maxWidth: .infinity, alignment: .leading)
        ...
        ARESSurfacePanel {
            VStack(alignment: .leading, spacing: 16) {
                HStack(alignment: .top, spacing: 12) {
                    ...
                    HStack(spacing: 8) {
                        ARESBadge(text: task.status.displayTitle, tint: KanbanColors.tint(for: task.status))
                        ARESBadge(text: task.priorityLabel, tint: task.priority == 0 ? .secondary : .orange, isMonospaced: true)
                    }
                }
    ...
    private func metadataPanel(_ task: KanbanTask) -> some View {
        ARESSurfacePanel(
            ...
            ARESInspectorFieldList(fields: metadataFields(for: task))

            if !task.skills.isEmpty {
                Divider()
                    .opacity(0.5)

                ARESWrappingFlowLayout(horizontalSpacing: 8, verticalSpacing: 8) {
                    ForEach(task.skills, id: \.self) { skill in
                        ARESBadge(text: skill, tint: .accentColor, isMonospaced: true)
                    }
                }
            }
    ...
    private func metadataFields(for task: KanbanTask) -> [ARESInspectorField] {
        var fields = [
            ARESInspectorField(
                id: "status",
                label: "Status",
            ...
            VStack(alignment: .leading, spacing: 0) {
                if task.canSpecify {
                    KanbanActionDisclosureRow(
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            Text(L10n.string("ARES may refine the title and body before moving this task from Triage to Todo."))
                                .font(.subheadline)
                                .foregroundStyle(.secondary)
    ...
    private func recoveryWarningPanel(_ task: KanbanTask) -> some View {
        if let warnings = task.warnings, warnings.hasWarnings {
            ARESSurfacePanel(
                ...
                subtitle: "ARES Agent marked this task for recovery."
            ) {
                VStack(alignment: .leading, spacing: 12) {
                    KanbanRecoveryWarningSummary(warnings: warnings)
    ...
    private func runsPanel(_ detail: KanbanTaskDetail) -> some View {
        ARESSurfacePanel(
            title: "Runs",
            subtitle: "Attempt history recorded by ARES."
        ) {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                HStack(spacing: 8) {
                    ARESBadge(text: L10n.string("%@ warning events", "\(warnings.count)"), tint: .orange)
            ...
            if ids.isEmpty {
                ...
                ARESWrappingFlowLayout(horizontalSpacing: 8, verticalSpacing: 8) {
                    ForEach(ids, id: \.self) { id in
                        ARESBadge(text: id, tint: .secondary, isMonospaced: true)
                    }
                }
            }
        }
...
private struct KanbanRunRow: View {
    ...
    var body: some View {
        ARESInsetSurface {
            VStack(alignment: .leading, spacing: 8) {
                HStack(spacing: 8) {
                    ARESBadge(text: "#\(run.id)", tint: .secondary, isMonospaced: true)
                    ARESBadge(text: run.resolvedOutcome, tint: run.endedAt == nil ? .orange : .secondary)

                    if let profile = run.profile {
                        ARESBadge(text: "@\(profile)", tint: .accentColor, isMonospaced: true)
                    }
                }
...
private struct KanbanEventRow: View {
    ...
    var body: some View {
        ARESInsetSurface {
            VStack(alignment: .leading, spacing: 8) {
                HStack(spacing: 8) {
                    ARESBadge(text: event.kind, tint: .secondary)

                    if let runID = event.runID {
                        ARESBadge(text: "run \(runID)", tint: .secondary, isMonospaced: true)
                    }
