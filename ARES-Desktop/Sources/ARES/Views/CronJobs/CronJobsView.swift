...
struct CronJobsView: View {
    ...
    var body: some View {
        ARESCollapsibleHSplitView(layout: $splitLayout, detailMinWidth: 460) {
            VStack(alignment: .leading, spacing: 18) {
                ARESPageHeader(
                    title: "Cron Jobs",
                    subtitle: "Browse, create and maintain ARES jobs discovered on the active host."
                ) {
        ...
        .alert(L10n.string("Remove cron job?"), isPresented: $showDeleteConfirmation) {
            ...
            if let jobToDelete {
                Text(L10n.string(
                    "“%@” will be removed from the remote ARES scheduler. This cannot be undone.",
    ...
    private var filterBar: some View {
        HStack(spacing: 10) {
            ...
            ARESCreateActionButton("New Job", help: "Create a new cron job") {
                startCreating()
            }
        ...
        if appState.isLoadingCronJobs && appState.cronJobs.isEmpty {
            ...
            ARESSurfacePanel {
                ContentUnavailableView(
                    ...
                    description: Text(L10n.string("No saved ARES cron jobs were discovered under %@ on this SSH target.", cronJobsPath))
                )
                .frame(maxWidth: .infinity, minHeight: 300)
            }
        } else {
            ...
            VStack(alignment: .leading, spacing: 10) {
                HStack(alignment: .top, spacing: 10) {
                    ...
                    HStack(spacing: 8) {
                        ...
                        if job.noAgent {
                            ARESBadge(text: "Script", tint: .blue)
                        }
...
private struct CronJobDetailView: View {
            ...
            VStack(alignment: .leading, spacing: 18) {
                if let job {
                    ...
                    if let lastError = job.lastError {
                        ARESSurfacePanel(
                            title: "Last Error",
                            subtitle: "Most recent execution failure reported by ARES."
                        ) {
                            Text(lastError)
                                .foregroundStyle(.red)
                    ...
                    if job.noAgent {
                        ...
                        ARESSurfacePanel(
                            title: "Prompt",
                            subtitle: "Payload ARES will run for this scheduled job."
                        ) {
                            ARESInsetSurface {
                    ...
                    ARESSurfacePanel {
                        ContentUnavailableView(
                            ...
                            description: Text(L10n.string("Choose a ARES cron job from the active host to inspect it, or create a new one."))
                        )
            ...
            VStack(alignment: .leading, spacing: 16) {
                HStack(alignment: .top, spacing: 12) {
                    ...
                    HStack(spacing: 8) {
                        ...
                        if job.noAgent {
                            ARESBadge(text: "Script Only", tint: .blue)
                        }
    ...
    private func metadataPanel(_ job: CronJob) -> some View {
        ARESSurfacePanel(
            title: "Details",
            subtitle: "Schedule metadata and recent execution markers reported by ARES."
        ) {
...
private struct CronJobEditorView: View {
        ...
        ARESSurfacePanel(
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                if draft.noAgent {
                    ...
                    Text(L10n.string("Script-only jobs run a script from the host's ARES scripts directory and deliver stdout directly without creating an agent turn."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                } else {
    ...
    private var schedulePanel: some View {
        ARESSurfacePanel(
            title: "Schedule",
            subtitle: "Choose whether ARES should run this once or on a recurring cadence."
        ) {
            VStack(alignment: .leading, spacing: 14) {
        ...
        ARESSurfacePanel(
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                if draft.deliveryPreset == .custom {
                    ...
                    Text(L10n.string("Use full ARES target syntax such as `telegram:-1001234567890`, `telegram:-1001234567890:17585`, or another supported platform target."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
...
private struct CronStatusBadge: View {
    ...
    var body: some View {
        ARESBadge(text: job.displayState, tint: tint)
    }
