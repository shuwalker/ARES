...
struct SessionsView: View {
    ...
    var body: some View {
        ARESCollapsibleHSplitView(layout: $splitLayout, detailMinWidth: 420) {
            VStack(alignment: .leading, spacing: 18) {
                ARESPageHeader(
                    title: "Sessions",
                    subtitle: "Browse the recent ARES conversations discovered on the active host."
                ) {
        ...
        if appState.isLoadingSessions && !hasVisibleSessions {
            ...
            ARESSurfacePanel {
                ContentUnavailableView(
                    ...
                    systemImage: "tray",
                    description: Text(L10n.string("No readable ARES sessions were discovered yet for this SSH target."))
                )
                .frame(maxWidth: .infinity, minHeight: 300)
            }
        } else {
    ...
    private var sessionsToolbar: some View {
        HStack(spacing: 10) {
            ARESCreateActionButton("New Chat") {
                searchText = ""
            ...
            content
                ...
                .background(
                    RoundedRectangle(cornerRadius: ARESTheme.rowCornerRadius, style: .continuous)
                        .fill(isSelected ? ARESTheme.selectedFill : ARESTheme.rowFill)
                )
                .overlay {
                    RoundedRectangle(cornerRadius: ARESTheme.rowCornerRadius, style: .continuous)
                        .strokeBorder(isSelected ? ARESTheme.selectedStroke : ARESTheme.subtleStroke, lineWidth: 1)
                }
                .contentShape(RoundedRectangle(cornerRadius: ARESTheme.rowCornerRadius, style: .continuous))
        }
        .buttonStyle(.plain)
        .overlay(alignment: .topTrailing) {
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .top, spacing: 10) {
                ...
                if let count = session.messageCount {
                    ARESBadge(text: L10n.string("%@ messages", "\(count)"), tint: .secondary)
                }
            }
