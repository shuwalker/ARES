...
struct SessionDetailView: View {
    ...
    var body: some View {
        ...
        .alert(L10n.string("Delete this session?"), isPresented: $showDeleteConfirmation, presenting: session) { session in
            ...
            Text(L10n.string(
                "“%@” will be removed from ARES Desktop and deleted on the remote ARES host as well. This action cannot be undone.",
    ...
    private var newChatSubtitle: String {
        guard let connection else {
            return L10n.string("Select an SSH host to start a live ARES TUI.")
        }
        return "\(connection.label) - \(connection.displayDestination) - \(connection.resolvedARESProfileName)"
    }
        ...
        if let session {
            ...
            ARESSurfacePanel {
                ContentUnavailableView(
                    ...
                    description: Text(L10n.string("Use New Chat to start the real ARES TUI, or choose an existing session to inspect its stored transcript."))
                )
    ...
    private func transcriptContent(for session: SessionSummary) -> some View {
        if messages.isEmpty {
            ...
            ARESSurfacePanel(
                title: "Transcript",
                subtitle: "Messages are shown in the order ARES stored them for this session."
            ) {
            ...
            VStack(spacing: 0) {
                HStack(spacing: 10) {
                    ...
                    if let exitCode = terminal.terminalSession.exitCode {
                        ARESBadge(text: L10n.string("Exited %@", "\(exitCode)"), tint: exitCode == 0 ? .secondary : .orange)
                    } else if terminal.terminalSession.isRunning {
                        ARESBadge(text: L10n.string("Running"), tint: Color(red: 0.0, green: 0.58, blue: 0.22))
                    }
                }
    ...
    private var chatPlaceholderTitle: String {
        ...
        return session == nil ? L10n.string("Start a new ARES TUI chat") : L10n.string("Start Chat for this session")
    }
    ...
    private var chatPlaceholderDescription: String {
        ...
        if let session {
            return L10n.string("ARES TUI will resume %@ over the existing SSH-first terminal path.", shortSessionID(session.id))
        }
        return L10n.string("ARES TUI will create the next session on the host; refresh Sessions after it exits or when you return to Transcript.")
    }
        ...
        ARESSurfacePanel {
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .top, spacing: 12) {
                    ...
                    HStack(spacing: 8) {
                        if let model = session.displayModel {
                            ARESBadge(text: model, tint: .orange)
                        }

                        if let count = session.messageCount {
                            ARESBadge(text: L10n.string("%@ messages", "\(count)"), tint: .accentColor)
                        }
                ...
                if !sessionMetadataFields.isEmpty {
                    ARESInspectorFieldList(fields: sessionMetadataFields)
                }
            }
        }
    }

    private var sessionMetadataFields: [ARESInspectorField] {
        var fields: [ARESInspectorField] = []
        ...
        VStack(alignment: .leading, spacing: 9) {
            HStack(spacing: 8) {
                ...
                if let onResumeInTerminal {
                    ...
                    .controlSize(.small)
                    .disabled(isSending)
                    .help(L10n.string("Open this ARES session in a fresh Terminal tab"))
                }
            }
...
private struct SessionPromptCardView: View {
    ...
    let isDisabled: Bool
    let onRespond: (ARESPromptCard, ARESPromptResponse) async -> Void
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        if let previewText = card.previewText {
                            VStack(alignment: .leading, spacing: 6) {
                                ...
                                .frame(maxHeight: 120)
                                .background(ARESTheme.insetFill, in: RoundedRectangle(cornerRadius: ARESTheme.insetCornerRadius, style: .continuous))
                                .overlay {
        ...
        ARESInsetSurface {
            HStack(alignment: .firstTextBaseline, spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text(L10n.string("ARES compacted this conversation into a new session. This session is now closed. Open the new session from the history list to continue."))
                        .font(.subheadline)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        HStack(spacing: 8) {
                            ...
                            if turn.autoApproveCommands {
                                ARESBadge(text: "Auto-approve", tint: .orange)
                            }
                        }
                    }
                }
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                if let content = message.content, !content.isEmpty {
                    ...
                    HStack(spacing: 8) {
                        ...
                        Text(L10n.string("ARES is thinking…"))
                            .foregroundStyle(.secondary)
                    }
                } else {
...
private struct ToolMessageCard: View {
    ...
    private var statusProminence: ARESBadge.BadgeProminence {
        switch summary?.statusKind {
