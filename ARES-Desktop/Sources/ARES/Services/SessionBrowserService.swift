...
final class SessionBrowserService: @unchecked Sendable {
    ...
    private var sessionListBody: String {
        ...
        try:
            ...
            fail(f"Unable to read the remote ARES session list: {exc}")
        finally:
            try:
    ...
    private var sessionDetailBody: String {
        ...
        try:
            ...
            fail(f"Unable to read the remote ARES transcript: {exc}")
        finally:
            try:
                if context and context.get("connection"):
                    context["connection"].close()
    ...
    private var sessionDeleteBody: String {
        ...
        try:
            ...
            if deleted_session_rows <= 0 and deleted_message_rows <= 0 and not deleted_jsonl_artifact:
                fail(f"No remote ARES session matching '{session_id}' was found to delete.")

            print(json.dumps({
                "ok": True,
            }, ensure_ascii=False))
        except Exception as exc:
            fail(f"Unable to delete the remote ARES session: {exc}")
        """
    }
