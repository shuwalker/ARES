...
final class ARESChatService: @unchecked Sendable {
    ...
    func sendMessage(
    ...
    ) async throws -> ARESChatTurnResult {
        let invocation = ARESChatInvocation(
            sessionID: sessionID,
            prompt: prompt,
        ...
        return try await sshTransport.executeJSON(
            on: connection,
            pythonScript: script,
            responseType: ARESChatTurnResult.self
        )
    }
    ...
    private var chatBody: String {
        ...
        def compact_output(stdout, stderr, exit_code):
            ...
            if not merged:
                return f"ARES chat exited with code {exit_code}."
            if len(merged) <= 4000:
                return merged
            return merged[-4000:]
        ...
        def approval_error(message):
            return (
                "ARES requested command approval, but this chat turn cannot collect manual approvals. "
            ...
            try:
                while True:
                    ...
                    if now - started_at > timeout_seconds:
                        ...
                        fail(
                            "ARES did not finish within the allotted time. The turn was stopped so the app would not remain blocked indefinitely."
        ...
        try:
            ...
            if hermes_path is None:
                fail("ARES CLI was not found in the remote SSH environment. Verify that `hermes` is installed and available on PATH for non-interactive SSH commands.")

            arguments = payload.get("arguments") or []
            if not isinstance(arguments, list) or not all(isinstance(item, str) for item in arguments):
                fail("Invalid ARES chat invocation.")
            ...
            fail(f"Unable to run ARES chat over SSH: {exc}")
        """
    }
}

private struct ARESChatRequest: Encodable {
