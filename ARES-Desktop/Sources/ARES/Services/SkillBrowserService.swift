...
final class SkillBrowserService: @unchecked Sendable {
    ...
    func listSkills(connection: ConnectionProfile) async throws -> [SkillSummary] {
        let script = try RemotePythonScript.wrap(
            EmptySkillRequest(hermesHome: connection.remoteARESHomePath),
            body: skillListBody
        )
    ...
    private var skillListBody: String {
        ...
        try:
            ...
            fail(f"Unable to read the remote ARES skill library: {exc}")
        """
    }
    ...
    private var skillDetailBody: String {
        ...
        try:
            ...
            fail(f"Unable to read the remote ARES skill detail: {exc}")
        """
    }
        ...
        try:
            ...
            if requested_source_id is not None:
                ...
                if source["is_read_only"]:
                    fail("External skill directories are read-only in ARES. Create a local skill with the same path to override it.")
            ...
            fail(f"Unable to save the remote ARES skill: {exc}")
        """
    }
    ...
    private var sharedSkillHelpers: String {
        ...
        def resolve_skill_file(source_id, relative_path):
            ...
            try:
                ...
                fail("The requested skill path escapes the configured ARES skill source.")
    ...
    private func launchableSkillRegistryCommand(for connection: ConnectionProfile) -> String {
        """
        export HERMES_HOME="\(connection.remoteARESHomeShellExpression)"; export PATH="\(connection.remoteARESSearchPathShellExpression)"; if command -v hermes >/dev/null 2>&1; then HERMES_BIN="$(command -v hermes)"; else printf 'ARES CLI not found.\\n' >&2; exit 127; fi; COLUMNS=240 "$HERMES_BIN" skills list --enabled-only
        """
    }
}
