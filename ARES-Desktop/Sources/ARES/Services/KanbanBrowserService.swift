...
final class KanbanBrowserService: @unchecked Sendable {
    ...
    func loadBoards(connection: ConnectionProfile, includeArchived: Bool = false) async throws -> KanbanBoardsResponse {
        let script = try RemotePythonScript.wrap(
            KanbanBoardsRequest(
                ...
                hermesHome: connection.remoteARESHomePath,
    ...
    func createTask(connection: ConnectionProfile, boardSlug: String, draft: KanbanTaskDraft) async throws -> String {
        let response = try await performMutation(
            ...
            request: KanbanMutationRequest(
                ...
                hermesHome: connection.remoteARESHomePath,
                boardSlug: boardSlug,
                author: connection.resolvedARESProfileName,
                action: "create",
                taskID: nil,
    ...
    private var boardsBody: String {
        ...
        try:
            ...
            fail(f"Unable to load remote ARES Kanban boards: {exc}")
        """
    }
    ...
    private var boardBody: String {
        ...
        try:
            ...
            fail(f"Unable to load the remote ARES Kanban board: {exc}")
        """
    }
    ...
    private var createBoardBody: String {
        ...
        try:
            ...
            if not hasattr(kb, "create_board"):
                fail("This ARES Agent build does not support creating Kanban boards. Run `hermes update` on the host.")
            ...
            fail(f"Unable to create the remote ARES Kanban board: {exc}")
        """
    }
    ...
    private var archiveBoardBody: String {
        ...
        try:
            ...
            if not hasattr(kb, "remove_board"):
                fail("This ARES Agent build does not support archiving Kanban boards. Run `hermes update` on the host.")
            board_slug = requested_board_slug()
            ...
            fail(f"Unable to archive the remote ARES Kanban board: {exc}")
        """
    }
    ...
    private var mutationBody: String {
        ...
        def link_task(kb, conn, parent_id, child_id):
            if kb is not None and hasattr(kb, "link_tasks"):
                ...
                return
            fail("This ARES Agent build does not support Kanban dependency links. Run `hermes update` on the host.")
            ...
            with connect_for_board(kb, board_slug, writable=True) as conn:
                if action == "create":
                    ...
                    if parent_ids and not supports_keyword(kb.create_task, "parents"):
                        fail("This ARES Agent build does not support Kanban parent links at task creation. Run `hermes update` on the host.")
                    ...
                    if max_retries is not None and not supports_keyword(kb.create_task, "max_retries"):
                        fail("This ARES Agent build does not support per-task retry limits. Run `hermes update` on the host.")
                    kwargs = {
                        "title": title,
                        "body": normalize_text(payload.get("body")),
                ...
                if action == "specify":
                    # Specify relies on auxiliary-client bootstrap that the real ARES CLI
                ...
                if action == "reclaim":
                    if not hasattr(kb, "reclaim_task"):
                        fail("This ARES Agent build does not support Kanban recovery reclaim. Run `hermes update` on the host.")
                ...
                if action == "reassign":
                    if not hasattr(kb, "reassign_task"):
                        fail("This ARES Agent build does not support Kanban recovery reassign. Run `hermes update` on the host.")
                    assignee = normalize_text(payload.get("assignee"))
                ...
                if action == "edit_result":
                    if not hasattr(kb, "edit_completed_task_result"):
                        fail("This ARES Agent build does not support editing completed Kanban task results. Run `hermes update` on the host.")
                    result = normalize_text(payload.get("result"))
                    if result is None:
                ...
                if action == "dispatch":
                    if supports_keyword(kb.dispatch_once, "board"):
                        ...
                        if board_slug != DEFAULT_BOARD:
                            fail("This ARES Agent build does not support multiple Kanban boards. Run `hermes update` on the host.")
                        res = kb.dispatch_once(
                            conn,
                            max_spawn=int(payload.get("max_spawn") or 8),
                        )
    ...
    private var kanbanPythonHelpers: String {
        ...
        def require_board_api(kb, operation):
            if kb is None or not hasattr(kb, "list_boards"):
                fail(f"This ARES Agent build does not support {operation}. Run `hermes update` on the host.")
        ...
        def run_hermes_cli(args, expect_json=False):
            ...
            if hermes_binary is None:
                fail("ARES CLI was not found on the active host.")
            home = pathlib.Path.home()
            env = os.environ.copy()
            ...
            if cli_profile and cli_profile != "default":
                # Keep Kanban rooted at the shared ARES home, but let the
            ...
            if completed.returncode != 0:
                message = (completed.stderr or completed.stdout or "ARES Kanban command failed.").strip()
                fail(message)
            ...
            try:
                ...
                fail(f"ARES Kanban command returned invalid JSON: {exc}")
        ...
        def ensure_notify_subs_table(conn):
            if table_exists(conn, "kanban_notify_subs"):
                return
            fail("This ARES Agent build does not support Kanban home-channel subscriptions. Run `hermes update` on the host.")
                ...
                if kb is not None:
                    try:
                        ...
                        try:
                            ...
                            conn = None
                        base["warning"] = (
                            "The remote ARES Kanban module could not read this board "
                            f"({exc}). Showing a direct database view instead. "
                            "Some Kanban actions may still fail until ARES Agent and the board database schema are in sync on the host."
                        )
                        conn = connect_sqlite_readonly(db_path)
                        conn.row_factory = sqlite3.Row
