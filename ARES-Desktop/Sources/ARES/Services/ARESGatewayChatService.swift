...
enum ARESGatewayError: LocalizedError, Equatable, Sendable {
    case notConnected
...
struct ARESChatBootstrapStatus: Equatable, Sendable {
    ...
    var preferredTransportMode: ARESChatTransportMode {
        canUseNativeChat ? .native : .fallback
    }
}

struct ARESGatewayEvent: Identifiable, Hashable, Sendable {
    let id = UUID()
    let type: String
...
struct ARESGatewayRPCErrorPayload: Codable, Hashable, Sendable {
    let code: Int
    let message: String
    let data: JSONValue?
}

enum ARESGatewayRequestID: Codable, Hashable, Sendable {
    case int(Int)
    case string(String)
...
private struct ARESGatewayOutgoingRequest: Encodable {
    let jsonrpc = "2.0"
    let id: Int
    let method: String
...
private struct ARESGatewayIncomingFrame: Decodable {
    let jsonrpc: String?
    let id: ARESGatewayRequestID?
    let result: JSONValue?
    let error: ARESGatewayRPCErrorPayload?
    let method: String?
    let params: ARESGatewayIncomingEventParams?
}

private struct ARESGatewayIncomingEventParams: Decodable {
    let type: String
    let sessionID: String?
    let payload: [String: JSONValue]?
...
actor ARESGatewayRPCClient {
    typealias Sender = @Sendable (String) async throws -> Void

    nonisolated let events: AsyncStream<ARESGatewayEvent>

    private let eventContinuation: AsyncStream<ARESGatewayEvent>.Continuation
    ...
    init() {
        let stream = AsyncStream<ARESGatewayEvent>.makeStream()
    ...
    func request(
        ...
        guard let sender else {
            throw ARESGatewayError.notConnected
        }
        guard !isClosed else {
            throw ARESGatewayError.closed
        }

        nextRequestID += 1
        let requestID = nextRequestID
        let request = ARESGatewayOutgoingRequest(id: requestID, method: method, params: params)
    ...
    func handleStdoutLine(_ line: String) async {
        ...
        do {
            let frame = try decoder.decode(ARESGatewayIncomingFrame.self, from: Data(line.utf8))
            if let requestID = frame.id?.intValue {
                ...
                if let error = frame.error {
                    continuation.resume(throwing: ARESGatewayError.remote(error.code, error.message))
                } else {
    ...
    func finish(throwing error: Error? = nil) {
        ...
        for (_, continuation) in pendingRequests {
            continuation.resume(throwing: error ?? ARESGatewayError.closed)
        }
...
actor ARESGatewayChatService {
    ...
    init(
        ...
        let rpcClient = ARESGatewayRPCClient()
...
extension SSHTransport {
    func probeNativeChatAvailability(on connection: ConnectionProfile) async -> ARESChatBootstrapStatus {
        var status = ARESChatBootstrapStatus()
        ...
        do {
            let versionResult = try await execute(
                on: connection,
                remoteCommand: connection.remoteServiceCommand(
                    connection.remoteARESCommandLine(arguments: ["--version"])
                ),
                allocateTTY: false
            )
        ...
        if status.fallbackReason == nil && !status.canUseNativeChat {
            if !status.pythonAvailable {
                ...
                status.fallbackReason = "ARES CLI is not available on the remote host."
            } else if !status.tuiGatewayAvailable {
                status.fallbackReason = "The ARES TUI gateway is not importable on the remote host."
            }
        }
