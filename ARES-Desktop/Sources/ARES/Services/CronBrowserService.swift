...
final class CronBrowserService: @unchecked Sendable {
    ...
    func listJobs(connection: ConnectionProfile) async throws -> [CronJob] {
        let script = try RemotePythonScript.wrap(
            EmptyCronRequest(
                ...
                profileName: connection.cliARESProfileName
            ),
            body: listJobsBody
        )
    ...
    private var listJobsBody: String {
        ...
        try:
            ...
            fail(f"Unable to read the remote ARES cron jobs: {exc}")
        """
    }
    ...
    private var commandBody: String {
        ...
        try:
            ...
            fail(f"Unable to launch ARES CLI: {exc}")

        if completed.returncode != 0:
            message = (completed.stderr or completed.stdout or f"ARES cron {command} failed.").strip()
            fail(message)
    ...
    private var mutationBody: String {
        ...
        def save_container(path, jobs, container_kind, container_key, container_payload):
            if container_kind == "list":
                ...
                # Preserve any scheduler metadata ARES keeps next to the jobs list.
