...
struct ARESSessionResumeInvocation: Equatable, Sendable {
    let sessionID: String
    ...
    init(sessionID: String, connection: ConnectionProfile) {
        self.sessionID = sessionID
        self.hermesProfileName = connection.cliARESProfileName
        self.startupCommandLine = connection.remoteARESCommandLine(arguments: Self.buildArguments(
            hermesProfileName: connection.cliARESProfileName,
            sessionID: sessionID
        ))
    }
...
struct ARESTUIInvocation: Equatable, Sendable {
    let sessionID: String?
    let hermesProfileName: String?
    let startupCommandLine: String
    ...
    init(sessionID: String?, connection: ConnectionProfile) {
        self.sessionID = sessionID
        self.hermesProfileName = connection.cliARESProfileName
        self.startupCommandLine = connection.remoteARESCommandLine(
...
enum ARESChatTransportMode: Equatable, Sendable {
    case native
    case fallback
}

enum ARESPromptKind: String, Equatable, Hashable, Sendable {
    case approval
    case clarify
    case sudo
    case secret
...
struct ARESPromptCard: Identifiable, Equatable, Hashable, Sendable {
    let id: String
    let sessionID: String?
    let requestID: String
    let kind: ARESPromptKind
    var title: String
    var message: String
    var choices: [String]
...
enum ARESPromptResponse: Equatable, Sendable {
    case approval(Bool)
    case text(String)
}

struct ARESToolActivityCard: Identifiable, Equatable, Hashable, Sendable {
    let id: String
    var title: String
    var status: String
    var detail: String?
...
struct ARESChatTurnResult: Codable, Sendable {
    let ok: Bool
    let sessionID: String?
    let stdout: String?
    let stderr: String?
...
enum ARESGatewayHistoryDecoder {
