# WebUI to ares-agent source dependency contract

This document records the current WebUI dependency on the ares-agent source
...
- `docker-compose.two-container.yml` mounts `ares-agent-src` at `/opt/ares`
  in the agent service and read-only at
  `/home/areswebui/.ares/ares-agent` in the WebUI service.
...
`ares-agent-src` mount. The WebUI should communicate with ares-agent through
...
| --- | --- | --- |
| `docker_agent_source_volume` | Compose files and Docker docs expose `ares-agent-src` and `/opt/ares` to make the agent checkout visible to WebUI. | Remove the WebUI source mount only after startup install and runtime imports have migrated. This needs Docker/compose follow-up work, not a runtime behavior change in this audit PR. |
| `startup_dependency_install` | `api/startup.py` discovers `ARES_WEBUI_AGENT_DIR` or `$ARES_HOME/ares-agent`; `server.py` calls `auto_install_agent_deps()` after import verification fails; `docker_init.bash` installs from the staged agent source. | Replace source-tree pip installs with a packaged ares-agent WebUI client plus an agent health/version capability contract. Keep `ARES_WEBUI_AGENT_DIR` during migration as an override/debug path, but it should stop being required in normal multi-container startup. |
| `runtime_auxiliary_model_metadata` | `api/streaming.py`, `api/routes.py`, `api/config.py`, and `api/providers.py` import `agent.auxiliary_client`, `agent.model_metadata`, `agent.models_dev`, `ares_cli.models`, and `agent.account_usage`. | Existing provider/model WebUI endpoints can keep serving UI data where they already wrap agent helpers. Missing surfaces need ares-agent endpoints or a client package for auxiliary task config, text auxiliary calls, context length, token estimate, provider catalog, and account usage. |
| `runtime_session_state` | `api/streaming.py`, `api/goals.py`, and `api/state_sync.py` import `ares_state.SessionDB` directly. | Move cross-container state reads and writes behind ares-agent session/state endpoints. WebUI-only presentation state can remain local, but agent session storage should not be opened from the WebUI container. |
| `runtime_gateway_provider` | `api/streaming.py` and `api/routes.py` import `ares_cli.runtime_provider`; adapter helpers such as `agent.anthropic_adapter` are also imported for gateway normalization. | Provider resolution, runtime routing, and gateway invocation should be ares-agent API calls. WebUI can keep request validation and display formatting, but it should not import runtime provider internals from the agent checkout. |
| `webui_local_or_client_package` | WebUI imports `ares_cli.auth`, `ares_cli.config`, `ares_cli.plugins`, `ares_cli.profiles`, `ares_cli.goals`, `agent.skill_utils`, `agent.credential_pool`, and `ares_constants`. | Pure schemas, constants, and parsing helpers can move into a small versioned client/shared package. Privileged data such as credential pools, auth status, profile mutation, plugin discovery, and goal persistence need ares-agent endpoints. UI-only formatting can remain in WebUI. |
...
- Provider/model catalog routes currently backed by `ares_cli.models`.
...
- Runtime-provider choices currently backed by `ares_cli.runtime_provider`.

### New ares-agent endpoints needed
...
- SessionDB/session state read and write operations now using
  `ares_state.SessionDB`.
- Runtime provider resolution and gateway normalization now using
  `ares_cli.runtime_provider` and `agent.anthropic_adapter`.
...
- Credential/auth/account usage access now using `agent.credential_pool`,
  `ares_cli.auth`, and `agent.account_usage`.
...
- Shared constants currently imported from `ares_constants`.
...
- Typed response/request models for the new ares-agent endpoints.

### WebUI-local items

The WebUI can keep code that is only presentation, validation, or routing glue:

- User-facing diagnostics that display whether `ARES_WEBUI_AGENT_DIR` is set.
...
- Startup dependency installation and `ARES_WEBUI_AGENT_DIR` are reported.
