...
| **All-in-one image** (community fork — third-party, not maintained by us) | Podman 3.4 / multi-arch / supervisord-style preference. | [sunnysktsang/ares-suite](https://github.com/sunnysktsang/ares-suite) — see [#1399](https://github.com/nesquena/ares-webui/issues/1399) for the original discussion |

> **Note (v0.14+):** If you use `docker-compose.three-container.yml`, both
> `ares-agent` and `ares-dashboard` initialise from the same image and write
> to the same `ares-home` volume simultaneously. This can cause overlapping lock
...
> avoids this by running a single `ares-agent` process that serves both the
...
Ares WebUI assumes one operator controls the container, mounted Ares home, and workspace.
The image does **not** install `sudo`, does not add runtime users to a sudo group, and does not
grant `NOPASSWD` escalation. If an agent/tool process gains a shell as `areswebui`, it should
...
UID/GID alignment and ownership preparation before the app can read `~/.ares`, `/workspace`,
`/app`, and `/uv_cache`. After that setup, `docker_init.bash` re-execs itself as the unprivileged
`areswebui` user and starts the server there. Init scratch files under `/tmp/areswebui_init`
...
```bash
git clone https://github.com/nesquena/ares-webui
cd ares-webui
...
That's it for a real personal Docker install. Your existing `~/.ares`
...
them manually from the Tasks panel. In Docker, scheduled jobs require the Ares gateway daemon
to tick while you are away. If System Settings shows `Gateway not configured`,
use `docker-compose.two-container.yml`,
`docker-compose.three-container.yml`, or run `ares gateway` separately before
...
your real `~/.ares` unless you intentionally want to test real state. Use an
isolated Ares home and follow
[`docs/onboarding-agent-checklist.md`](onboarding-agent-checklist.md) instead.

> **Linux note**: run Compose as the user who owns the Ares home. The command
> `sudo docker compose up -d` can make Compose expand `${HOME}` as `/root`, so
> the default `${HOME}/.ares` bind mount becomes `/root/.ares` instead of
> your user's real Ares directory. Prefer adding your user to the `docker group`
...
The default Ares WebUI Docker image stays CPU-only. GPU user-space packages
...
`areswebui` runtime user, so the WebUI process keeps access to `/dev/dri`.
...
```bash
docker run --rm --gpus all ares-webui:gpu nvidia-smi
```
...
and container toolkit setup before debugging Ares WebUI. The container image
...
The cron list itself is still read from the shared `ARES_HOME` volume, not
...
The three-container layout adds the dashboard but is otherwise the same shape. If you must stay single-container, you can run `ares gateway` inside the container as a long-lived background process, but the compose split is sturdier.
...
services:
  ares-webui:
    environment:
      ...
      # ARES_WEBUI_GATEWAY_BASE_URL=http://ares-agent:8642 also works.
```
...
services:
  ares-webui:
    environment:
      ...
      # ARES_WEBUI_GATEWAY_API_KEY=... when the gateway requires API auth.
```

`ARES_WEBUI_GATEWAY_USE_RUNS_API=true` is required for gateway approval cards because approval-capable gateway runs emit approval requests on the runs API transport. Leaving it unset keeps browser chat on the legacy chat-completions path.
...
and the WebUI still needs `ARES_API_URL` or `ARES_WEBUI_GATEWAY_BASE_URL` to
...
```bash
export GATEWAY_BASE_URL="${ARES_API_URL:-${ARES_WEBUI_GATEWAY_BASE_URL:-http://ares-agent:8642}}"
docker compose -f docker-compose.two-container.yml exec ares-agent ares gateway status
...
For container-to-container diagnostics, set one of `ARES_API_URL` or `ARES_WEBUI_GATEWAY_BASE_URL` in the WebUI environment, then restart WebUI.

Refs #2785, #4483.

## Three-service unified setup (v0.14+)

Since v0.14, `ares-agent` can serve the gateway API and the built-in dashboard
from the same process by setting `ARES_DASHBOARD_HOST` and
`ARES_DASHBOARD_PORT`. Running agent and dashboard in one container means a
single writer to `ares-home`, eliminating the concurrent-init write conflicts
that occur when `ares-agent` and `ares-dashboard` both start from the same
...
| Service | Image | Ports |
|---|---|---|
| `ares-agent` | `nousresearch/ares-agent:latest` | 8642 (gateway), 9119 (dashboard) |
| `ares-webui` | `ghcr.io/nesquena/ares-webui:latest` | 8787 (chat UI) |
...
services:
  ares-agent:
    ...
    environment:
      ...
      - ARES_UID=${UID:-1000}
      - ARES_GID=${GID:-1000}
      - ARES_DASHBOARD_HOST=0.0.0.0
      - ARES_DASHBOARD_PORT=9119
...
Check `ares gateway run --help` for the exact flag names for your agent release —
the env-var equivalents shown above (`ARES_DASHBOARD_HOST`, `ARES_DASHBOARD_PORT`)
...
`ares-dashboard` to `ares-agent` with a `condition: service_healthy` healthcheck
...
Until the compatibility boundary work in [#1925](https://github.com/nesquena/ares-webui/issues/1925) and [#2491](https://github.com/nesquena/ares-webui/issues/2491) land, the WebUI and Ares Agent deployment should be treated as a release pair: the WebUI release is tested against its matching agent release and should be upgraded/pinned together.

If you use `latest`, use it consistently on both sides and avoid mixing a fixed tag with `latest`:
- fixed WebUI tag + `ares-agent:latest`
- `ares-webui:latest` + fixed `ares-agent` tag
...
If you see behavior issues after a mixed-version upgrade, capture both WebUI and ares-agent versions and the compose layout in the issue.

### 1. "Permission denied" at startup

**Symptom**: Container starts but immediately crashes, logs show:
```
PermissionError: [Errno 13] Permission denied: '/home/areswebui/.ares/...'
```
...
**Symptom**: You set `ARES_HOME_MODE=0640` (or some other group-readable mode) on your host `.env` file, container starts, then errors out:
```
...
- `ARES_SKIP_CHMOD=1` — bypass the fixer entirely
- `ARES_HOME_MODE=0640` — allow group bits, only strip world-readable

Both are documented in `api/startup.py::fix_credential_permissions()`.

> ⚠️ **Multi-container warning**: `ARES_HOME_MODE` has DIFFERENT semantics in the agent image vs. the WebUI:
> - **WebUI**: credential FILE mode threshold (`0640` allows group bits on `.env`)
> - **Agent**: `ARES_HOME` *directory* mode (default `0700`)
>
> `0640` on a directory has no owner-execute bit, so the agent can't traverse its own home → bricked. For multi-container setups, use `ARES_HOME_MODE=0750` (group-traversable) or `0701` (x-only). The compose files have per-service comments that match each side's semantics.
...
```
!! WARNING: ares-agent source not found.
!!   Looked in: /home/areswebui/.ares/ares-agent
!!              /opt/ares
```

**Cause**: The agent's source (`/opt/ares` inside the agent container) needs to be exposed to the WebUI container via a shared volume. The two-container compose file does this via `ares-agent-src` named volume, but if you're using bind mounts incorrectly the path won't resolve.

**Fix**: Use the named volumes that ship with `docker-compose.two-container.yml` — don't replace them with bind mounts unless you know what you're doing. The agent container writes its source to `/opt/ares`, and the WebUI mounts that volume at `/home/areswebui/.ares/ares-agent`.

If you must use a bind mount: pick a host path, then mount it to `/opt/ares` in the agent container AND `/home/areswebui/.ares/ares-agent` in the WebUI container.
...
- **Combined image** ([sunnysktsang/ares-suite](https://github.com/sunnysktsang/ares-suite)) — community fork that ships agent+webui+dashboard in one container

### 6. "config.yaml not loaded"

**Symptom**: You have a `config.yaml` in your host `~/.ares/`, but the WebUI shows "no model configured" or doesn't pick up your custom providers.

**Cause**: Either the file isn't readable (UID/GID issue, see #1) or it's not in the expected path inside the container.

**Fix**:
- Verify: `docker exec ares-webui ls -la /home/areswebui/.ares/config.yaml`
...
### 7. "On Podman: can't share .ares between containers"
...
**Fix**: Either upgrade to Podman 4+ (which fixes this), or use the [single-container setup](#5-minute-quickstart-single-container), or use the [community all-in-one image](https://github.com/sunnysktsang/ares-suite).

### 8. "API base URL set to localhost fails from Docker" (#3012)

**Symptom**: A provider, local model server, webhook, or custom API works on the host at `http://localhost:<port>`, but fails when the same URL is configured in Ares WebUI running in Docker.
...
The WebUI container doesn't ship with the agent's Python deps — at startup it runs `uv pip install /home/areswebui/.ares/ares-agent` to install them from the shared volume. The WebUI mount is read-only; the agent container is the only writer.

## Upgrading the agent container

The `ares-agent-src` named volume is initialised from the agent image's `/opt/ares` on first `up`. Docker reuses the volume verbatim on every subsequent `up` — **even after `docker pull` of a newer agent image**. The cached volume content masks the new image's source tree, so a fresh `docker pull` of `nousresearch/ares-agent:latest` does not by itself give you the new agent code, dependencies, or entrypoint.

This is the root cause of [#1416](https://github.com/nesquena/ares-webui/issues/1416): the symptom looked like a missing entrypoint, but the entrypoint was actually present in the new image and hidden behind the stale named volume.
...
Replace `<project>` with your Compose project name (the parent directory by default; check with `docker volume ls`). The `ares-home` volume (config, sessions, state) is left untouched — only `ares-agent-src` (the agent's installed Python source) is recreated.

> The single-container setup (`docker-compose.yml`) does not use `ares-agent-src` and is not affected by this upgrade pattern — pulling a newer WebUI image and `docker compose up -d --force-recreate` is sufficient.
...
- The gateway API (port 8642) is bound by the agent service only; the WebUI cannot bind it. Other containers reach the gateway via the `ares-net` Docker network.
...
- **Filesystem boundary.** Both services share `ares-home` (config, sessions, state), and the WebUI mounts the agent's installed source from `ares-agent-src`. The WebUI mount is read-only (since v0.51.84), but the agent service still has write access, and both services share the home volume.
...
- **Trust boundary on the agent source.** The WebUI installs Python dependencies from the shared `ares-agent-src` volume at startup. The read-only mount means a compromised WebUI cannot rewrite the agent source, but it does run code from that volume.
...
The direct source mount is a compatibility bridge, not the long-term API contract. The current source/API boundary inventory and decoupling task list live in [`docs/rfcs/agent-source-boundary.md`](rfcs/agent-source-boundary.md) for [#2453](https://github.com/nesquena/ares-webui/issues/2453). If you customize the compose files with bind mounts, keep the WebUI-side agent source mount read-only unless you are intentionally doing local development; `docker_init.bash` warns at startup when that path is writable.

## Bind-mount migration (advanced)

If you really need to bind-mount an existing host `~/.ares` (e.g. you're keeping config in dotfiles, sharing with a non-Docker `ares` install, etc.):
...
volumes:
  ...
  ares-agent-src:
    ...
    driver_opts:
      type: none
      o: bind
      device: /opt/ares-agent-source
```

**Critical requirements**:

1. The host directory MUST be readable by your container UID. Run `id -u` on the host and ensure `~/.ares` is owned by that UID (or readable via group bits).
2. ALL containers sharing the volume must run as the SAME UID/GID. Set `UID=$(id -u)` and `GID=$(id -g)` in `.env`.
3. If you run Compose with sudo, do not rely on `${HOME}` defaults: `sudo` often changes `$HOME` to `/root`, so `${ARES_HOME:-${HOME}/.ares}` becomes `/root/.ares`. Prefer running Docker as your user; otherwise pass absolute paths with `sudo -E`, for example `ARES_HOME=/home/youruser/.ares ARES_WORKSPACE=/home/youruser/workspace sudo -E docker compose up -d`, and confirm the rendered bind mount with `docker compose config`.
4. If your host `.env` is mode 0640, set `ARES_SKIP_CHMOD=1` or `ARES_HOME_MODE=0640` so the startup hook doesn't try to enforce 0600.
...
- #1416 — agent-image upgrade requires removing `ares-agent-src` named volume (see [Upgrading the agent container](#upgrading-the-agent-container))
- #1389 — `ARES_HOME_MODE` override (fixed in v0.50.254 — agent honors `ARES_SKIP_CHMOD` and `ARES_HOME_MODE`)
...
- #3006 — `sudo docker compose` can mount `/root/.ares` instead of the user's Ares home
- #3243 — optional GPU runtime image/docs for containerized acceleration workloads
- #858 — two-container `/opt/ares` path confusion
...
If you hit a new failure mode not covered here, please [open an issue](https://github.com/nesquena/ares-webui/issues/new) with:

1. Which compose file you used
2. The error from `docker logs ares-webui`
3. `docker exec ares-webui id` output
4. `docker exec ares-webui ls -la /home/areswebui/.ares` output
