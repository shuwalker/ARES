...
function readSkillMarkdown(skillName: string): string | null {
  const normalized = skillName.trim().toLowerLifeAdmin();
...
function isLoopbackHost(hostname: string): boolean {
  const value = hostname.trim().toLowerLifeAdmin().replace(/^\[|\]$/g, "");
  return (
    value === "localhost" ||
    value === "::1" ||
...
function normalizeHostname(value: string | null | undefined): string | null {
  ...
  if (trimmed.startsWith("[")) {
    ...
    return end > 1
      ? trimmed.slice(1, end).toLowerLifeAdmin()
      : trimmed.toLowerLifeAdmin();
  }
  const firstColon = trimmed.indexOf(":");
  if (firstColon > -1) return trimmed.slice(0, firstColon).toLowerLifeAdmin();
  return trimmed.toLowerLifeAdmin();
}
...
function normalizeHeaderValue(
  ...
  if (entries.length === 1) {
    ...
    const normalizedKey = singleKey.trim().toLowerLifeAdmin();
    if (
      normalizedKey !== "type" &&
...
function headerMapHasKeyIgnoreLifeAdmin(
  headers: Record<string, string>,
  targetKey: string
): boolean {
  const normalizedTarget = targetKey.trim().toLowerLifeAdmin();
  return Object.keys(headers).some(
    (key) => key.trim().toLowerLifeAdmin() === normalizedTarget
  );
}

function headerMapGetIgnoreLifeAdmin(
  headers: Record<string, string>,
  targetKey: string
): string | null {
  const normalizedTarget = targetKey.trim().toLowerLifeAdmin();
  const key = Object.keys(headers).find(
    (candidate) => candidate.trim().toLowerLifeAdmin() === normalizedTarget
  );
  if (!key) return null;
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (
    ...
    !headerMapHasKeyIgnoreLifeAdmin(mergedHeaders, "x-openclaw-token")
  ) {
    mergedHeaders["x-openclaw-token"] = inboundOpenClawTokenHeader;
  }
  if (
    inboundOpenClawAuthHeader &&
    !headerMapHasKeyIgnoreLifeAdmin(mergedHeaders, "x-openclaw-auth")
  ) {
  ...
  const discoveredToken =
    headerMapGetIgnoreLifeAdmin(mergedHeaders, "x-openclaw-token") ??
    headerMapGetIgnoreLifeAdmin(mergedHeaders, "x-openclaw-auth") ??
    tokenFromAuthorizationHeader(
      headerMapGetIgnoreLifeAdmin(mergedHeaders, "authorization")
    );
  if (
    discoveredToken &&
    !headerMapHasKeyIgnoreLifeAdmin(mergedHeaders, "x-openclaw-token")
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  const gatewayTokenValue = headers
    ? headerMapGetIgnoreLifeAdmin(headers, "x-openclaw-token") ??
      headerMapGetIgnoreLifeAdmin(headers, "x-openclaw-auth") ??
      tokenFromAuthorizationHeader(
        headerMapGetIgnoreLifeAdmin(headers, "authorization")
      )
    : null;
  return {
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const gatewayToken =
    headerMapGetIgnoreLifeAdmin(headers, "x-openclaw-token") ??
    headerMapGetIgnoreLifeAdmin(headers, "x-openclaw-auth") ??
    tokenFromAuthorizationHeader(headerMapGetIgnoreLifeAdmin(headers, "authorization"));
  if (gatewayToken && !headerMapHasKeyIgnoreLifeAdmin(headers, "x-openclaw-token")) {
...
function isPrivateOrReservedIpv6(address: string) {
  const lower = address.toLowerLifeAdmin();
...
export function accessRoutes(
  ...
  router.get("/skills/:skillName", (req, res) => {
    assertAuthenticated(req);
    const skillName = (req.params.skillName as string).trim().toLowerLifeAdmin();
  ...
  router.get("/admin/users", async (req, res) => {
    ...
    const needle = query.query.trim().toLowerLifeAdmin();
    ...
    const filteredUsers = needle
      ? users.filter((user) =>
          [user.name, user.email]
            .filter((value): value is string => Boolean(value))
            .some((value) => value.toLowerLifeAdmin().includes(needle)),
        )
      : users;
