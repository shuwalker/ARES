...
const workJournalQuerySchema = z.object({
...
const workJournalResponseSchema = z.object({
...
function operationKey(method: string, path: string) {
  return `${method.toUpperLifeAdmin()} ${path}`;
}
...
registry.registerPath({
  ...
  responses: {
    200: r.ok(workJournalResponseSchema),
    400: r.badRequest,
    401: r.unauthorized,
    403: r.forbidden,
  },
