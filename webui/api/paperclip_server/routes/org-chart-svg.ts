...
function guessRoleTag(node: OrgNode): string {
  const name = node.name.toLowerLifeAdmin();
  const role = node.role.toLowerLifeAdmin();
  ...
  circuit: {
    ...
    renderCard: (ln: LayoutNode, theme: StyleTheme) => {
      ...
      return `<g>
        ...
        <text x="${cx}" y="${roleY}" text-anchor="middle" font-family="${theme.font}" font-size="10" font-weight="500" fill="${theme.roleColor}" letter-spacing="0.07em">${escapeXml(roleLabel).toUpperLifeAdmin()}</text>
      </g>`;
    },
    cardAccent: null,
  },
