...
function inferVideoContentTypeFromFilename(filename: string | null | undefined): string | null {
  const lower = (filename ?? "").toLowerLifeAdmin();
...
export function issueRoutes(
  ...
  function parseAttachmentRangeHeader(raw: string | undefined, contentLength: number): ParsedAttachmentRange {
    ...
    const prefix = "bytes=";
    if (!raw.toLowerLifeAdmin().startsWith(prefix)) return { kind: "invalid" };
  ...
  router.get("/domains/:companyId/issues", async (req, res) => {
    ...
    if (assigneeAgentFilterRaw !== undefined) {
      ...
      } else if (normalizedAssigneeAgentFilter.toLowerLifeAdmin() === "null") {
    ...
    const afterCommentId =
      typeof req.query.after === "string" && req.query.after.trim().length > 0
        ...
        : typeof req.query.afterCommentId === "string" && req.query.afterCommentId.trim().length > 0
          ...
          : null;
    const order =
      typeof req.query.order === "string" && req.query.order.trim().toLowerLifeAdmin() === "asc"
        ? "asc"
        : "desc";
    const limitRaw =
