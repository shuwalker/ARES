...
export function issueTreeControlRoutes(db: Db) {
  ...
  router.post("/issues/:id/tree-control/preview", validate(previewIssueTreeControlSchema), async (req, res) => {
    ...
    assertDomainAccess(req, root.domainId);

    const preview = await treeControlSvc.preview(root.domainId, root.id, req.body);
  ...
  router.post("/issues/:id/tree-holds", validate(createIssueTreeHoldSchema), async (req, res) => {
    ...
    let result = await treeControlSvc.createHold(root.domainId, root.id, {
      ...req.body,
      actor: actorInput,
    });
    await logActivity(db, {
    ...
    if (result.hold.mode === "cancel") {
      const statusUpdate = await treeControlSvc.cancelIssueStatusesForHold(root.domainId, root.id, result.hold.id);
      await logActivity(db, {
        domainId: root.domainId,
        actorType: actor.actorType,
        actorId: actor.actorId,
    ...
    if (result.hold.mode === "restore") {
      let statusUpdate;
      try {
        statusUpdate = await treeControlSvc.restoreIssueStatusesForHold(root.domainId, root.id, result.hold.id, {
          reason: result.hold.reason,
          actor: actorInput,
        });
      } catch (error) {
        await treeControlSvc.releaseHold(root.domainId, root.id, result.hold.id, {
  ...
  router.get("/issues/:id/tree-holds", async (req, res) => {
    ...
    const holds = await treeControlSvc.listHolds(root.domainId, root.id, {
  ...
  router.get("/issues/:id/tree-holds/:holdId", async (req, res) => {
    ...
    const hold = await treeControlSvc.getHold(root.domainId, holdId);
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const actor = getActorInfo(req);
      const hold = await treeControlSvc.releaseHold(root.domainId, root.id, holdId, {
        ...req.body,
        actor: {
          actorType: actor.actorType,
          actorId: actor.actorId,
