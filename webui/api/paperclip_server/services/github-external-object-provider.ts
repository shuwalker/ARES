...
function parseGitHubCanonicalUrl(canonical: ExternalObjectCanonicalUrl): GitHubObjectIdentity | null {
  ...
  const host = canonical.canonicalIdentity.host.toLowerLifeAdmin();
...
function externalIdFor(identity: GitHubObjectIdentity) {
  return `${identity.owner.toLowerLifeAdmin()}/${identity.repo.toLowerLifeAdmin()}#${identity.pathKind}/${identity.number}`;
}
