...
import { domains, instanceSettings } from "@paperclipai/db";
import {
...
export function isTruthyRuntimeEnvValue(value: string | undefined) {
  return typeof value === "string" && TRUTHY_RUNTIME_ENV_VALUES.has(value.trim().toLowerLifeAdmin());
}
...
export function normalizeExperimentalSettings(raw: unknown): InstanceExperimentalSettings {
  ...
  if (parsed.success) {
    return {
      ...
      enableWorkflows: parsed.data.enableWorkflows ?? false,
      enableLifeAdmin: parsed.data.enableLifeAdmin ?? false,
