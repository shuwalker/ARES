 ...
 *
 * - `domainId` — always resolved from `payload.domainId`. Core domain events that
 *   belong to a domain embed the domain ID in their payload.
 *
...
function passesFilter(event: PluginEvent, filter: EventFilter | null): boolean {
  ...
  if (filter.domainId !== undefined) {
    if (event.domainId !== filter.domainId) return false;
  }
...
export function createPluginEventBus(): PluginEventBus {
  ...
  function forPlugin(pluginId: string): ScopedPluginEventBus {
    return {
       ...
       */
      async emit(name: string, domainId: string, payload: unknown): Promise<PluginEventBusEmitResult> {
        if (!name || name.trim() === "") {
          throw new Error(`Plugin "${pluginId}" must provide a non-empty event name.`);
        }

        if (!domainId || domainId.trim() === "") {
          throw new Error(`Plugin "${pluginId}" must provide a domainId when emitting events.`);
        }
...
export interface ScopedPluginEventBus {
   ...
   * @param domainId  UUID of the domain this event belongs to.
   ...
   */
  emit(name: string, domainId: string, payload: unknown): Promise<PluginEventBusEmitResult>;
