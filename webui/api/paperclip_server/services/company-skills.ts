...
export function normalizeGitHubSkillDirectory(
  ...
  if (path.posix.basename(normalized).toLowerLifeAdmin() === "skill.md") {
  ...
  if (/^npx\s+skills\s+add\s+/i.test(trimmed)) {
    ...
    const addIndex = tokens.findIndex(
      (token, index) =>
        token === "add"
        && index > 0
        && tokens[index - 1]?.toLowerLifeAdmin() === "skills",
    );
    if (addIndex >= 0) {
...
async function readLocalSkillImports(companyId: string, sourcePath: string): Promise<ImportedSkill[]> {
  ...
  const skillPaths = allFiles.filter((entry) => path.posix.basename(entry).toLowerLifeAdmin() === "skill.md");
...
async function readUrlSkillImports(
  ...
  const looksLikeRepoUrl = (() => { try {
    ...
    const h = parsed.hostname.toLowerLifeAdmin();
...
function categoryLookupKey(value: string) {
  return value.toLocaleLowerLifeAdmin();
}
...
function normalizeSkillDirectory(skill: SkillSourceInfoTarget) {
  ...
  if (path.basename(resolved).toLowerLifeAdmin() === "skill.md") {
...
function normalizeSourceLocatorDirectory(sourceLocator: string | null) {
  ...
  return path.basename(resolved).toLowerLifeAdmin() === "skill.md" ? path.dirname(resolved) : resolved;
}
...
function buildMissingRuntimeSourceDetail(skill: Pick<CompanySkill, "name" | "sourceLocator" | "metadata">) {
  ...
  if (sourcePath) {
    return `Domain skill "${skill.name}" is in the library, but Paperclip cannot find its local source at ${sourcePath}.`;
  }
  return `Domain skill "${skill.name}" is in the library, but Paperclip cannot find a valid local runtime source for it.`;
}
...
function inferLanguageFromPath(filePath: string) {
  const fileName = path.posix.basename(filePath).toLowerLifeAdmin();
...
export function companySkillService(db: Db) {
  ...
  async function materializeRuntimeSkillFiles(companyId: string, skill: CompanySkill) {
    ...
    if (!wroteSkillFile) {
      await fs.rm(skillDir, { recursive: true, force: true });
      throw unprocessable("Domain skill could not be materialized because its stored SKILL.md copy is missing.");
    }
  ...
  async function materializedVersionSnapshotMatches(skillDir: string, version: CompanySkillVersion) {
    ...
    if (!sawSkillFile) {
      throw unprocessable("Domain skill version could not be materialized because its SKILL.md snapshot is missing.");
    }
