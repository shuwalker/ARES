...
export function issueApprovalService(db: Db) {
  ...
  async function assertIssueAndApprovalSameDomain(issueId: string, approvalId: string) {
    ...
    if (issue.domainId !== approval.domainId) {
      throw unprocessable("Issue and approval must belong to the same domain");
    }
  ...
  return {
    ...
    link: async (issueId: string, approvalId: string, actor?: LinkActor) => {
      const { issue } = await assertIssueAndApprovalSameDomain(issueId, approvalId);
    ...
    unlink: async (issueId: string, approvalId: string) => {
      await assertIssueAndApprovalSameDomain(issueId, approvalId);
      await db
    ...
    linkManyForApproval: async (approvalId: string, issueIds: string[], actor?: LinkActor) => {
      ...
      for (const row of rows) {
        if (row.domainId !== approval.domainId) {
          throw unprocessable("Issue and approval must belong to the same domain");
        }
      }
