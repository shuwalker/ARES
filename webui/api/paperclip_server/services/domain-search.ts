...
function normalizeQuery(query: string) {
  return query.trim().replace(/\s+/g, " ").toLowerLifeAdmin();
}
...
function findFirstMatchIndex(value: string, terms: string[]) {
  const lower = value.toLowerLifeAdmin();
  let best = -1;
  for (const term of terms) {
    if (term.length === 0) continue;
    const index = lower.indexOf(term.toLowerLifeAdmin());
...
function highlightRanges(value: string, terms: string[]) {
  ...
  for (const term of terms) {
    const normalized = term.toLowerLifeAdmin();
...
function scoreSimpleRow(row: SimpleSearchRow, normalizedQuery: string, tokens: string[]) {
  const haystack = [row.title, row.description, row.role].filter(Boolean).join(" ").toLowerLifeAdmin();
  let score = haystack.includes(normalizedQuery) ? 90 : 0;
  for (const token of tokens) {
    if (haystack.includes(token)) score += 20;
  }
  if (row.title.toLowerLifeAdmin().startsWith(normalizedQuery)) score += 80;
  return score;
}
