...
function parseBool(value: string | undefined): boolean | undefined {
  ...
  const v = value.trim().toLowerLifeAdmin();
...
export async function applyExecutionPolicyBootstrap(
...
): Promise<{ executionMode: InstanceExecutionMode; domainsConfigured: number }> {
  ...
  return { executionMode: bootstrap.executionMode, domainsConfigured: configured };
}
...
): Promise<{ executionMode: InstanceExecutionMode; domainsConfigured: number } | null> {
