...
export function issueTreeControlService(db: Db) {
  async function listTreeIssues(domainId: string, rootIssueId: string): Promise<TreeIssue[]> {
    const root = await db
      .select()
      .from(issues)
      .where(and(eq(issues.id, rootIssueId), eq(issues.domainId, domainId)))
      .then((rows) => rows[0] ?? null);
    if (!root) {
    ...
    while (frontier.length > 0) {
      ...
      const children = await db
        .select()
        .from(issues)
        .where(and(eq(issues.domainId, domainId), inArray(issues.parentId, parentIds)))
  ...
  async function activeRunsForTree(domainId: string, treeIssues: TreeIssue[]) {
  ...
  async function activeHoldsByIssueId(domainId: string, issueIds: string[]) {
    ...
    const rows = await db
      ...
      .where(
        and(
          eq(issueTreeHoldMembers.domainId, domainId),
  ...
  async function activeCancelSnapshotsByIssueId(domainId: string, rootIssueId: string) {
    const activeCancelHolds = await listHolds(domainId, rootIssueId, {
      status: "active",
      mode: "cancel",
      includeMembers: true,
    });
  ...
  async function activePauseHoldsForIssueIds(domainId: string, issueIds: string[]) {
    if (issueIds.length === 0) return [];
    return db
      .select()
      .from(issueTreeHolds)
      .where(
        and(
          eq(issueTreeHolds.domainId, domainId),
          eq(issueTreeHolds.status, "active"),
  ...
  async function getActivePauseHoldGate(
    ...
    while (
      ...
      const parent: { parentId: string | null } | null = await db
        ...
        .from(issues)
        .where(and(eq(issues.id, currentIssueId), eq(issues.domainId, domainId)))
        .then((rows) => rows[0] ?? null);
  ...
  async function preview(
    ...
    const treeIssues = await listTreeIssues(domainId, rootIssueId);
    const issueIds = treeIssues.map((issue) => issue.id);
    const [activeRunRows, holdsByIssueId, activeCancelSnapshots] = await Promise.all([
      activeRunsForTree(domainId, treeIssues),
      activeHoldsByIssueId(domainId, issueIds),
      input.mode === "restore"
        ? activeCancelSnapshotsByIssueId(domainId, rootIssueId)
  ...
  async function createHold(
    ...
    const holdPreview = await preview(domainId, rootIssueId, {
      mode: input.mode,
    ...
    if (input.mode === "resume") {
      ...
      const activePauseHolds = await activePauseHoldsForIssueIds(domainId, issueIds);
      ...
      if (resumedPauseHoldIds.length > 0) {
        await Promise.all(
          activePauseHolds.map((pauseHold) =>
            releaseHold(domainId, pauseHold.rootIssueId, pauseHold.id, {
      ...
      const releasedResumeHold = await releaseHold(domainId, rootIssueId, resumeHold.id, {
        reason: releaseReason,
        metadata: {
          resumedPauseHoldIds,
          resumeMode: "subtree",
  ...
  async function cancelIssueStatusesForHold(
    ...
    const hold = await getHold(domainId, holdId);
  ...
  async function restoreIssueStatusesForHold(
    ...
    const restoreHold = await getHold(domainId, restoreHoldId);
    ...
    const updatedIssues = await db.transaction(async (tx) => {
      ...
      if (releasedCancelHoldIds.length > 0) {
        await tx
          ...
          .where(and(eq(issueTreeHolds.domainId, domainId), inArray(issueTreeHolds.id, releasedCancelHoldIds)));
      }
      ...
      await tx
        ...
        .where(and(eq(issueTreeHolds.domainId, domainId), eq(issueTreeHolds.id, restoreHoldId)));
    ...
    return {
      ...
      updatedIssues,
      releasedCancelHoldIds,
      restoreHold: await getHold(domainId, restoreHoldId),
    };
  }

  async function getHold(domainId: string, holdId: string) {
    const hold = await db
      .select()
      .from(issueTreeHolds)
      .where(and(eq(issueTreeHolds.id, holdId), eq(issueTreeHolds.domainId, domainId)))
      .then((rows) => rows[0] ?? null);
    if (!hold) return null;
    const members = await db
      .select()
      .from(issueTreeHoldMembers)
      .where(and(eq(issueTreeHoldMembers.domainId, domainId), eq(issueTreeHoldMembers.holdId, holdId)))
  ...
  async function cancelUnclaimedWakeupsForTree(domainId: string, rootIssueId: string, reason: string) {
    const treeIssues = await listTreeIssues(domainId, rootIssueId);
    const issueIds = treeIssues.map((issue) => issue.id);
    if (issueIds.length === 0) return [];
    const now = new Date();
