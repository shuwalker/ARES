...
export function pluginManagedSkillService(
  ...
  async function getBinding(domainId: string, skillKey: string) {
    return db
      .select()
  ...
  async function upsertBinding(
    ...
    const existing = await getBinding(domainId, declaration.skillKey);
    if (existing) {
      if (
  ...
  async function getSkill(domainId: string, skillId: string) {
    return skills.getById(domainId, skillId);
  }
  ...
  async function managedSkillDefaultDrift(
    ...
    for (const filePath of paths) {
      ...
      try {
        currentFiles[filePath] = (await skills.readFile(domainId, skill.id, filePath))?.content ?? null;
      } catch {
  ...
  async function resolvedSkill(
    ...
    return resolution(
      ...
      skill,
      status,
      await managedSkillDefaultDrift(domainId, skill, declaration),
    );
  }
  ...
  async function importDeclaredSkill(
    ...
    const beforeByKey = mode === "reconcile"
      ? await skills.getByKey(domainId, canonicalSkillKey(options.pluginKey, declaration.skillKey))
      : null;
    if (beforeByKey) {
      await upsertBinding(domainId, declaration, beforeByKey.id);
    ...
    await upsertBinding(domainId, declaration, imported.id);
    const status: PluginManagedSkillResolution["status"] = mode === "reset" ? "reset" : "created";
    return { skill: imported, status };
  }

  async function get(skillKey: string, domainId: string) {
    const declaration = declarationFor(skillKey);
    const binding = await getBinding(domainId, skillKey);
    if (!binding) return resolvedSkill(domainId, declaration, null, "missing");
    const skill = await getSkill(domainId, binding.resourceId);
    return resolvedSkill(domainId, declaration, skill, skill ? "resolved" : "missing");
  }

  async function reconcile(skillKey: string, domainId: string) {
    const declaration = declarationFor(skillKey);
    const current = await get(skillKey, domainId);
    if (current.skill) {
      await upsertBinding(domainId, declaration, current.skill.id);
      return current;
    }
    const imported = await importDeclaredSkill(domainId, declaration, "reconcile");
    ...
    return resolvedSkill(domainId, declaration, imported.skill, imported.status);
  }

  async function reset(skillKey: string, domainId: string) {
    const declaration = declarationFor(skillKey);
    const imported = await importDeclaredSkill(domainId, declaration, "reset");
    await logActivity(db, {
      domainId,
      actorType: "plugin",
      actorId: options.pluginId,
    ...
    return resolvedSkill(domainId, declaration, imported.skill, "reset");
  }
