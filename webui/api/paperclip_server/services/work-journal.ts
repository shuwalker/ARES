...
export interface WorkJournalQuery {
  ...
  limit?: number;
  offset?: number;
  canReadIssue?: (issue: WorkJournalIssueAccessInput) => Promise<boolean>;
}
...
type RunUsage = NonNullable<WorkJournalSpan["usage"]>;
...
function actorId(type: JournalActorType, id: string) {
  return `${type}:${id}`;
}
...
export function normalizeJournalWindow(input: { from?: Date; to?: Date }, now = new Date()) {
...
export function workJournalService(db: Db) {
  async function filterReadableIssues(
    rows: IssueRow[],
    canReadIssue: NonNullable<WorkJournalQuery["canReadIssue"]> | undefined,
  ) {
  ...
  async function collectIssueIds(input: WorkJournalQuery, from: Date, to: Date) {
  ...
  async function loadIssues(input: WorkJournalQuery, issueIds: string[]) {
  ...
  async function applyUserLens(input: WorkJournalQuery, rows: IssueRow[], from: Date, to: Date) {
  ...
  async function getJournal(input: WorkJournalQuery): Promise<WorkJournalResult> {
    const { from, to, capped } = normalizeJournalWindow(input);
    ...
    const events: WorkJournalEvent[] = [];
    const edges: WorkJournalEdge[] = [];
    ...
    const spanByRunId = new Map<string, WorkJournalSpan>();
