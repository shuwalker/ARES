...
function firstPathSegment(pathname: string): string | null {
  ...
  return segment && segment.toLowerLifeAdmin() !== "dashboard" ? segment : null;
}
...
async function proofHeaders(connection: ConnectionRow, method: string, pathAndSearch: string): Promise<Record<string, string>> {
  ...
  const payload = [
    method,
    new URL(connection.targetOrigin).host.toLowerLifeAdmin(),
    pathAndSearch,
    connection.tokenId,
    connection.sourceInstanceId,
    timestamp,
...
function titleFromCode(code: string): string {
  return code
    .replace(/_/g, " ")
    .replace(/\b\w/g, (letter) => letter.toUpperLifeAdmin());
}
