...
export function pluginManagedRoutineService(
  ...
  async function getBinding(domainId: string, routineKey: string) {
    return db
      .select({
        id: pluginManagedResources.id,
        domainId: pluginManagedResources.domainId,
  ...
  async function upsertBinding(
    ...
    const existing = await getBinding(domainId, declaration.routineKey);
    if (existing) {
      return db
  ...
  async function getRoutineWithManagedBy(domainId: string, declaration: PluginManagedRoutineDeclaration) {
    const binding = await getBinding(domainId, declaration.routineKey);
  ...
  async function resolveAgentId(
    ...
    if (overrides?.assigneeAgentId !== undefined) {
      ...
      const row = await db
        .select({ id: agents.id })
        .from(agents)
        .where(and(eq(agents.domainId, domainId), eq(agents.id, overrides.assigneeAgentId)))
        .then((rows) => rows[0] ?? null);
    ...
    const row = await db
      .select({ id: agents.id })
      .from(agents)
      .where(and(eq(agents.domainId, domainId), eq(agents.id, binding.resourceId)))
      .then((rows) => rows[0] ?? null);
  ...
  async function resolveProjectId(
    ...
    if (overrides?.projectId !== undefined) {
      ...
      const row = await db
        ...
        .from(projects)
        .where(and(eq(projects.domainId, domainId), eq(projects.id, overrides.projectId)))
        .then((rows) => rows[0] ?? null);
    ...
    const row = await db
      .select({ id: projects.id })
      .from(projects)
      .where(and(eq(projects.domainId, domainId), eq(projects.id, binding.resourceId)))
      .then((rows) => rows[0] ?? null);
  ...
  async function resolveRefs(
    ...
    const [agent, project] = await Promise.all([
      resolveAgentId(domainId, declaration, overrides),
      resolveProjectId(domainId, declaration, overrides),
    ]);
  ...
  async function createManagedRoutine(
    ...
    const refs = await resolveRefs(domainId, declaration, overrides);
    if (refs.missingRefs.length > 0) {
      return resolution(domainId, declaration, null, "missing_refs", refs.missingRefs);
    }

    const created = await routinesSvc.create(domainId, {
    ...
    await upsertBinding(domainId, declaration, created.id);
    await ensureDefaultTriggers(created.id, declaration);
    const routine = await getRoutineWithManagedBy(domainId, declaration);
    ...
    return resolution(domainId, declaration, routine, "created");
  }

  async function get(routineKey: string, domainId: string) {
    const declaration = declarationFor(routineKey);
    const routine = await getRoutineWithManagedBy(domainId, declaration);
    return resolution(domainId, declaration, routine, routine ? "resolved" : "missing");
  }

  async function reconcile(routineKey: string, domainId: string, overrides?: RoutineOverrides) {
    const declaration = declarationFor(routineKey);
    const current = await get(routineKey, domainId);
    if (current.routine) {
      await upsertBinding(domainId, declaration, current.routine.id);
      await ensureDefaultTriggers(current.routine.id, declaration);
      return current;
    }
    return createManagedRoutine(domainId, declaration, overrides);
  }

  async function reset(routineKey: string, domainId: string, overrides?: RoutineOverrides) {
    const declaration = declarationFor(routineKey);
    const current = await get(routineKey, domainId);
    if (!current.routine) {
      return createManagedRoutine(domainId, declaration, overrides);
    ...
    const refs = await resolveRefs(domainId, declaration, overrides);
    if (refs.missingRefs.length > 0) {
      return resolution(domainId, declaration, current.routine, "missing_refs", refs.missingRefs);
    }
    ...
    await upsertBinding(domainId, declaration, updated.id);
    ...
    return resolution(domainId, declaration, routine, "reset");
  }
  ...
  async function run(routineKey: string, domainId: string, overrides?: RoutineOverrides) {
    const declaration = declarationFor(routineKey);
    const current = await get(routineKey, domainId);
    if (!current.routine) throw notFound("Managed routine not found");
