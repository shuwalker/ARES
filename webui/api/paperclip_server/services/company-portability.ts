...
function normalizeExportPathSegment(value: string | null | undefined, preserveLifeAdmin = false) {
  if (!value) return null;
...
function deriveLocalExportNamespace(skill: CompanySkill, slug: string) {
  ...
  if (skill.sourceLocator) {
    ...
    candidates.push(basename.toLowerLifeAdmin() === "skill.md" ? path.basename(path.dirname(skill.sourceLocator)) : basename);
  }
...
function deriveSkillExportDirCandidates(
  ...
  const pushSuffix = (value: string | null | undefined, preserveLifeAdmin = false) => {
    const normalized = normalizeExportPathSegment(value, preserveLifeAdmin);
...
function isSensitiveEnvKey(key: string) {
  const normalized = key.trim().toLowerLifeAdmin();
  return (
    normalized === "token" ||
...
function readZonedDateParts(startsAt: string, timeZone: string) {
  try {
    ...
    const weekday = WEEKDAY_TO_CRON[parts.weekday?.toLowerLifeAdmin() ?? ""];
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (frequency === "hourly") {
    ...
    const cronWeekdays = weekdays
      .map((entry) => WEEKDAY_TO_CRON[entry.toLowerLifeAdmin()])
      .filter((entry): entry is string => Boolean(entry));
...
function buildManifestFromPackageFiles(
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      attachmentMaxBytes:
        typeof paperclipDomain.attachmentMaxBytes === "number" && Number.isFinite(paperclipDomain.attachmentMaxBytes)
          ? Math.max(1, Math.floor(paperclipDomain.attachmentMaxBytes))
          : null,
      ...
      feedbackDataSharingConsentAt:
        typeof paperclipDomain.feedbackDataSharingConsentAt === "string"
          ? paperclipDomain.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(paperclipDomain.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(paperclipDomain.feedbackDataSharingTermsVersion),
    },
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  async function exportBundle(
    ...
    const normalizeProjectSelector = (selector: string) => selector.trim().toLowerLifeAdmin();
  ...
  async function importBundle(
    ...
    try {
          ...
          if (!logoFile) {
            ...
            if (!contentType || !COMPANY_LOGO_CONTENT_TYPE_EXTENSIONS[contentType]) {
              ...
              try {
                ...
                const createdAsset = await assetRecords.create(targetDomain.id, {
      ...
      const existingAgents = await agents.list(targetDomain.id);
      for (const existing of existingAgents) {
        const slug = normalizeAgentUrlKey(existing.name) ?? existing.id;
      ...
      const existingProjects = await projects.list(targetDomain.id);
      for (const existing of existingProjects) {
      ...
      if (include.agents) {
        for (const planAgent of plan.preview.plan.agentPlans) {
          ...
          let created = await agents.create(targetDomain.id, {
            ...patch,
            status: createdStatus,
          });
          await access.ensureMembership(targetDomain.id, "agent", created.id, "member", "active");
          await access.setPrincipalPermission(
            targetDomain.id,
            "agent",
            created.id,
      ...
      if (include.projects) {
        for (const planProject of plan.preview.plan.projectPlans) {
          ...
          if (planProject.action === "update" && planProject.existingProjectId) {
            ...
            const created = await projects.create(targetDomain.id, projectPatch);
      ...
      if (include.issues) {
        ...
        for (const manifestIssue of sourceManifest.issues) {
          ...
          if (manifestIssue.recurring) {
            ...
            const createdRoutine = await routines.create(targetDomain.id, {
              projectId,
              goalId: null,
              parentIssueId: null,
          ...
          const createdIssue = await issues.create(targetDomain.id, {
            projectId,
            projectWorkspaceId,
            title: manifestIssue.title,
            description,
