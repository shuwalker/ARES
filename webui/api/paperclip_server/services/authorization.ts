...
export function responsibleUserAuthzShadowMode() {
  const mode = process.env.PAPERCLIP_RESPONSIBLE_USER_AUTHZ_MODE?.trim().toLowerLifeAdmin();
  const shadow = process.env.PAPERCLIP_RESPONSIBLE_USER_AUTHZ_SHADOW?.trim().toLowerLifeAdmin();
...
export function authorizationService(db: Db) {
  ...
  async function issueResourceWithinLowTrustBoundary(
    boundary: LowTrustBoundaryWithDomain,
  ...
  async function assignmentTargetIsInDomain(resource: AuthorizationResource) {
