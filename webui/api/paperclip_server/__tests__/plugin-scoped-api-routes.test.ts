...
describe.sequential("plugin scoped API routes", () => {
  ...
  it("dispatches a board GET route with params, query, actor, and domain context", async () => {
    const apiRoutes = manifest([
      {
        routeKey: "summary.get",
        method: "GET",
        path: "/domains/:domainSlug/summary",
        auth: "board",
    ...
    const res = await request(app)
      .get(`/api/plugins/${pluginId}/api/domains/acme/summary?domainId=${domainId}&view=compact`)
    ...
    expect(workerManager.call).toHaveBeenCalledWith(pluginId, "handleApiRequest", expect.objectContaining({
      routeKey: "summary.get",
      method: "GET",
      params: { domainSlug: "acme" },
      query: { domainId, view: "compact" },
      domainId,
  ...
  it("only forwards allowlisted response headers from plugin routes", async () => {
    ...
    const res = await request(app)
      .get(`/api/plugins/${pluginId}/api/domains/acme/summary?domainId=${domainId}`);
    ...
    const apiRoutes = manifest([
      {
        ...
        domainResolution: { from: "issue", param: "issueId" },
      },
    ]);
  ...
  it("returns a clear error for disabled plugins without worker dispatch", async () => {
    ...
    const res = await request(app)
      .get(`/api/plugins/${pluginId}/api/summary?domainId=${domainId}`);
