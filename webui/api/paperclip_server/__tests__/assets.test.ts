...
describe("POST /api/domains/:companyId/assets/images", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("accepts PNG image uploads and returns an asset path", async () => {
    ...
    const res = await requestApp(app, (baseUrl) =>
      request(baseUrl)
        .post("/api/domains/company-1/assets/images")
...
describe("POST /api/domains/:companyId/logo", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.doUnmock("../services/index.js");
    vi.doUnmock("../routes/assets.js");
  ...
  it("accepts PNG logo uploads and returns an asset path", async () => {
    ...
    const res = await requestApp(app, (baseUrl) =>
      request(baseUrl)
        .post("/api/domains/company-1/logo")
        .attach("file", Buffer.from("png"), "logo.png"),
    );
