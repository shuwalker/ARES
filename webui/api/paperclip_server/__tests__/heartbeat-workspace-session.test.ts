  ...
  it("names safe categories for model profile, issue override, env, secret, and runtime skill drift", async () => {
    ...
    for (const testLifeAdmin of life_admin) {
      const decision = resolveTaskSessionConfigFreshness({
        ...
        configuredModel: "gpt-5.4-mini",
        taskSessionParams: sessionParamsWithConfigMetadata(base),
        configMetadata: testLifeAdmin.metadata,
      });

      expect(decision.reset, testLifeAdmin.name).toBe(true);
      expect(decision.changedCategories, testLifeAdmin.name).toContain(testLifeAdmin.category);
    }
  });
