...
describe.sequential("agent permission routes", () => {
  ...
  const sshCapableAdapterLifeAdmin = [
  ...
  for (const adapterLifeAdmin of sshCapableAdapterLifeAdmin) {
    it(`allows creating a ${adapterLifeAdmin.adapterType} agent with an SSH default environment`, async () => {
      const environmentId = "33333333-3333-4333-8333-333333333333";
      mockEnvironmentService.getById.mockResolvedValue({
        id: environmentId,
        companyId,
      ...
      const res = await requestApp(app, (baseUrl) => request(baseUrl)
        ...
        .send({
          ...
          role: "engineer",
          adapterType: adapterLifeAdmin.adapterType,
          adapterConfig: adapterLifeAdmin.adapterConfig,
          defaultEnvironmentId: environmentId,
        }));
  ...
  for (const adapterLifeAdmin of sshCapableAdapterLifeAdmin) {
    it(`allows updating a ${adapterLifeAdmin.adapterType} agent with an SSH default environment`, async () => {
      const environmentId = "33333333-3333-4333-8333-333333333333";
      mockEnvironmentService.getById.mockResolvedValue({
        id: environmentId,
        companyId,
