...
const mockBoardAuthService = vi.hoisted(() => ({
  ...
  resolveBoardActivityDomainIds: vi.fn(),
...
describe.sequential("cli auth routes", () => {
  ...
  it.sequential("creates a CLI auth challenge with approval metadata", async () => {
    ...
    const res = await request(app)
      ...
      .send({
        command: "paperclipai domain import",
  ...
  it.sequential("approves a CLI auth challenge for a signed-in board user", async () => {
    ...
    mockBoardAuthService.resolveBoardActivityDomainIds.mockResolvedValue(["domain-1"]);
  ...
  it.sequential("logs approve activity for instance admins without domain memberships", async () => {
    mockBoardAuthService.approveCliAuthChallenge.mockResolvedValue({
      status: "approved",
      challenge: {
        id: "challenge-2",
    ...
    mockBoardAuthService.resolveBoardActivityDomainIds.mockResolvedValue(["domain-a", "domain-b"]);
    ...
    expect(res.status).toBe(200);
    expect(mockBoardAuthService.resolveBoardActivityDomainIds).toHaveBeenCalledWith({
      userId: "admin-1",
      requestedDomainId: null,
      boardApiKeyId: "board-key-2",
    });
  ...
  it.sequential("logs revoke activity with resolved audit domain ids", async () => {
    mockBoardAuthService.assertCurrentBoardKey.mockResolvedValue({
      id: "board-key-3",
      userId: "admin-2",
    });
    mockBoardAuthService.resolveBoardActivityDomainIds.mockResolvedValue(["domain-z"]);
  ...
  it.sequential("creates a named board API key and logs audit activity", async () => {
    ...
    mockBoardAuthService.resolveBoardActivityDomainIds.mockResolvedValue(["11111111-1111-4111-8111-111111111111"]);
    ...
    const res = await request(app)
      ...
      .send({
        name: "external-admin",
        requestedDomainId: "11111111-1111-4111-8111-111111111111",
