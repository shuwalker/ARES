...
describeEmbeddedPostgres("productivity review service", () => {
  ...
  async function seedAssignedIssue(opts?: {
    ...
    const issuePrefix = `PR${domainId.replace(/-/g, "").slice(0, 6).toUpperLifeAdmin()}`;
    ...
    return { domainId, managerId, coderId, issueId, issuePrefix, createdAt };
  }
  ...
  async function listProductivityReviews(domainId: string) {
    return db
      .select()
      .from(issues)
      .where(and(eq(issues.domainId, domainId), eq(issues.originKind, PRODUCTIVITY_REVIEW_ORIGIN_KIND)))
  ...
  it("creates exactly one manager-assigned review for a no-comment run streak and rate-limits immediate refresh", async () => {
    ...
    const first = await service.reconcileProductivityReviews({ now, domainId: seeded.domainId });
    const second = await service.reconcileProductivityReviews({ now, domainId: seeded.domainId });

    expect(first.created).toBe(1);
    expect(second.updated).toBe(0);
    expect(second.existing).toBe(1);
    const reviews = await listProductivityReviews(seeded.domainId);
  ...
  it("refreshes open productivity reviews only once per interval and caps refresh comments", async () => {
    ...
    const service = productivityReviewService(db);
    await service.reconcileProductivityReviews({ now, domainId: seeded.domainId });
    const [review] = await listProductivityReviews(seeded.domainId);
  ...
  it("caps productivity review creation per source issue in the rolling creation window", async () => {
    ...
    expect(await listProductivityReviews(seeded.domainId)).toHaveLength(3);
  });
  ...
  it("does not count cancelled productivity reviews toward the creation cap", async () => {
    ...
    expect(await listProductivityReviews(seeded.domainId)).toHaveLength(4);
  });
  ...
  it("creates a long-active review without enabling a continuation hold", async () => {
    ...
    const result = await service.reconcileProductivityReviews({ now, domainId: seeded.domainId });
  ...
  it("ignores non-assignee comments when evaluating high-churn productivity reviews", async () => {
    ...
    expect(result.created).toBe(0);
    expect(await listProductivityReviews(seeded.domainId)).toHaveLength(0);
  });
