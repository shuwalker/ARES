...
import { domains, cloudUpstreamConnections, cloudUpstreamRuns, domainSkills, createDb } from "@paperclipai/db";
...
describeEmbeddedPostgres("cloud upstream persistence", () => {
  ...
  it("marks orphaned running runs failed during startup reconciliation", async () => {
    ...
    await db.insert(cloudUpstreamRuns).values([
      cloudRunRow({ id: runningRunId, connectionId, domainId, status: "running" }),
      cloudRunRow({ id: succeededRunId, connectionId, domainId, status: "succeeded", completedAt: reconciledAt }),
    ]);
  ...
  it("rejects a new run when the connection already has a running run", async () => {
    ...
    const runningRunId = randomUUID();
    await seedDomain(domainId);
    await db.insert(cloudUpstreamConnections).values(cloudConnectionRow({ id: connectionId, domainId }));
    await db.insert(cloudUpstreamRuns).values(
      cloudRunRow({ id: runningRunId, connectionId, domainId, status: "running" }),
    );

    await expect(cloudUpstreamService(db).createRun({ connectionId, domainId })).rejects.toMatchObject({
      status: 409,
  ...
  it("preserves a cancelled run when an in-flight createRun tries to finish", async () => {
    ...
    globalThis.fetch = vi.fn(async (input) => {
      ...
      if (path.endsWith("/chunks")) {
        ...
        await service.cancelRun(connectionId, run.id, domainId);
    ...
    const result = await service.createRun({ connectionId, domainId });
...
function cloudConnectionRow(input: { id: string; domainId: string }) {
