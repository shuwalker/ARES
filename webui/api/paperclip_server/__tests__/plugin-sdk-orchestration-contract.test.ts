...
function issue(input: Partial<Issue> & Pick<Issue, "id" | "domainId" | "title">): Issue {
  const now = new Date();
  return {
    id: input.id,
...
describe("plugin SDK orchestration contract", () => {
  it("supports expanded issue create fields and relation helpers", async () => {
    ...
    harness.seed({
      issues: [issue({ id: blockerIssueId, domainId, title: "Blocker" })],
    });
    ...
    await expect(harness.ctx.issues.relations.get(created.id, domainId)).resolves.toEqual({
      blockedBy: [
    ...
    await expect(harness.ctx.issues.relations.removeBlockers(created.id, [blockerIssueId], domainId)).resolves.toEqual({
      blockedBy: [],
      blocks: [],
    });

    await expect(harness.ctx.issues.relations.addBlockers(created.id, [blockerIssueId], domainId)).resolves.toEqual({
      blockedBy: [expect.objectContaining({ id: blockerIssueId })],
      blocks: [],
    });

    await expect(
      harness.ctx.issues.getSubtree(created.id, domainId, { includeRelations: true }),
  ...
  it("enforces checkout and wakeup capabilities in the test harness", async () => {
    ...
    await expect(
      harness.ctx.issues.requestWakeup(checkedOutIssueId, domainId, {
        reason: "mission_advance",
      }),
    ).resolves.toMatchObject({ queued: true });

    await expect(
      harness.ctx.issues.requestWakeups([checkedOutIssueId], domainId, {
        reason: "mission_advance",
  ...
  it("rejects wakeups when blockers are unresolved", async () => {
    ...
    harness.seed({
      issues: [
        issue({ id: blockerIssueId, domainId, title: "Unresolved blocker", status: "todo" }),
        issue({
          id: blockedIssueId,
          domainId,
          title: "Blocked work",
    ...
    await expect(
      harness.ctx.issues.requestWakeup(blockedIssueId, domainId),
