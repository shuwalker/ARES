...
function createDbStub(inviteRows: unknown[], domainRows: unknown[]) {
  let selectCall = 0;
  return {
    select() {
      selectCall += 1;
...
describe("GET /invites/:token/logo", () => {
  ...
  it("serves the domain logo for an active invite without domain auth", async () => {
    const invite = {
      id: "invite-1",
      domainId: "domain-1",
    ...
    expect(mockStorage.headObject).toHaveBeenCalledWith("domain-1", "assets/domains/logo-1");
    expect(mockStorage.getObject).toHaveBeenCalledWith("domain-1", "assets/domains/logo-1");
  });
