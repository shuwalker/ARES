...
describe("issue goal fallback", () => {
  it("assigns the domain goal when creating an issue without project or goal", () => {
    expect(
      resolveIssueGoalId({
        projectId: null,
        goalId: null,
  ...
  it("does not force a domain goal when the project has no goal", () => {
    expect(
      resolveIssueGoalId({
        projectId: "project-1",
        goalId: null,
  ...
  it("backfills the domain goal on update for legacy no-project issues", () => {
    expect(
      resolveNextIssueGoalId({
  ...
  it("switches from the domain fallback to the project goal when a project is added later", () => {
    expect(
      resolveNextIssueGoalId({
        currentProjectId: null,
