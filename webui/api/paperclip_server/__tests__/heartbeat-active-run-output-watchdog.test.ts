...
async function truncateDomainsWithDeadlockRetry(db: ReturnType<typeof createDb>) {
  for (let attempt = 0; attempt < 5; attempt += 1) {
    try {
      await db.execute(sql.raw(`TRUNCATE TABLE "domains" CASCADE`));
      return;
    } catch (error) {
...
describeEmbeddedPostgres("active-run output watchdog", () => {
  ...
  afterEach(async () => {
    ...
    await truncateDomainsWithDeadlockRetry(db);
  });
  ...
  async function seedRunningRun(opts: {
    ...
    const issuePrefix = `W${domainId.replace(/-/g, "").slice(0, 6).toUpperLifeAdmin()}`;
    ...
    if (opts.logChunk) {
      ...
      const handle = await store.begin({ domainId, agentId: coderId, runId });
    ...
    return { domainId, managerId, coderId, issueId, runId, issuePrefix };
  }

  it("creates one medium-priority evaluation issue for a suspicious silent run", async () => {
    const now = new Date("2026-04-22T20:00:00.000Z");
    const { domainId, managerId, runId } = await seedRunningRun({
      now,
      ageMs: ACTIVE_RUN_OUTPUT_SUSPICION_THRESHOLD_MS + 60_000,
    });
    const heartbeat = heartbeatService(db);

    const first = await heartbeat.scanSilentActiveRuns({ now, domainId });
    const second = await heartbeat.scanSilentActiveRuns({ now, domainId });
    ...
    const evaluations = await db
      .select()
      .from(issues)
      .where(and(eq(issues.domainId, domainId), eq(issues.originKind, "stale_active_run_evaluation")));
    ...
    expect(evaluations[0]).toMatchObject({
      ...
      originId: runId,
      originFingerprint: `stale_active_run:${domainId}:${runId}`,
    });
  ...
  it("redacts sensitive values from actual run-log evidence", async () => {
    ...
    const { domainId } = await seedRunningRun({
      now,
      ageMs: ACTIVE_RUN_OUTPUT_SUSPICION_THRESHOLD_MS + 60_000,
      logChunk: [
    ...
    await heartbeat.scanSilentActiveRuns({ now, domainId });
  ...
  it("raises critical stale-run evaluations with high priority but does not hard-block the source issue", async () => {
    const now = new Date("2026-04-22T20:00:00.000Z");
    const { domainId, issueId } = await seedRunningRun({
      now,
      ageMs: ACTIVE_RUN_OUTPUT_CRITICAL_THRESHOLD_MS + 60_000,
    });
    const heartbeat = heartbeatService(db);

    const result = await heartbeat.scanSilentActiveRuns({ now, domainId });
    ...
    const blockerRelations = await db
      .select()
      .from(issueRelations)
      .where(and(eq(issueRelations.domainId, domainId), eq(issueRelations.relatedIssueId, issueId)));
  ...
  it("emits the source-issue escalation comment only once across repeated critical scans", async () => {
    ...
    const second = await heartbeat.scanSilentActiveRuns({ now: later, domainId });
    expect(second.created).toBe(0);
  ...
  it("does not re-file after a stale-run evaluation is dismissed as false positive", async () => {
    const now = new Date("2026-04-22T20:00:00.000Z");
    const { domainId, runId, managerId } = await seedRunningRun({
      now,
      ageMs: ACTIVE_RUN_OUTPUT_SUSPICION_THRESHOLD_MS + 60_000,
    });
    const heartbeat = heartbeatService(db);
  ...
  it("folds terminal source issues with same-run durable evidence instead of creating watchdog work", async () => {
    const now = new Date("2026-04-22T20:00:00.000Z");
    const { domainId, coderId, issueId, runId } = await seedRunningRun({
      now,
      ageMs: ACTIVE_RUN_OUTPUT_CRITICAL_THRESHOLD_MS + 60_000,
      sourceStatus: "done",
  ...
  it("still escalates terminal source issues without same-run terminal evidence", async () => {
    const now = new Date("2026-04-22T20:00:00.000Z");
    const { domainId, runId } = await seedRunningRun({
      now,
      ageMs: ACTIVE_RUN_OUTPUT_CRITICAL_THRESHOLD_MS + 60_000,
      sourceStatus: "done",
    });
  ...
  it("still escalates when a same-run comment is followed by another actor marking the source done", async () => {
    const now = new Date("2026-04-22T20:00:00.000Z");
    const { domainId, issueId, runId, issuePrefix } = await seedRunningRun({
      now,
      ageMs: ACTIVE_RUN_OUTPUT_CRITICAL_THRESHOLD_MS + 60_000,
  ...
  it("folds existing evaluation and active watchdog recovery action idempotently", async () => {
    const now = new Date("2026-04-22T20:00:00.000Z");
    const { domainId, managerId, issueId, runId, issuePrefix } = await seedRunningRun({
      now,
      ageMs: ACTIVE_RUN_OUTPUT_CRITICAL_THRESHOLD_MS + 60_000,
      sourceStatus: "done",
      sameRunTerminalEvidence: "activity",
    ...
    await db.insert(issueRecoveryActions).values({
      ...
      fingerprint: `active-run-watchdog:${domainId}:${runId}:${issueId}`,
      evidence: { runId },
  ...
  it("skips snoozed runs and healthy noisy runs", async () => {
    ...
    const staleResult = await heartbeat.scanSilentActiveRuns({ now, domainId: stale.domainId });
    const noisyResult = await heartbeat.scanSilentActiveRuns({ now, domainId: noisy.domainId });
  ...
  it("records watchdog decisions through recovery owner authorization", async () => {
    ...
    const scan = await heartbeat.scanSilentActiveRuns({ now, domainId });
  ...
  it("rejects agent watchdog decisions using issues not bound to the target run", async () => {
    const now = new Date("2026-04-22T20:00:00.000Z");
    const { domainId, managerId, coderId, runId, issuePrefix } = await seedRunningRun({
      now,
      ageMs: ACTIVE_RUN_OUTPUT_SUSPICION_THRESHOLD_MS + 60_000,
    });
    const heartbeat = heartbeatService(db);
    ...
    await db.insert(issues).values({
      ...
      originKind: "stale_active_run_evaluation",
      originId: otherRunId,
      originFingerprint: `stale_active_run:${domainId}:${otherRunId}`,
    });
  ...
  it("suppresses repeat alerts when evaluation is closed on the board without a watchdog decision", async () => {
    ...
    const third = await heartbeat.scanSilentActiveRuns({ now, domainId });
  ...
  it("still allows re-arm after continue decision even when issue was closed on board", async () => {
    ...
    const rearm = await heartbeat.scanSilentActiveRuns({ now: rearmAt, domainId });
