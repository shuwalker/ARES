...
function manifest(): PaperclipPluginManifestV1 {
  return {
    ...
    skills: [{
      ...
      description: "Use LLM Wiki tools to maintain domain knowledge.",
      files: [{
...
describeEmbeddedPostgres("plugin-managed skills", () => {
  ...
  async function seedDomainAndPlugin(pluginManifest = manifest()) {
  ...
  it("installs and resolves managed domain skills by stable resource key", async () => {
  ...
  it("preserves operator edits during reconcile and restores manifest defaults on reset", async () => {
    const { domainId, services } = await seedDomainAndPlugin();
    const created = await services.skills.managedReconcile({ domainId, skillKey: "wiki-maintainer" });
    ...
    await db
      ...
      .where(eq(domainSkills.id, created.skillId!));

    const reconciled = await services.skills.managedReconcile({ domainId, skillKey: "wiki-maintainer" });
    ...
    const reset = await services.skills.managedReset({ domainId, skillKey: "wiki-maintainer" });
  ...
  it("relinks an existing canonical skill without overwriting operator edits", async () => {
    ...
    const relinked = await services.skills.managedReconcile({ domainId, skillKey: "wiki-maintainer" });
  ...
  it("reports drift when installed skill files differ from plugin defaults", async () => {
    ...
    const drifted = await services.skills.managedReconcile({ domainId, skillKey: "wiki-maintainer" });
  ...
  it("adds the canonical managed key to manifest-provided markdown skills", async () => {
    ...
    const { domainId, services } = await seedDomainAndPlugin(pluginManifest);

    const created = await services.skills.managedReconcile({ domainId, skillKey: "markdown-skill" });
