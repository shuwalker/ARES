...
interface RuntimeContractLifeAdmin {
  name: string;
  driver: string;
...
describeEmbeddedPostgres("environment runtime driver contract", () => {
  ...
  async function seedEnvironment(input: {
    ...
    if (typeof config.privateKey === "string" && config.privateKey.length > 0) {
      const secret = await secretService(db).create(domainId, {
  ...
  async function runContract(testLifeAdmin: RuntimeContractLifeAdmin) {
    const cleanup = await testLifeAdmin.setup?.();
    try {
      const runtime = environmentRuntimeService(db);
      const { domainId, environment, issueId, runId } = await seedEnvironment({
        driver: testLifeAdmin.driver,
        config: testLifeAdmin.config,
      });
      ...
      expect(acquired.lease.domainId).toBe(domainId);
      ...
      testLifeAdmin.expectLease(acquired.lease, environment);
  ...
  const contractLifeAdmin: RuntimeContractLifeAdmin[] = [
    {
      name: "local",
      driver: "local",
      config: {},
  ...
  for (const testLifeAdmin of contractLifeAdmin) {
    it(`${testLifeAdmin.name} satisfies the acquire/release host contract`, async () => {
      await runContract(testLifeAdmin);
    });
  }
