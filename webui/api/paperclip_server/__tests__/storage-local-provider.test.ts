...
describe("local disk storage provider", () => {
  ...
  it("round-trips bytes through storage service", async () => {
    ...
    const fetched = await service.getObject("domain-1", stored.objectKey);
  ...
  it("streams only requested byte ranges", async () => {
    ...
    const fetched = await service.getObject("domain-1", stored.objectKey, { range: { start: 2, end: 5 } });
    const fetchedBody = await readStreamToBuffer(fetched.stream);
  ...
  it("blocks cross-domain object access", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-storage-"));
    tempRoots.push(root);
    ...
    await expect(service.getObject("domain-b", stored.objectKey)).rejects.toMatchObject({ status: 403 });
  });
  ...
  it("delete is idempotent", async () => {
    ...
    await service.deleteObject("domain-1", stored.objectKey);
    await service.deleteObject("domain-1", stored.objectKey);
    await expect(service.getObject("domain-1", stored.objectKey)).rejects.toMatchObject({ status: 404 });
  });
});
