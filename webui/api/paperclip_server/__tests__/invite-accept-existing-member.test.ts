...
describe("POST /invites/:token/accept", () => {
  ...
  it("does not consume a human invite when the signed-in user is already a domain member", async () => {
    ...
    expect(res.body.error).toBe("You already belong to this domain");
    expect(updateMock).not.toHaveBeenCalled();
  });

  it("grants domain access immediately for a human invite", async () => {
  ...
  it("replays a consumed human invite for the same user and repairs domain access", async () => {
