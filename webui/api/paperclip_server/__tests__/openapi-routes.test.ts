...
const explicitOpenApiCoverageExclusions = new Set([
  // Workflow routes are experimental and not yet represented in the public OpenAPI document.
  "workflows.ts",
  // LifeAdmin routes are experimental (enableLifeAdmin flag) and not yet in the public OpenAPI document.
  "life_admin.ts",
]);
...
function resolveMountedPath(file: string, prefix: string, routePath: string) {
  if ((file === "domains.ts" || file === "health.ts") && routePath === "/") {
    return prefix;
  }
  if (file === "domains.ts" || file === "health.ts") {
...
function loadActualRoutes() {
  ...
  for (const file of fs.readdirSync(ROUTES_DIR).filter((entry) => entry.endsWith(".ts"))) {
    ...
    for (const match of source.matchAll(ROUTE_LITERAL_PATTERN)) {
      const method = match[1].toUpperLifeAdmin();
  ...
  for (const [routePath, pathItem] of Object.entries<Record<string, Record<string, unknown>>>(spec.paths ?? {})) {
    for (const method of Object.keys(pathItem)) {
      if (HTTP_METHODS.has(method)) {
        routes.add(`${method.toUpperLifeAdmin()} ${routePath}`);
      }
    }
  }
...
describe("openapi routes", () => {
  it("serves the generated OpenAPI document", async () => {
    ...
    expect(res.body.paths["/api/domains"].post.responses["201"]).toBeDefined();
    expect(res.body.paths["/api/domains"].post.requestBody.content["application/json"].schema).toMatchObject({
      type: "object",
      properties: {
  ...
  it("documents auth and reviewed response-code invariants", () => {
    ...
    expect(spec.paths["/api/domains/import"].post.responses["202"]).toBeDefined();
  });
});
