...
if (!embeddedPostgresSupport.supported) {
  console.warn(
    `Skipping embedded Postgres domain skill test run tests on this host: ${embeddedPostgresSupport.reason ?? "unsupported environment"}`,
  );
}

describeEmbeddedPostgres("domainSkillService skill test runs", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("paperclip-domain-skill-test-runs-");
  ...
  afterEach(async () => {
    ...
    await db.delete(domainSkillTestRuns);
    await db.delete(domainSkillTestRunTemplates);
  ...
  async function seedSkillAndAgent() {
    ...
    return { domainId, skillId, agentId };
  }

  const runDeps = (domainId: string) => ({
    createHarnessIssue: async (issue: Parameters<Parameters<typeof svc.createTestRun>[4]["createHarnessIssue"]>[0]) => {
      await db.insert(issues).values({ ...issue, domainId, priority: "medium" });
      return { id: issue.id };
    },
  ...
  it("cleans up the harness issue if persisting the test run fails", async () => {
    const { domainId, skillId, agentId } = await seedSkillAndAgent();
  ...
  it("appends the built-in default template while keeping the input snapshot clean", async () => {
    ...
    expect(run.renderedTemplateBody).toContain(`domain/${domainId}/review`);
  ...
  it("manages custom templates and renders only explicit placeholders", async () => {
    const { domainId, skillId, agentId } = await seedSkillAndAgent();
    const template = await svc.createTestRunTemplate(domainId, {
      name: "Focused smoke",
      description: "Short run",
      body: "Run {{skillName}} v{{skillVersion}} for {{runId}} on {{issueId}} into {{outputDocumentKey}}.",
    }, { type: "user", userId: "local-board" });

    const listed = await svc.listTestRunTemplates(domainId);
    ...
    await expect(
      svc.createTestRunTemplate(domainId, {
        name: "Bad template",
        body: "Use {{unknownPlaceholder}}.",
      }, { type: "user", userId: "local-board" }),
    ).rejects.toThrow(/unknown template placeholder/i);

    const updated = await svc.updateTestRunTemplate(domainId, template.id, {
    ...
    await expect(
      svc.updateTestRunTemplate(domainId, "built-in:default-test-template", { name: "Changed" }),
    ).rejects.toThrow(/read-only/i);

    const deleted = await svc.deleteTestRunTemplate(domainId, template.id);
    expect(deleted?.deletedAt).toBeInstanceOf(Date);
    expect((await svc.listTestRunTemplates(domainId)).map((entry) => entry.id)).toEqual([
      "built-in:default-test-template",
    ]);
  });

  it("rejects unknown or cross-domain template ids", async () => {
    const first = await seedSkillAndAgent();
    const second = await seedSkillAndAgent();
    const otherTemplate = await svc.createTestRunTemplate(second.domainId, {
      name: "Other domain",
  ...
  it("re-run can use the viewed template body snapshot after source template edits", async () => {
    ...
    await svc.updateTestRunTemplate(domainId, template.id, {
  ...
  it("only deletes terminal runs and soft-deletes them out of history", async () => {
    ...
    await expect(
      svc.deleteTestRun(domainId, skillId, run.id, { hideHarnessIssue: async () => null }),
    ).rejects.toThrow(/cancel the run/i);

    await svc.completeTestRunForIssue({ domainId, issueId: run.issueId, outcome: "succeeded" });

    const hidden: string[] = [];
    const deleted = await svc.deleteTestRun(domainId, skillId, run.id, {
    ...
    expect(await svc.listTestRuns(domainId, skillId)).toHaveLength(0);
    expect(await svc.getTestRunDetail(domainId, skillId, run.id)).toBeNull();

    // Deleting again is a no-op 404 (returns null).
    expect(
      await svc.deleteTestRun(domainId, skillId, run.id, { hideHarnessIssue: async () => null }),
    ).toBeNull();
  });
  ...
  it("ignores superseded test harness issue transitions", async () => {
    ...
    const listed = await svc.listTestRuns(domainId, skillId);
    ...
    expect(await svc.getTestRunDetail(domainId, skillId, first.id)).not.toBeNull();
    expect(await svc.markTestRunRunning(domainId, first.issueId)).toBeNull();
    ...
    expect(await svc.cancelTestRun(domainId, skillId, first.id, {
    ...
    const firstRow = await db
      .select({
        ...
        error: domainSkillTestRuns.error,
        supersededAt: domainSkillTestRuns.supersededAt,
        outputSnapshot: domainSkillTestRuns.outputSnapshot,
      })
      .from(domainSkillTestRuns)
      .where(eq(domainSkillTestRuns.id, first.id))
      .then((rows) => rows[0] ?? null);
    ...
    const deletedFirst = await svc.deleteTestRun(domainId, skillId, first.id, {
      hideHarnessIssue: async (issueId) => {
    ...
    expect((await svc.listTestRuns(domainId, skillId)).map((run) => run.id)).toEqual([replacement.id]);

    const runningReplacement = await svc.markTestRunRunning(domainId, replacement.issueId);
  ...
  it("snapshots output and keeps run history after harness issue retention", async () => {
    ...
    await db.insert(agents).values({
      ...
      adapterConfig: {
        model: "gpt-5.4",
        paperclipSkillSync: { desiredSkills: [`domain/${domainId}/review`] },
    ...
    const input = await svc.createTestInput(domainId, skillId, {
      name: "life_admin/simple",
      content: "Try the review skill",
    }, { type: "user", userId: "local-board" });
    const run = await svc.createTestRun(domainId, skillId, {
      inputId: input.id,
      agentId,
    }, { type: "user", userId: "local-board" }, {
      createHarnessIssue: async (issue) => {
    ...
    await db
      ...
      .where(eq(domainSkillTestRuns.id, run.id));

    const pruned = await svc.pruneExpiredTestHarnessIssues(domainId);
    expect(pruned.pruned).toBe(1);
    const detail = await svc.getTestRunDetail(domainId, skillId, run.id);
  ...
  it("hydrates rich documents, attachments, and work products scoped to the run's harness issue", async () => {
    ...
    // Sibling run in the same domain whose content must not leak into `run`'s detail.
    ...
    async function seedIssueContent(issueId: string, marker: string) {
      ...
      await db.insert(issueDocuments).values({ domainId, issueId, documentId, key: "output" });
      ...
      await db.insert(issueAttachments).values({ id: attachmentId, domainId, issueId, assetId });
  ...
  it("keeps hydration domain-scoped and reports a deleted harness issue", async () => {
    const first = await seedSkillAndAgent();
    const second = await seedSkillAndAgent();
    const run = await svc.createTestRun(
      first.domainId,
    ...
    // Cross-domain access never resolves another domain's run.
    expect(await svc.getTestRunDetail(second.domainId, first.skillId, run.id)).toBeNull();
    expect(await svc.getTestRunDetail(second.domainId, second.skillId, run.id)).toBeNull();
    ...
    await db
      ...
      .where(eq(domainSkillTestRuns.id, run.id));
    const detail = await svc.getTestRunDetail(first.domainId, first.skillId, run.id);
    expect(detail).not.toBeNull();
    expect(detail?.harnessIssue).toBeNull();
