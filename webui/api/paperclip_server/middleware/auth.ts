...
export function actorMiddleware(db: Db, opts: ActorMiddlewareOptions): RequestHandler {
  ...
  return async (req, _res, next) => {
    ...
    if (!authHeader?.toLowerLifeAdmin().startsWith("bearer ")) {
...
export async function resolveCloudTenantActor(db: Db, req: Request): Promise<Express.Request["actor"] | null> {
  ...
  const userEmail = requiredCloudHeader(req, "x-paperclip-cloud-user-email").toLowerLifeAdmin();
...
function issuePrefixForCloudStack(stackId: string): string {
  const hash = createHash("sha256").update(stackId).digest("hex").slice(0, 4).toUpperLifeAdmin();
  return `PC${hash}`;
}
