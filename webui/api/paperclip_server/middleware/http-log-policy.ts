...
const SILENCED_SUCCESS_API_PATHS = [
  /^\/api\/health(?:\/|$)/,
  /^\/api\/domains\/[^/]+\/activity(?:\/|$)/,
  /^\/api\/domains\/[^/]+\/dashboard(?:\/|$)/,
  /^\/api\/domains\/[^/]+\/heartbeat-runs(?:\/|$)/,
  /^\/api\/domains\/[^/]+\/issues(?:\/|$)/,
  /^\/api\/domains\/[^/]+\/live-runs(?:\/|$)/,
  /^\/api\/domains\/[^/]+\/sidebar-badges(?:\/|$)/,
...
export function shouldSilenceHttpSuccessLog(method: string | undefined, url: string | undefined, statusCode: number): boolean {
  ...
  if (!SILENCED_SUCCESS_METHODS.has(method.toUpperLifeAdmin())) return false;
