...
function extractHostname(req: Request): string | null {
  ...
  try {
    return new URL(`http://${raw}`).hostname.trim().toLowerLifeAdmin();
  } catch {
    return raw.trim().toLowerLifeAdmin();
  }
}
...
function normalizeAllowedHostnames(values: string[]): string[] {
  ...
  for (const value of values) {
    const trimmed = value.trim().toLowerLifeAdmin();
    if (!trimmed) continue;
    unique.add(trimmed);
  }
...
export function resolvePrivateHostnameAllowSet(opts: { allowedHostnames: string[]; bindHost: string }): Set<string> {
  ...
  const bindHost = opts.bindHost.trim().toLowerLifeAdmin();
