...
function parseAcceptEncoding(value: string | string[] | undefined): EncodingPreference[] {
  ...
  return raw
    ...
    .map((part) => {
      ...
      const encoding = encodingPart?.trim().toLowerLifeAdmin() ?? "";
      const qParam = paramParts
        .map((param) => param.trim())
        .find((param) => param.toLowerLifeAdmin().startsWith("q="));
...
function isJsonContentType(value: unknown): boolean {
  const contentType = String(value ?? "").toLowerLifeAdmin();
...
function shouldPassthroughWrite(res: Parameters<RequestHandler>[1]): boolean {
  ...
  const alreadyEncoded = res.hasHeader("Content-Encoding") && String(res.getHeader("Content-Encoding")).toLowerLifeAdmin() !== "identity";
  return (
