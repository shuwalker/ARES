...
function parseOrigin(value: string | undefined) {
  ...
  try {
    ...
    return `${url.protocol}//${url.host}`.toLowerLifeAdmin();
  } catch {
    return null;
  }
}
...
function trustedOriginsForRequest(req: Request) {
  const origins = new Set(DEFAULT_DEV_ORIGINS.map((value) => value.toLowerLifeAdmin()));
  const forwardedHost = req.header("x-forwarded-host")?.split(",")[0]?.trim();
  const host = forwardedHost || req.header("host")?.trim();
  if (host) {
    origins.add(`http://${host}`.toLowerLifeAdmin());
    origins.add(`https://${host}`.toLowerLifeAdmin());
  }
...
export function boardMutationGuard(): RequestHandler {
  return (req, res, next) => {
    if (SAFE_METHODS.has(req.method.toUpperLifeAdmin())) {
      next();
      return;
    }
