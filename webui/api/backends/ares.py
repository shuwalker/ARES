"""
Ares Backend Adapter for ARES.

This adapter wraps the existing Ares in-process execution.
Ares itself is never modified — this is pure ARES-side routing.
"""
...
class AresBackend(AgenticBackend):
    name = "ares"
    supports_tools = True
    ...
    def is_available(self) -> bool:
        # Ares is always available when the WebUI is running
        return True

    def get_worker_target(self) -> tuple:
        """Return the Ares streaming worker target."""
    ...
    def identity_projection(self) -> Dict[str, Any]:
        ...
        cfg = get_config()
        bot_name = (cfg.get("agent") or {}).get("name") or "Ares"
        return {
            "name": bot_name,
            "description": "Ares Agent in-process runtime",
            "avatar_state": "idle",
        }
    ...
    def run_turn(self, message: str, session_id: str, **kwargs) -> Dict[str, Any]:
        """
        Execute one agent turn via the Ares streaming path.

        The streaming path is the primary execution path for Ares
        ...
        stream_id = f"ares-sync-{uuid.uuid4().hex[:12]}"
        ...
        from api.config import get_config
        cfg = get_config()
        model = str((cfg.get("model") or {}).get("default", "")) or "ares-default"
