"""Let a JROS-primary Companion borrow Hermes's tools over MCP.

Hermes already ships a production MCP server built for exactly this shape —
an external agent loop that wants Hermes's tool surface without adopting its
whole runtime: ``agent/transports/hermes_tools_mcp_server.py`` (web_search,
...
not touch Hermes source, and preserves any other MCP servers already
configured there. The companion piece — actually booting JROS with MCP
enabled — is ``with_mcp=`` on ``boot_for_tui`` (JROS) / the
``jros_hermes_tools_enabled`` config flag this module's caller checks.
"""
...
HERMES_TOOLS_SERVER_NAME = "hermes-tools"
...
def hermes_mcp_available() -> bool:
    """True when ARES can find both a local JROS install and a Hermes
...
def sync_hermes_mcp_server(*, enabled: bool = True) -> dict[str, Any]:
    """Merge (or remove) the 'hermes-tools' MCP server entry in JROS's
    ...
    if enabled and _AGENT_DIR is None:
        raise RuntimeError("No Hermes Agent checkout found — cannot expose its tools over MCP.")
    ...
    servers = [s for s in data.get("servers", []) if s.get("name") != HERMES_TOOLS_SERVER_NAME]

    if enabled:
        servers.append({
            "name": HERMES_TOOLS_SERVER_NAME,
            "enabled": True,
            "command": PYTHON_EXE,
...
def set_hermes_tools_enabled(enabled: bool) -> dict[str, Any]:
    """Flip the addition on/off: sync JROS's mcp_config.json entry AND
    persist jros_hermes_tools_enabled so the next Companion boot picks it
    up (see api.jros_gateway_chat._jros_hermes_tools_enabled)."""
    from api.config import _get_config_path, _load_yaml_config_file, _save_yaml_config_file, reload_config

    sync_result = sync_hermes_mcp_server(enabled=enabled)

    config_path = _get_config_path()
    cfg = _load_yaml_config_file(config_path)
    cfg["jros_hermes_tools_enabled"] = bool(enabled)
