...
Covers:
- ARES_WEBUI_SKIP_ONBOARDING=1 bypasses the wizard unconditionally (chat_ready not required)
- ARES_WEBUI_SKIP_ONBOARDING unset leaves default behaviour unchanged
...
_COMMON_PATCHES = [
    ...
    ("api.onboarding.verify_ares_imports",lambda: (True, [], [])),
...
class TestSkipOnboardingEnvVar(unittest.TestCase):
    ...
    def test_skip_env_1_and_chat_ready_marks_completed(self):
        """ARES_WEBUI_SKIP_ONBOARDING=1 + chat_ready=True → completed=True."""
        status = self._run_status(_READY_RUNTIME, {"ARES_WEBUI_SKIP_ONBOARDING": "1"})
    ...
    def test_skip_env_true_and_chat_ready_marks_completed(self):
        """ARES_WEBUI_SKIP_ONBOARDING=true also accepted."""
        status = self._run_status(_READY_RUNTIME, {"ARES_WEBUI_SKIP_ONBOARDING": "true"})
        self.assertTrue(status["completed"])

    def test_skip_env_yes_and_chat_ready_marks_completed(self):
        """ARES_WEBUI_SKIP_ONBOARDING=yes also accepted."""
        status = self._run_status(_READY_RUNTIME, {"ARES_WEBUI_SKIP_ONBOARDING": "yes"})
        self.assertTrue(status["completed"])

    def test_skip_env_1_works_even_when_not_chat_ready(self):
        """ARES_WEBUI_SKIP_ONBOARDING=1 skips unconditionally — chat_ready is NOT required."""
        status = self._run_status(_NOT_READY_RUNTIME, {"ARES_WEBUI_SKIP_ONBOARDING": "1"})
        self.assertTrue(status["completed"],
    ...
    def test_skip_env_unset_leaves_default_false(self):
        ...
        env = {k: v for k, v in os.environ.items() if k != "ARES_WEBUI_SKIP_ONBOARDING"}
...
class TestApplyOnboardingKeySync(unittest.TestCase):
    ...
    def test_api_key_set_in_os_environ_after_apply(self):
        ...
        with patch("api.onboarding._load_yaml_config", return_value=mock_cfg), \
             ...
             patch("api.onboarding._get_active_ares_home", return_value=pathlib.Path("/tmp")):
...
class TestApplyOnboardingSkipGuard(unittest.TestCase):
    ...
    def test_apply_setup_blocked_when_skip_env_set(self):
        ...
        with patch.dict(os.environ, {"ARES_WEBUI_SKIP_ONBOARDING": "1"}, clear=False), \
