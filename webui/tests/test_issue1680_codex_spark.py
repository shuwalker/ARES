...
def _install_fake_ares_models(monkeypatch, provider_model_ids):
...
def test_openai_codex_group_uses_provider_model_ids_for_spark(monkeypatch, tmp_path):
    ...
    copied _PROVIDER_MODELS["openai-codex"] without asking ares_cli.
    """
    calls = []
    ...
    _install_fake_ares_models(monkeypatch, provider_model_ids)
    ...
    # sys.modules['ares_cli.models'] without restoring it, list_available_providers
    ...
    # gpt-5.3-codex-spark when ares_cli.provider_model_ids returns it".
    if calls != ["openai-codex"]:
        import pytest
        pytest.skip(f"ares_cli stub not active for openai-codex (likely test-isolation pollution from sibling test). Got calls={calls}")
