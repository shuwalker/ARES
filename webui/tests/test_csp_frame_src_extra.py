...
`ARES_WEBUI_CSP_FRAME_EXTRA` lets an operator widen what the WebUI page may
...
def test_csp_frame_src_default_is_self_only(monkeypatch):
    ...
    monkeypatch.delenv("ARES_WEBUI_CSP_FRAME_EXTRA", raising=False)
...
def test_csp_frame_src_extra_in_enforced_policy(monkeypatch):
    ...
    monkeypatch.setenv("ARES_WEBUI_CSP_FRAME_EXTRA", "http://127.0.0.1:3000")
...
def test_csp_frame_src_rejects_directive_injection(monkeypatch, caplog):
    ...
    assert "Ignoring invalid ARES_WEBUI_CSP_FRAME_EXTRA" in caplog.text


def test_csp_frame_src_rejects_paths(monkeypatch):
    from api.helpers import _build_csp_report_only_policy

    monkeypatch.setenv("ARES_WEBUI_CSP_FRAME_EXTRA", "https://app.example.com/embed")
    policy = _build_csp_report_only_policy()
...
def test_csp_frame_src_rejects_ws_scheme(monkeypatch):
    ...
    monkeypatch.setenv("ARES_WEBUI_CSP_FRAME_EXTRA", "wss://socket.example.com")
    policy = _build_csp_report_only_policy()
...
def test_csp_frame_src_rejects_invalid_ports(monkeypatch):
    ...
    monkeypatch.setenv("ARES_WEBUI_CSP_FRAME_EXTRA", "https://app.example.com:99999")
    policy = _build_csp_report_only_policy()
