...
def test_home_chown_skips_ares_agent_subtree():
    """The chown walk must skip the entire ares-agent mount, not just
    ...
    setup additionally mounts the whole ares-agent source tree :ro on the
    WebUI side (#2470).  Either failure mode breaks the chown walk under
    `set -e`; pruning the parent path covers both."""
    assert "chown_home_areswebui()" in INIT_SCRIPT
    # The prune target should be the whole ares-agent subtree, not just
    # the inner `.git/objects` directory. The old narrower prune was
    # insufficient once the entire mount became :ro.
    assert "-path \"/home/areswebui/.ares/ares-agent\" -prune" in INIT_SCRIPT, (
        "chown walk must prune the entire ares-agent path (not just "
...
def test_home_chown_helper_documents_readonly_mount_compat():
    ...
    chown_fn_start = INIT_SCRIPT.index("chown_home_areswebui()")
    ...
    assert "read-only" in fn_block.lower() or "ro" in fn_block.lower(), (
        "chown_home_areswebui must document why the entire ares-agent "
...
def test_root_init_uses_git_object_safe_chown_helper():
    ...
    assert "chown_home_areswebui || error_exit" in root_section
    assert 'chown -R "${WANTED_UID}:${WANTED_GID}" /home/areswebui' not in root_section
