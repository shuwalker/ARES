...
``_find_ares_md`` exactly — including the bug that ``stop_at = git_root``
...
``/tmp/x/project/subdir`` could then surface ``/tmp/x/ARES.md`` (a file
...
def _ctx(workspace):
    return read_active_project_context(pathlib.Path(workspace))


def test_non_git_workspace_does_not_walk_above_workspace(tmp_path):
    """Sentinel ARES.md placed *above* the workspace in a non-git tree must
    ...
    above.mkdir()
    (above / "ARES.md").write_text("LEAKED FROM OUTSIDE", encoding="utf-8")

    workspace = above / "project" / "subdir"
    workspace.mkdir(parents=True)
    # Intentionally no .git anywhere in the tree, and no ARES/AGENTS file
    # inside the workspace — so any non-empty result means we walked above.

    data = _ctx(workspace)

    assert "LEAKED FROM OUTSIDE" not in data["content"], (
        "Non-git workspace walk leaked an ARES.md above the workspace root"
    )
    assert data["path"] == "", (
...
def test_git_workspace_walk_to_git_root_is_unchanged(tmp_path):
    ...
    ``test_project_context_walks_ares_md_to_git_root_but_not_agents_md``
    intent specifically against the #4164 patch site."""
    root = tmp_path / "repo"
    nested = root / "src" / "deep" / "pkg"
    nested.mkdir(parents=True)
    (root / ".git").mkdir()
    (root / "ARES.md").write_text("# Root-of-repo context", encoding="utf-8")

    data = _ctx(nested)

    assert "Root-of-repo context" in data["content"]
    assert data["path"].endswith("ARES.md")


def test_non_git_workspace_bound_is_workspace_not_first_parent(tmp_path):
    """Edge case: if the workspace itself contains a ARES.md AND a parent
    ...
    parent.mkdir()
    (parent / "ARES.md").write_text("PARENT COPY", encoding="utf-8")

    workspace = parent / "ws"
    workspace.mkdir()
    (workspace / "ARES.md").write_text("WORKSPACE COPY", encoding="utf-8")
    ...
    assert not any(
        str(parent / "ARES.md") == s.get("path") for s in data["shadowed"]
    )
