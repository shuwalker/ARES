...
before the `ARES_HOME` env mutation that stamps the per-session profile.
Result: `_load_mcp_config()` always read the default profile's
`~/.ares/config.yaml`, never the non-default profile's MCP servers.

The fix moves the call past the `_ENV_LOCK` env-mutation block so
`discover_mcp_tools()` runs with the correct `ARES_HOME` for the session's
profile.
...
def test_discover_mcp_tools_called_after_ares_home_mutation():
    """The fix for #1968: `discover_mcp_tools()` must execute AFTER the
    `ARES_HOME = _profile_home` assignment, otherwise non-default profile
    MCP servers are never discovered.
    """
    home_set_line = _line_of(r"os\.environ\['ARES_HOME'\]\s*=\s*_profile_home")
    ...
    assert discover_call_line > home_set_line, (
        ...
        f"ARES_HOME mutation at line {home_set_line} (issue #1968). "
...
def test_discover_mcp_tools_called_after_env_lock_release():
    ...
    ares-agent), and holding the env lock across that would serialize all
