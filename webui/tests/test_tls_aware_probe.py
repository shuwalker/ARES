...
def _load_bootstrap():
    spec = importlib.util.spec_from_file_location("ares_bootstrap_probe", BOOTSTRAP)
...
def test_helper_scheme_http_without_tls():
    out = subprocess.run(
        ["bash", "-c", f". {HEALTH_PROBE}; ares_webui_probe_scheme"],
...
def test_helper_scheme_https_with_tls():
    out = subprocess.run(
        ...
        env={**os.environ, "ARES_WEBUI_TLS_CERT": "/c", "ARES_WEBUI_TLS_KEY": "/k"},
    )
...
def test_helper_plain_http_probe():
    with _Server() as srv:
        res = _run_helper("127.0.0.1", srv.port,
                          {"ARES_WEBUI_TLS_CERT": "", "ARES_WEBUI_TLS_KEY": ""})
...
def test_helper_self_signed_warns_and_succeeds(self_signed_cert):
    ...
    with _Server(cert, key) as srv:
        res = _run_helper("127.0.0.1", srv.port,
                          {"ARES_WEBUI_TLS_CERT": cert, "ARES_WEBUI_TLS_KEY": key})
...
def test_helper_insecure_optin_is_silent(self_signed_cert):
    ...
    with _Server(cert, key) as srv:
        res = _run_helper("127.0.0.1", srv.port, {
            "ARES_WEBUI_TLS_CERT": cert,
            "ARES_WEBUI_TLS_KEY": key,
            "ARES_WEBUI_TLS_INSECURE_PROBE": "1",
        })
    assert res.returncode == 0, res.stderr
    assert '"status": "ok"' in res.stdout
...
def test_bootstrap_probe_self_signed(monkeypatch, self_signed_cert):
    cert, key = self_signed_cert
    monkeypatch.setenv("ARES_WEBUI_TLS_CERT", cert)
    monkeypatch.setenv("ARES_WEBUI_TLS_KEY", key)
    monkeypatch.delenv("ARES_WEBUI_TLS_INSECURE_PROBE", raising=False)
    bs = _load_bootstrap()
    with _Server(cert, key) as srv:
...
def test_bootstrap_probe_returns_scheme_actually_used(monkeypatch, self_signed_cert):
    ...
    monkeypatch.delenv("ARES_WEBUI_TLS_CERT", raising=False)
    monkeypatch.delenv("ARES_WEBUI_TLS_KEY", raising=False)
    bs = _load_bootstrap()
    with _Server() as srv:
...
def test_bootstrap_probe_insecure_optin_http_fallback(monkeypatch, self_signed_cert):
    ...
    monkeypatch.setenv("ARES_WEBUI_TLS_CERT", cert)
    monkeypatch.setenv("ARES_WEBUI_TLS_KEY", key)
    monkeypatch.setenv("ARES_WEBUI_TLS_INSECURE_PROBE", "1")
    bs = _load_bootstrap()
...
def test_ctl_status_survives_readonly_env_vars(tmp_path: Path):
    ...
    (repo / ".env").write_text("UID=1000\nGID=1000\nARES_WEBUI_PORT=8787\n",
                               encoding="utf-8")
    env = os.environ.copy()
    env["ARES_HOME"] = str(tmp_path / ".ares")
    res = subprocess.run(
        ["bash", str(repo / "ctl.sh"), "status"],
        capture_output=True, text=True, env=env, timeout=30,
    )
    assert res.returncode == 0, res.stderr
    assert "ares-webui" in res.stdout
