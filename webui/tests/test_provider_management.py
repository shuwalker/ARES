...
class TestGetProviders:
    ...
    def test_openai_codex_provider_card_prefers_live_catalog(self, monkeypatch, tmp_path):
        ...
        /api/models already uses ares_cli/Codex cache discovery for Codex.  The
        ...
        fake_models = sys.modules["ares_cli.models"]
        fake_models.provider_model_ids = lambda pid: (
...
class TestSetProviderKey:
    ...
    def test_set_key_writes_to_env_file(self, monkeypatch, tmp_path):
        """Setting a key should write the env var to ~/.ares/.env."""
        _install_fake_ares_cli(monkeypatch)
        monkeypatch.setattr(profiles, "get_active_ares_home", lambda: tmp_path)
        # Also pin ARES_HOME so code that reads it directly gets tmp_path,
        ...
        try:
            ...
            assert env_path.exists(), f".env not written to {env_path}; ARES_HOME={__import__('os').environ.get('ARES_HOME')!r}"
...
class TestIssue1410OllamaEnvVarBleed:
    ...
    ares_cli/runtime_provider.py only consumes OLLAMA_API_KEY when the
