...
def test_self_persistence_contract_exports_durable_ares_layer():
    ...
    assert contract["identity_owner"] == "ares"
    ...
    assert "durable_identity" in contract["capabilities"]
...
def test_self_persistence_prompt_section_instructs_ares_not_backend():
    ...
    assert "ARES owns the self-persistent person layer" in prompt
    assert "Hermes supplies the agent loop" in prompt
    assert "JROS supplies robotics and embodiment primitives" in prompt
    assert "Do not bury identity or task continuity inside a swappable backend" in prompt
