...
Session ID format produced by ares-agent: cron_<job_id>_<YYYYMMDD>_<HHMMSS>
"""
import json
import sqlite3
...
def _write_jobs_json(ares_home, jobs):
    """Write cron/jobs.json with the given jobs list."""
    cron_dir = ares_home / "cron"
...
def fake_ares_home(tmp_path, monkeypatch):
    """Point get_cli_sessions() at a temporary ARES_HOME and disable
...
def test_cron_session_uses_job_name_when_title_missing(fake_ares_home):
