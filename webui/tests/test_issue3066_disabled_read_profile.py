...
def test_disabled_read_uses_active_profile_config(tmp_path, monkeypatch):
    ...
    config.yaml (via _get_config_path), not from ARES_HOME."""
    from api import skills_store as routes
    ...
    monkeypatch.setattr("api.skills_store._config_path", lambda: config_path)
...
def test_skills_list_disabled_reflects_active_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("api.skills_store._config_path", lambda: config_path)
    monkeypatch.setattr("api.skills_store.active_skills_dir", lambda: skills_dir)
...
def test_profile_switch_changes_disabled_state(tmp_path, monkeypatch):
    ...
    # "Active" profile A
    monkeypatch.setattr("api.skills_store._config_path", lambda: profile_a_config)
    ...
    # "Switch" to profile B
    monkeypatch.setattr("api.skills_store._config_path", lambda: profile_b_config)
...
def test_normalize_disabled_set_handles_edge_cases():
    ...
    from api.skills_store import _normalize_disabled_set
