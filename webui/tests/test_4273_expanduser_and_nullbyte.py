...
not blocked (#510/#521: Ares commonly runs as root, where /root is the
...
def test_root_remains_unblocked_for_root_deployments():
    ...
    # (Ares-as-root deployments). This pins that our null-byte work did not
    ...
    assert "'/root'" not in src and "PurePosixPath('/root')" not in src, (
        "/root must not be blocked — breaks Ares-as-root deployments (#510/#521)"
    )
...
def test_workspaces_add_route_has_home_carveout_before_block():
    ...
    src = (Path(__file__).resolve().parent.parent / "fastapi_app" / "routers" / "workspaces.py").read_text(encoding="utf-8")
    assert "_home_path()" in src and "_is_within(candidate, home)" in src, (
