...
<ares_root>/gateway.pid.  Profile-scoped gateways (running via
``gateway run --profile <name>`` or with ``active_profile`` set) write
their PID file under <ares_root>/profiles/<name>/gateway.pid instead of
...
pytest.importorskip("ares_constants", reason="ares-agent not installed")
...
def _call(monkeypatch, root: Path, profile_dir: Path | None = None) -> Path | None:
    ...
    monkeypatch.setattr(ares_constants, "get_default_ares_root", lambda: root)
    if profile_dir is not None:
        monkeypatch.setattr(profiles, "get_active_ares_home", lambda: str(profile_dir))
...
def test_returns_root_path_when_profile_lookup_raises(tmp_path, monkeypatch):
    """get_active_ares_home() raising must be caught; root path returned."""
    root = tmp_path / "ares"
    root.mkdir()
    ...
    monkeypatch.setattr(profiles, "get_active_ares_home", _raise)
