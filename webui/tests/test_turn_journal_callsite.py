...
def test_chat_start_appends_submitted_turn_journal_before_worker_thread_start():
    src = Path("api/chat_runtime.py").read_text(encoding="utf-8")
    save_idx = src.index("_prepare_session_locked(")
...
def test_chat_start_writes_turn_journal_after_session_lock_and_handles_failure():
    ...
    stream_registration_idx = src.index("STREAMS[stream_id] = channel", append_idx)
