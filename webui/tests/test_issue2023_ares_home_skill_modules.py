...
active profile home.  The modules live in ares-agent and may not be importable
...
def test_set_ares_home_patches_both_skill_tool_module_caches(monkeypatch, tmp_path):
    from api.profiles import _set_ares_home
    ...
    _set_ares_home(new_home)

    assert skills_tool.ARES_HOME == new_home
    assert skills_tool.SKILLS_DIR == new_home / "skills"
    assert skill_manager_tool.ARES_HOME == new_home
