...
type GroupMode = "status" | "profile" | "none";
...
function statusGroupLabel(status: string | undefined): string {
...
function ScheduleFormDialog({
  ...
  useEffect(() => {
    if (open && editingJob) {
      setForm({
        ...
        schedule: editingJob.schedule ?? "0 * * * *",
        prompt: editingJob.prompt ?? "",
        deliver: editingJob.deliver ?? "",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const trimmedSchedule = form.schedule.trim();
    if (!trimmedSchedule || !trimmedPrompt) return;
    setSaving(true);
    setError(null);
    try {
      if (isEdit && editingJob) {
        await aresApi.scheduleUpdate({
      ...
      setError(readableError(err, isEdit ? "Failed to update schedule." : "Failed to create schedule."));
      setSaving(false);
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEdit
              ? "Modify this scheduled job's configuration."
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="grid gap-4 py-2">
          ...
          <CronExpressionBuilder value={form.schedule} onChange={(v) => setField("schedule", v)} />
          <div className="grid gap-2">
          ...
          <div className="grid gap-2">
            <Label htmlFor="sched-deliver">Deliver to (optional)</Label>
            <Input
              id="sched-deliver"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="grid gap-2">
              <Label htmlFor="sched-model">Model (optional)</Label>
              <Input
                id="sched-model"
            ...
            <div className="grid gap-2">
              <Label htmlFor="sched-provider">Provider (optional)</Label>
              <Input
                id="sched-provider"
          ...
          <div className="grid gap-2">
            <Label htmlFor="sched-profile">Profile (optional)</Label>
            <Input
              id="sched-profile"
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={saving || !form.schedule.trim() || !form.prompt.trim()}>
...
function DeleteDialog({
  ...
  async function handleDelete() {
    ...
    setError(null);
    try {
      await aresApi.scheduleDelete(job.job_id);
      onOpenChange(false);
      onDeleted();
    } catch (err) {
      setError(readableError(err, "Failed to delete schedule."));
      setDeleting(false);
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Are you sure you want to delete &ldquo;{job?.name || job?.job_id?.slice(0, 8) || "this schedule"}&rdquo;? This cannot be undone.
          </DialogDescription>
        </DialogHeader>
        {error && <p className="text-sm text-destructive">{error}</p>}
        <DialogFooter>
...
export default function RoutinesPage() {
  ...
  const [deletingJob, setDeletingJob] = useState<ScheduleEntry | null>(null);
  ...
  const grouped = useMemo(() => {
    if (groupMode === "none") {
      return [{ label: "", items: schedules }];
    }
    const map = new Map<string, ScheduleEntry[]>();
    for (const job of schedules) {
      let key: string;
      if (groupMode === "status") {
        key = statusGroupLabel(job.status);
      } else {
        key = job.profile || "default";
      }
      const list = map.get(key) ?? [];
      list.push(job);
      map.set(key, list);
    }
    // Deterministic order for status groups
    const statusOrder = ["Active", "Paused", "Failed", "Other"];
    ...
    return keys.map((key) => ({ label: key, items: map.get(key)! }));
  }, [schedules, groupMode]);
  ...
  async function handleToggle(job: ScheduleEntry) {
    ...
    try {
      ...
      setSchedules((prev) =>
        prev.map((s) =>
          s.job_id === job.job_id
            ? { ...s, enabled: !s.enabled, status: job.enabled ? "paused" : "active" }
            : s,
        ),
      );
    } catch {
  ...
  return (
    <div className="page-stack">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <div className="mb-4 grid size-12 place-items-center rounded-lg bg-muted">
            <RefreshCw className="size-6 text-muted-foreground/50" />
          </div>
                      ...
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex shrink-0 items-center gap-1">
                          <Button
                            variant="ghost"
                            size="icon-sm"
                            aria-label={job.enabled ? "Pause routine" : "Enable routine"}
      ...
      <HistoryDialog
        ...
        onOpenChange={(v) => { if (!v) setHistoryJob(null); }}
      />
      <DeleteDialog
        job={deletingJob}
        open={deletingJob !== null}
        onOpenChange={(v) => { if (!v) setDeletingJob(null); }}
