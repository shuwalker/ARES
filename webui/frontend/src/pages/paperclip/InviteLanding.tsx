...
function formatHumanRole(role: string | null | undefined) {
  if (!role) return null;
  return role.charAt(0).toUpperLifeAdmin() + role.slice(1);
}
    ...
    <div className="min-h-screen bg-zinc-950 px-6 py-12 text-zinc-100">
      <div className="mx-auto max-w-md border border-zinc-800 bg-zinc-950 p-6" data-testid="invite-pending-approval">
        ...
        <div className="mt-4 space-y-3">
          ...
          <p className="text-sm text-zinc-400">
            Ask them to visit <a href={approvalUrl} className="text-zinc-200 underline underline-offset-2 hover:text-zinc-100">Domain Settings → Members</a> to approve your request.
          </p>
...
export function InviteLandingPage() {
  ...
  const { setSelectedCompanyId } = useDomain();
  ...
  const authMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({ queryKey: queryKeys.access.currentBoardAccess });
      const { companies: freshDomains } = await queryClient.fetchQuery(companiesListQueryOptions);

      if (invite?.companyId && freshDomains.some((company) => company.id === invite.companyId)) {
        clearPendingInviteToken(token);
        setSelectedCompanyId(invite.companyId, { source: "manual" });
        navigate("/", { replace: true });
        return;
        ...
        <div className="grid gap-6 lg:grid-cols-(--gtc-36)">
          <section className={`${panelClassName} space-y-6`}>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-(--tracking-caps) text-zinc-500">Domain</div>
              ...
              <div className="border border-zinc-800 p-3">
                ...
                <div className="mt-1 text-sm text-zinc-100">
                  {showsAgentForm ? "Agent join request" : requestedHumanRole ?? "Domain access"}
                </div>
              </div>
              <div className="border border-zinc-800 p-3">
