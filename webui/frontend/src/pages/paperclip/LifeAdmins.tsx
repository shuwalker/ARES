...
function LifeAdminEmptyHero() {
  return (
    <div className="mx-auto flex max-w-xl flex-col items-center gap-4 py-16 text-center">
      <Layers className="h-10 w-10 text-muted-foreground" />
      <h2 className="text-lg font-semibold">No cases yet</h2>
      <p className="text-sm text-muted-foreground">
        LifeAdmin are durable work products — blog posts, tweet storms, docs pages — that tasks create and
...
{slug, target_audience, publish_url} and key <release>/<slug>."`}
      ...
      <p className="text-xs text-muted-foreground">
        Feature is gated by the <code className="font-mono">enableLifeAdmin</code> experimental flag
...
export function LifeAdmin() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: "LifeAdmin" }]);
  }, [setBreadcrumbs]);
  ...
  const patchLifeAdmin = useMutation({
  ...
  const allLifeAdmin = useMemo(() => casesQuery.data ?? [], [casesQuery.data]);
  ...
  const distinctTypes = useMemo(
    () => [...new Set([
      ...allLifeAdmin.map((c) => c.caseType),
      ...viewState.typeFilters,
    ])].sort(),
    [allLifeAdmin, viewState.typeFilters],
  );

  const filtered = useMemo(() => {
    return allLifeAdmin.filter((caseRow) => caseMatchesViewFilters(caseRow, viewState));
  }, [allLifeAdmin, viewState]);
  ...
  const sortedTreeSource = useMemo(() => {
    const rows = [...allLifeAdmin];
    rows.sort(caseSortCompare);
    return rows;
  }, [allLifeAdmin, caseSortCompare]);
  ...
  const noLifeAdminAtAll = allLifeAdmin.length === 0 && activeFilters.length === 0;
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <h1 className="text-xl font-bold">LifeAdmin</h1>
                ...
                treeRows.map(({ row, depth, childCount, collapsed }) => {
                  ...
                  return (
                    <CaseListRow
                      ...
                      statusPending={patchLifeAdmin.isPending && patchLifeAdmin.variables?.caseId === row.id}
                      onStatusChange={(caseId, status) => patchLifeAdmin.mutate({ caseId, status })}
                      treeDepth={depth}
                      childCount={childCount}
