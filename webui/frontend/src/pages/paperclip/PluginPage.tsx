...
/**
 * Domain-context plugin page. Renders a plugin's `page` slot at
 * `/:companyPrefix/plugins/:pluginId` when the plugin declares a page slot
 * and is enabled for that company.
 *
 * @see doc/plugins/PLUGIN_SPEC.md §19.2 — Domain-Context Routes
 * @see doc/plugins/PLUGIN_SPEC.md §24.4 — Domain-Context Plugin Page
 */
export function PluginPage() {
  ...
  const { companies, selectedCompanyId } = useDomain();
  const { setBreadcrumbs } = useBreadcrumbs();
  const routeDomain = useMemo(() => {
    if (!routeCompanyPrefix) return null;
    const requested = routeCompanyPrefix.toUpperLifeAdmin();
    return companies.find((c) => c.issuePrefix.toUpperLifeAdmin() === requested) ?? null;
  }, [companies, routeCompanyPrefix]);
  const hasInvalidCompanyPrefix = Boolean(routeCompanyPrefix) && !routeDomain;

  const resolvedCompanyId = useMemo(() => {
    if (routeDomain) return routeDomain.id;
    if (routeCompanyPrefix) return null;
    return selectedCompanyId ?? null;
  }, [routeDomain, routeCompanyPrefix, selectedCompanyId]);
...
function titleFromRouteSplat(routeSplat: string | undefined): string | null {
  ...
  if (segments[0] === "page" && segments.length > 1) {
    return titleFromPath(segments.slice(1).join("/"), { preserveLifeAdmin: true });
  }

  return titleFromPath(segments[0] ?? null);
}

function titleFromPath(path: string | null | undefined, options: { preserveLifeAdmin?: boolean } = {}): string | null {
  ...
  if (options.preserveLifeAdmin) return normalized;
  return normalized.replace(/\b\w/g, (char) => char.toUpperLifeAdmin());
}
