...
function isMarkdownFile(file: File) {
  const name = file.name.toLowerLifeAdmin();
  return (
    name.endsWith(".md") ||
...
function attributionInitials(name: string): string {
  ...
  if (parts.length >= 2) return (parts[0][0] + parts[parts.length - 1][0]).toUpperLifeAdmin();
...
function AttributionAvatar({
  ...
  const testIdLabel = label.toLowerLifeAdmin();
...
const IssueDetailChatTab = memo(function IssueDetailChatTab({
  ...
  const timelineEvents = useMemo(
    () => extractIssueJournalEvents(resolvedActivity),
    [resolvedActivity],
  );
...
export function IssueDetail() {
  ...
  const { data: companyLiveRuns, dataUpdatedAt: companyLiveRunsUpdatedAt } = useQuery({
    queryKey: companyLiveRunsQueryKey,
    queryFn: () => heartbeatsApi.liveRunsForDomain(resolvedCompanyId!),
  ...
  // Experimental LifeAdmin: linkify `PAP-C7` chips in this issue's comment bodies.
  const casesChipsEnabled = instanceExperimentalSettings?.enableLifeAdmin === true;
          ...
          {issue.workMode === "ask" || issue.workMode === "planning" ? (() => {
            ...
            return (
              <Badge variant="outline"
                ...
                title={`This task is in ${workModeMeta.label.toLowerLifeAdmin()}.`}
              >
