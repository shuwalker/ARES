...
function fieldValueByName(fields: Record<string, unknown>, name: string): unknown {
  return fields[name] ?? fields[name.charAt(0).toUpperLifeAdmin() + name.slice(1)];
}

function caseFieldKeyVariants(key: string): string[] {
  if (!key) return [key];
  return [key, key.charAt(0).toUpperLifeAdmin() + key.slice(1), key.charAt(0).toLowerLifeAdmin() + key.slice(1)];
}
...
function CaseRelationshipsSection({
  ...
  return (
    <section className="space-y-3" aria-label="LifeAdmin relationships">
      {parent ? (
...
function CaseLabelsPicker({
  ...
  const filtered = search.trim()
    ? all.filter((l) => l.name.toLowerLifeAdmin().includes(search.trim().toLowerLifeAdmin()))
    : all;
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="start" className="w-64 p-2">
        ...
        {search.trim() && !all.some((l) => l.name.toLowerLifeAdmin() === search.trim().toLowerLifeAdmin()) && (
...
function CasePropertiesContent({
  ...
  return (
    <div className={cn("space-y-4", isFull && "space-y-6")}>
      <PropertySection title="LifeAdmin" first>
      ...
      <PropertySection title={`Children${childLifeAdmin.length > 0 ? ` ${childLifeAdmin.length}` : ""}`}>
        <CaseChildrenTree children={childLifeAdmin} />
      </PropertySection>
...
export function CaseDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: "LifeAdmin", href: caseHref() },
      { label: caseData ? `${caseData.identifier} — ${caseData.title}` : (caseIdentifier ?? "LifeAdmin") },
    ]);
  ...
  if (caseQuery.isError || !caseData) {
    return (
      <div className="mx-auto max-w-md py-16 text-center">
        <p className="text-sm text-muted-foreground">LifeAdmin not found.</p>
  ...
  function copyCaseToClipboard(currentLifeAdmin: CaseDetailData) {
    const markdown = [
      `# ${currentLifeAdmin.identifier} ${currentLifeAdmin.title}`,
      "",
      `- Key: ${currentLifeAdmin.key ?? "none"}`,
      `- Type: ${currentLifeAdmin.caseType}`,
      `- Status: ${STATUS_LABEL[currentLifeAdmin.status]}`,
      currentLifeAdmin.labels.length > 0 ? `- Labels: ${currentLifeAdmin.labels.map((label) => label.name).join(", ")}` : "- Labels: none",
    ].join("\n");
