/**
 * Work Journal page (PAP-12424 / Phase C of PAP-12405).
 *
...
import type { WorkJournalResult } from "@paperclipai/shared";
import { workJournalApi, type WorkJournalParams } from "@/api/workJournal";
...
import {
  ...
  nearestZoomForScale,
  type VisibleJournalWindow,
  type ZoomLevel,
  zoomScaleForLevel,
} from "@/components/timeline/WorkJournalChart";
...
function rangeWindow(range: DateRangeState): Pick<WorkJournalParams, "from" | "to"> | null {
...
function spanStartMs(span: WorkJournalResult["spans"][number]) {
  return new Date(span.start).getTime();
}

function spanEndMs(span: WorkJournalResult["spans"][number], fallbackEndMs: number) {
  return span.end ? new Date(span.end).getTime() : fallbackEndMs;
}

function spanWindowOverlap(
  span: WorkJournalResult["spans"][number],
...
function spanWindowTokens(span: WorkJournalResult["spans"][number], rawMs: number, clippedMs: number) {
...
function dataWindow(data: WorkJournalResult): VisibleJournalWindow {
  return {
...
export function timelineSummary(data: WorkJournalResult, visibleWindow: VisibleJournalWindow = dataWindow(data)) {
...
export function Journal() {
  ...
  const [visibleWindow, setVisibleWindow] = useState<VisibleJournalWindow | null>(null);
  ...
  const params: WorkJournalParams | null = useMemo(() => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: [...queryKeys.workJournal(selectedCompanyId ?? ""), dateRange.fromDate, dateRange.toDate],
    queryFn: () => workJournalApi.get(selectedCompanyId!, params!),
  ...
  const handleVisibleWindowChange = useCallback((nextWindow: VisibleJournalWindow) => {
  ...
  const toolbar = (
    <div className="flex flex-wrap items-start gap-3">
      {summary && <JournalSummaryStats summary={summary} />}
      <div className="ml-auto flex items-center gap-1 pt-3" aria-label="Journal zoom controls">
        <Button
          type="button"
          variant="outline"
          size="icon-xs"
