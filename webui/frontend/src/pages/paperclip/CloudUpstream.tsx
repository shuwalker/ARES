...
export function CloudUpstream() {
  const { selectedDomain, selectedCompanyId } = useDomain();
  ...
  }, [selectedDomain?.name, setBreadcrumbs]);
  ...
  const settingsPath = useMemo(() => {
    ...
    return applyCompanyPrefix("/company/settings/cloud-upstream", pathPrefix ?? selectedDomain?.issuePrefix ?? null);
  }, [location.pathname, selectedDomain?.issuePrefix]);
  ...
  if (!selectedCompanyId || !selectedDomain) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="max-w-2xl text-sm text-muted-foreground">
            Push {selectedDomain.name} into a Paperclip Cloud stack. Automations stay paused until activation.
          </p>
        </div>
