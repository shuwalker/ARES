...
export function CompanyImport() {
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      const importedDomain = await companiesApi.get(result.company.id);
      ...
      setSelectedCompanyId(importedDomain.id);
      pushToast({
        tone: "success",
      ...
      window.location.assign(`/${importedDomain.issuePrefix}/dashboard`);
    },
    onError: (err) => {
      pushToast({
        tone: "error",
        ...
        <Field label="Target" hint="Import into this company or create a new one.">
          ...
          >
            ...
            <option value="existing">
              Existing company: {selectedDomain?.name}
            </option>
          </select>
        </Field>
