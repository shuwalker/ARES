...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null) {
  const normalizedLabel = sourceLabel?.toLowerLifeAdmin() ?? "";
...
function discoveryMatchesSearch(card: DiscoveryCard, query: string): boolean {
  ...
  ].join(" ").toLowerLifeAdmin();
  return haystack.includes(query.toLowerLifeAdmin());
}
      ...
      {step === 0 ? (
        ...
        <div className="space-y-4 text-sm">
          <div className="grid grid-cols-(--gtc-26) gap-y-2">
            ...
            <span>{draft.sharingScope === "private" ? "Private" : "Domain"}</span>
          ...
          <div className="space-y-2">
            ...
            <div className="grid gap-2 sm:grid-cols-3">
              {(["company", "private"] as const).map((scope) => (
                ...
                >
                  <span className="block font-medium">{scope === "company" ? "Domain" : "Private"}</span>
...
function CatalogList({
  ...
  const lowered = catalogFilter.trim().toLowerLifeAdmin();
  const filtered = skills.filter((skill) => {
    ...
    const haystack = `${skill.name} ${skill.slug} ${skill.key} ${skill.description} ${skill.category} ${skill.tags.join(" ")} ${skill.recommendedForRoles.join(" ")}`.toLowerLifeAdmin();
...
function AttachAgentsPopover({
  ...
  const filtered = agents
    .filter((agent) => agent.name.toLowerLifeAdmin().includes(filter.toLowerLifeAdmin()))
    .sort((a, b) => {
...
function SkillList({
  ...
  const filteredSkills = skills.filter((skill) => {
    const haystack = `${skill.name} ${skill.key} ${skill.slug} ${skill.sourceLabel ?? ""}`.toLowerLifeAdmin();
    if (!haystack.includes(skillFilter.toLowerLifeAdmin())) return false;
  ...
  if (filteredSkills.length === 0) {
    if (sourceFilter !== "all" && skills.length > 0) {
      return (
        <div className="px-4 py-6 text-sm text-muted-foreground">
          No {SOURCE_FILTER_LABELS[sourceFilter].toLowerLifeAdmin()} skills installed.{" "}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              >
                <option value="company">Domain — visible inside this company</option>
