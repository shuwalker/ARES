...
export function Search() {
  ...
  useEffect(() => {
    if (!data) return;
    const upper = trimmedQuery.toUpperLifeAdmin();
    if (!IDENTIFIER_PATTERN.test(upper)) return;
    if (lastIdentifierRedirectRef.current === upper) return;
    const exact = data.results.find(
      (result) => result.type === "issue" && result.issue?.identifier?.toUpperLifeAdmin() === upper,
    );
  ...
  useEffect(() => {
    function handler(event: KeyboardEvent) {
      ...
      const tag = target?.tagName?.toLowerLifeAdmin();
  ...
  if (isEmpty) {
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          We couldn’t find a match in {describeScope(scope).toLowerLifeAdmin()}. Try widening the scope or rephrasing your
          query.
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
