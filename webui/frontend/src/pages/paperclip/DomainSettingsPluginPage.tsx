...
export function CompanySettingsPluginPage() {
  ...
  const routeDomain = useMemo(() => {
    ...
    return companies.find((company) => company.issuePrefix.toUpperLifeAdmin() === requested) ?? null;
  }, [companies, routeCompanyPrefix]);
  const hasInvalidCompanyPrefix = Boolean(routeCompanyPrefix) && !routeDomain;
  const resolvedCompanyId = routeDomain?.id ?? (routeCompanyPrefix ? null : selectedCompanyId ?? null);
