...
import type {
  WorkflowCaseChildRow,
  WorkflowCompanyCaseEvent,
  WorkflowDetail,
  WorkflowListItem,
  WorkflowStage,
...
import { WorkflowStageHistoryPanel } from "../components/WorkflowStageHistoryPanel";
...
import { formatWorkflowItemEvent, INTERNAL_FIELD_KEYS } from "../lib/pipeline-item-detail";
...
import { StageHealthWarnings } from "../components/WorkflowHealthWarnings";
import {
...
import { getWorkflowStageColumnTone } from "../lib/pipeline-stage-presentation";
...
export function resolveWorkflowSettingsFallbackStageId(
  stages: Array<Pick<WorkflowStage, "id">>,
...
function stageConfig(stage: WorkflowStage | null | undefined): StageConfig {
...
export function syncWorkflowStageAutomationVariables(
...
function stageAutomation(stage: WorkflowStage | null | undefined) {
...
function stageNewEntriesDisabled(stage: WorkflowStage | null | undefined) {
 ...
 */
function stageAutomationDetail(stage: WorkflowStage | null | undefined) {
  const automation = stageConfig(stage).automation;
  if (!automation || typeof automation !== "object" || Array.isArray(automation)) {
...
function savedStageVariables(
  ...
  return syncWorkflowStageAutomationVariables(savedTitleTemplate, savedBody, existing);
}
...
type WorkflowWithOptionalConnections = (WorkflowDetail | WorkflowListItem) & {
  connections?: WorkflowListItem["connections"];
};
...
function pipelineCarryOverFields(source: { pipeline: WorkflowWithOptionalConnections; depth: number }): CarryOverFieldOption[] {
...
function pipelineBreakdownTargetIds(pipeline: WorkflowWithOptionalConnections) {
  const ids: string[] = [];
  for (const stage of pipeline.stages ?? []) {
    const targetWorkflowId = readStageBreakdown(stage)?.targetWorkflowId;
    if (targetWorkflowId) ids.push(targetWorkflowId);
  }
  return ids;
}

function upstreamWorkflowIds(
  pipeline: WorkflowWithOptionalConnections,
  candidates: WorkflowWithOptionalConnections[],
) {
  ...
  for (const id of pipeline.connections?.upstreamWorkflowIds ?? []) ids.add(id);
  return [...ids];
}

function buildCarryOverFieldGroups(
  pipeline: WorkflowWithOptionalConnections | null,
  candidates: WorkflowWithOptionalConnections[],
): CarryOverFieldGroup[] {
  if (!pipeline) return [];
  const byId = new Map<string, WorkflowWithOptionalConnections>();
  for (const candidate of candidates) byId.set(candidate.id, candidate);
  const listedCurrentWorkflow = byId.get(pipeline.id);
  byId.set(pipeline.id, listedCurrentWorkflow ? { ...listedCurrentWorkflow, ...pipeline } : pipeline);

  const sources: Array<{ pipeline: WorkflowWithOptionalConnections; depth: number }> = [];
  const queue: Array<{ pipeline: WorkflowWithOptionalConnections; depth: number }> = [{ pipeline: byId.get(pipeline.id)!, depth: 0 }];
  ...
  while (queue.length > 0) {
    ...
    sources.push(next);
    for (const upstreamId of upstreamWorkflowIds(next.pipeline, [...byId.values()]).sort()) {
...
function buildIncomingCarryOverFieldGroups(
  pipeline: WorkflowDetail | null,
  stage: WorkflowStage | null,
  candidates: WorkflowListItem[],
): CarryOverFieldGroup[] {
  ...
  return [...byId.values()].flatMap((sourceWorkflow) => {
    return (sourceWorkflow.stages ?? []).flatMap((sourceStage) => {
      const breakdown = readStageBreakdown(sourceStage);
      if (!breakdown || breakdown.targetWorkflowId !== pipeline.id || breakdown.targetStageKey !== stage.key) {
        return [];
      }
      const sourceFieldGroups = buildCarryOverFieldGroups(sourceWorkflow, [...byId.values()]);
      ...
      const fields = selectedKeys.map((key) => {
        ...
        return field ?? {
          ...
          label: key,
          required: false,
          originId: sourceWorkflow.id,
          originLabel: sourceWorkflow.name,
          originDescription: sourceStage.name,
        };
      });
      if (fields.length === 0) return [];
      return [{
        id: `${sourceWorkflow.id}:${sourceStage.id}`,
        label: sourceWorkflow.name,
...
type StageFormValues = {
  ...
  breakdownTargetWorkflowId: string;
...
type WorkflowTransitionRecord = { fromStageId: string; toStageId: string; label?: string | null };
...
function computeStageForm(
  ...
  return {
    ...
    breakdownTargetWorkflowId: breakdown?.targetWorkflowId ?? "",
...
function sortedStages(pipeline: WorkflowDetail | null | undefined) {
...
function nextStageByPosition(stages: WorkflowStage[], stage: WorkflowStage | null | undefined) {
  if (!stage) return null;
  return stages.find((candidate) => candidate.id !== stage.id && candidate.position > stage.position) ?? null;
}

function nextStageForInsert(stages: WorkflowStage[], position: number) {
...
export function isWorkflowSettingsStageSectionAvailable(kind: string, section: StageSectionKey) {
  return stageNavGroups(kind).some((group) => group.items.some((item) => item.id === section));
}

function defaultReviewTarget(stages: WorkflowStage[], selectedStageId: string | null, kind: string) {
...
function dedupeEdges(edges: WorkflowTransitionEdge[]) {
  const seen = new Set<string>();
...
function itemExampleSource(row: WorkflowCaseChildRow | null) {
  if (!row) return null;
...
function buildAutomationVariableGroups(input: {
  ...
  stage: WorkflowStage;
  sampleRow: WorkflowCaseChildRow | null;
}): AutomationVariableGroup[] {
  const sampleLifeAdmin = input.sampleRow?.case ?? null;
  ...
  const itemVariables: AutomationVariableOption[] = [
    ...
    },
    {
      ...
      example: sampleLifeAdmin?.summary ?? "",
      exampleSource,
    },
    {
      key: "case_id",
    ...
    },
    {
      ...
      example: sampleLifeAdmin?.caseKey ?? "",
      exampleSource,
    },
    {
      key: "case_title",
    ...
    },
    {
      ...
      example: sampleLifeAdmin?.version ?? "",
      exampleSource,
    },
  ];
  const fieldVariables: AutomationVariableOption[] = [];
  const fields = sampleLifeAdmin?.fields && typeof sampleLifeAdmin.fields === "object" && !Array.isArray(sampleLifeAdmin.fields)
    ? sampleLifeAdmin.fields
    : {};
...
function StageEventsList({
  ...
  events: WorkflowCompanyCaseEvent[];
...
export function WorkflowSettings() {
  ...
  const [breakdownTargetWorkflowId, setBreakdownTargetWorkflowId] = useState("");
  ...
  const [pipelineName, setWorkflowName] = useState("");
  const [pipelineDescription, setWorkflowDescription] = useState("");
  ...
  // Domain secrets back the Secrets tab — the same inventory used by routines,
  ...
  const pipelineLifeAdminQuery = useQuery({
  ...
  const breakdownTargetIntakeQuery = useQuery({
    queryKey: breakdownTargetWorkflowId
      ? queryKeys.pipelines.intakeForm(breakdownTargetWorkflowId)
      : ["pipelines", "intake-form", "none-breakdown"],
    queryFn: () => pipelinesApi.getIntakeForm(breakdownTargetWorkflowId),
    enabled: !!selectedCompanyId && !!breakdownTargetWorkflowId,
  });
  ...
  const sampleCaseRow = useMemo(() => {
    const rows = pipelineLifeAdminQuery.data ?? [];
    return rows.find((row) => row.case.stageId === selectedStage?.id) ?? rows[0] ?? null;
  }, [pipelineLifeAdminQuery.data, selectedStage?.id]);
  ...
  const fallbackStageId = resolveWorkflowSettingsFallbackStageId(stages, selectedStageId, requestedStageId);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setBreakdownTargetWorkflowId(form.breakdownTargetWorkflowId);
  ...
  useEffect(() => {
    if (!selectedStage) return;
    if (requestedStageSection && isWorkflowSettingsStageSectionAvailable(selectedStage.kind, requestedStageSection)) {
  ...
  useEffect(() => {
    if (!selectedStage) return;
    if (!isWorkflowSettingsStageSectionAvailable(selectedStage.kind, activeStageSection)) {
  ...
  useEffect(() => {
    if (!pipeline) return;
    setWorkflowName(pipeline.name);
    setWorkflowDescription(pipeline.description ?? "");
  ...
  const saveStage = useMutation({
    mutationFn: async () => {
      ...
      if (breakdownEnabled && breakdownTargetWorkflowId && breakdownTargetStageKey) {
        config.breakdown = {
          targetWorkflowId: breakdownTargetWorkflowId,
  ...
  const addStage = useMutation({
    mutationFn: async (afterStage: WorkflowStage | null) => {
  ...
  const deleteStage = useMutation({
    mutationFn: async () => {
      ...
      return pipelinesApi.deleteStage(pipelineId, selectedStage.id, {
        moveLifeAdminToStageId: deleteMoveTargetStageId || null,
      });
    },
    onSuccess: async () => {
  ...
  const saveWorkflowDetails = useMutation({
    mutationFn: () =>
  ...
  const archiveWorkflow = useMutation({
    ...
    onSuccess: async (_result, archived) => {
      ...
      if (archived) {
        ...
        await refreshWorkflow();
        pushToast({ title: "Workflow restored", tone: "success" });
      }
    },
  });
  ...
  if (!pipeline) {
    return <EmptyState icon={Hexagon} message="Workflow not found." />;
  }

  const isArchived = Boolean(pipeline.archivedAt);
  const archiveEnabled = archiveConfirmation === pipeline.name && !archiveWorkflow.isPending;
  ...
  const breakdownTargetWorkflow = breakdownTargetOptions.find((candidate) => candidate.id === breakdownTargetWorkflowId)
    ?? (pipelinesListQuery.data ?? []).find((candidate) => candidate.id === breakdownTargetWorkflowId)
    ?? null;
  const breakdownTargetStages = [...(breakdownTargetWorkflow?.stages ?? [])].sort(
  ...
  const breakdownTargetArchived = Boolean(breakdownTargetWorkflow?.archivedAt);
  const breakdownIntakeSettingsHref = breakdownTargetWorkflowId
    ? `/pipelines/${breakdownTargetWorkflowId}/settings${breakdownIntakeStageId ? `?stage=${breakdownIntakeStageId}` : ""}`
    : null;
  ...
  const breakdownCopyNames: BreakdownCopyNames = {
    targetWorkflowName: breakdownTargetWorkflow?.name ?? "",
  ...
  const breakdownSummary = breakdownEnabled
    ...
    : null;
  const transitionTargetsControl = !isReviewStage && !isWorkflowTerminalStageKind(stageKind) ? (
  ...
  const breakdownSettingsCard = !isWorkflowTerminalStageKind(stageKind) ? (
            ...
            <div className="space-y-1">
              <div className="flex w-full max-w-sm items-center">
                <select
                  ...
                  value={breakdownTargetWorkflowId}
                  onChange={(event) => {
                    setBreakdownTargetWorkflowId(event.target.value);
                ...
                {breakdownTargetWorkflowId ? (
                  <Link
                    to={`/pipelines/${breakdownTargetWorkflowId}`}
                    aria-label={`Open ${breakdownTargetWorkflow?.name ?? "selected"} pipeline`}
                    title={`Open ${breakdownTargetWorkflow?.name ?? "selected"} pipeline`}
          ...
          <FieldRow label="starting at">
            <div className="space-y-1">
              <select
                ...
                disabled={!breakdownTargetWorkflowId}
                ...
                {breakdownTargetWorkflowId ? (
                  <div className="flex flex-wrap items-center gap-1 text-muted-foreground">
                    ...
                    <span className="font-medium text-foreground">
                      {breakdownTargetWorkflow?.name ?? "selected pipeline"}
                    </span>
                            ...
                            >
                              ...
                              {targetField?.required ? (
                                <span className="text-xs text-muted-foreground">
                                  Required by {breakdownTargetWorkflow?.name ?? "destination"}
                                </span>
                              ) : targetField ? (
                                <span className="text-xs text-muted-foreground">
                                  Validated by {breakdownTargetWorkflow?.name ?? "destination"}
                                </span>
                              ) : null}
                            </label>
                          );
    ...
    <div className="space-y-6">
      ...
      >
        <div className="mb-3 flex items-start justify-between gap-3">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button type="button" variant="outline" size="icon" className="h-8 w-8" title="Workflow actions">
                <MoreHorizontal className="h-4 w-4" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end">
              {isArchived ? (
                <DropdownMenuItem onSelect={() => archiveWorkflow.mutate(false)}>
        ...
        <div className="grid gap-3 md:grid-cols-(--gtc-13) md:items-end">
          ...
          {detailsDirty || saveWorkflowDetails.isPending ? (
            <Button type="submit" disabled={saveWorkflowDetails.isPending || !pipelineName.trim()}>
              <Save className="h-4 w-4" />
              {saveWorkflowDetails.isPending ? "Saving..." : "Save details"}
            </Button>
          ) : null}
        </div>
        {saveWorkflowDetails.error ? (
          <p className="mt-3 text-sm text-destructive">{saveWorkflowDetails.error.message}</p>
        ) : null}
      </form>
            ...
            <div className="overflow-x-auto border-y border-border py-4">
              <div className="flex min-w-max items-center gap-2">
                {stages.map((stage, index) => {
                  ...
                  const canInsertAfter = !isWorkflowTerminalStageKind(stage.kind);
            ...
            >
              <div className="flex flex-col gap-5 md:flex-row md:gap-0">
                ...
                <div className="min-w-0 flex-1 md:px-8">
                  ...
                  {activeStageSection === "instructions" && !isWorkflowTerminalStageKind(stageKind) ? (
                  ...
                  {activeStageSection === "advanced" ? (
                    <div className="w-full max-w-3xl space-y-8">
                      ...
                      {isWorkflowTerminalStageKind(stageKind) ? null : breakdownEnabled ? (
                        <EmptyState
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            {archiveWorkflow.error ? (
              <p className="text-sm text-destructive">{archiveWorkflow.error.message}</p>
            ) : null}
          </div>
          <DialogFooter>
            <Button
            ...
            <Button
              ...
              onClick={() => archiveWorkflow.mutate(true)}
            >
              <Archive className="h-4 w-4" />
              {archiveWorkflow.isPending ? "Archiving..." : "Archive pipeline"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
