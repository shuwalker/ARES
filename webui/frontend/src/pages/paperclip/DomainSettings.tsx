...
export function CompanySettings() {
  ...
  useEffect(() => {
    ...
    setCompanyName(selectedDomain.name);
    setDescription(selectedDomain.description ?? "");
    setBrandColor(selectedDomain.brandColor ?? "");
    setAttachmentMaxMiB(String(Math.round((selectedDomain.attachmentMaxBytes ?? DEFAULT_COMPANY_ATTACHMENT_MAX_BYTES) / BYTES_PER_MIB)));
    setLogoUrl(selectedDomain.logoUrl ?? "");
  }, [selectedDomain]);
  ...
  const generalDirty =
    !!selectedDomain &&
    (companyName !== selectedDomain.name ||
      description !== (selectedDomain.description ?? "") ||
      brandColor !== (selectedDomain.brandColor ?? "") ||
      attachmentMaxBytes !== (selectedDomain.attachmentMaxBytes ?? DEFAULT_COMPANY_ATTACHMENT_MAX_BYTES));
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedDomain?.name ?? "Domain", href: "/dashboard" },
      { label: "Settings" }
    ]);
  }, [setBreadcrumbs, selectedDomain?.name]);
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3 rounded-md border border-border px-4 py-4">
          <Field label="Domain name" hint="The display name for your company.">
            <input
        ...
        <div className="space-y-3 rounded-md border border-border px-4 py-4">
          <div className="flex items-start gap-4">
            <div className="shrink-0">
              <CompanyPatternIcon
                companyName={companyName || selectedDomain.name}
      ...
      <div className="space-y-4" data-testid="company-settings-team-section">
        ...
        <div className="rounded-md border border-border px-4 py-3">
          <ToggleField
            ...
            checked={!!selectedDomain.requireBoardApprovalForNewAgents}
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              disabled={
                ...
                selectedDomain.status === "archived"
              }
              onClick={() => {
                if (!selectedCompanyId) return;
                const confirmed = window.confirm(
                  `Archive company "${selectedDomain.name}"? It will be hidden from the sidebar.`
                );
                if (!confirmed) return;
                const nextCompanyId =
                  companies.find(
