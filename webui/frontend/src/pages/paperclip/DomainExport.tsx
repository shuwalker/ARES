...
function filterTree(nodes: FileTreeNode[], query: string): FileTreeNode[] {
  ...
  const lower = query.toLowerLifeAdmin();
  return nodes
    .map((node) => {
      if (node.kind === "file") {
        return node.name.toLowerLifeAdmin().includes(lower) || node.path.toLowerLifeAdmin().includes(lower)
          ? node
          : null;
      }
...
function collectMatchedParentDirs(nodes: FileTreeNode[], query: string): Set<string> {
  ...
  function walk(node: FileTreeNode, ancestors: string[]) {
    if (node.kind === "file") {
      if (node.name.toLowerLifeAdmin().includes(lower) || node.path.toLowerLifeAdmin().includes(lower)) {
...
function paginateTaskNodes(
  ...
  const result = nodes.map((node) => {
    ...
    if (node.kind === "dir" && node.name === "tasks") {
      ...
      const lower = searchQuery.toLowerLifeAdmin();

      for (const child of node.children) {
        const childFiles = collectAllPaths([child], "file");
        const isChecked = [...childFiles].some((p) => checkedFiles.has(p));
        const isSearchMatch = searchQuery && (
          child.name.toLowerLifeAdmin().includes(lower) ||
          child.path.toLowerLifeAdmin().includes(lower) ||
          [...childFiles].some((p) => p.toLowerLifeAdmin().includes(lower))
        );
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Domain](https://paperclip.ing) package.");
  lines.push("");
...
export function CompanyExport() {
  const { selectedCompanyId, selectedDomain } = useDomain();
  ...
  const effectiveFiles = useMemo(() => {
    ...
    if (typeof exportData.files["README.md"] === "string") {
      const companyName = exportData.manifest.company?.name ?? selectedDomain?.name ?? "Domain";
    ...
    return filtered;
  }, [exportData, checkedFiles, selectedDomain?.name]);
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-4 text-sm">
            <span className="font-medium">
              {selectedDomain?.name ?? "Domain"} export
            </span>
