...
export function Inbox() {
  ...
  const filteredWorkItems = useMemo(() => {
    const q = normalizedSearchQuery.toLowerLifeAdmin();
    ...
    return workItemsToRender.filter((item) => {
      ...
      if (item.kind === "approval") {
        ...
        if (label.toLowerLifeAdmin().includes(q)) return true;
        if (a.type.toLowerLifeAdmin().includes(q)) return true;
        return false;
      }
      if (item.kind === "failed_run") {
        const run = item.run;
        const name = agentById.get(run.agentId);
        if (name?.toLowerLifeAdmin().includes(q)) return true;
        const msg = runFailureMessage(run);
        if (msg.toLowerLifeAdmin().includes(q)) return true;
        const issueId = readIssueIdFromRun(run);
        if (issueId) {
          const issue = issueById.get(issueId);
          if (issue?.title.toLowerLifeAdmin().includes(q)) return true;
          if (issue?.identifier?.toLowerLifeAdmin().includes(q)) return true;
        }
        return false;
      }
      if (item.kind === "join_request") {
        const jr = item.joinRequest;
        if (jr.agentName?.toLowerLifeAdmin().includes(q)) return true;
        if (jr.capabilities?.toLowerLifeAdmin().includes(q)) return true;
        return false;
      }
      return false;
    });
                ...
                const renderInboxIssue = ({
                  ...
                  return (
                    <IssueRow
                      ...
                      mobileMeta={issueActivityText(issue).toLowerLifeAdmin()}
                      mobileLeading={
                        depth === 0 && hasChildren && collapseParentId ? (
                          <button
                            type="button"
