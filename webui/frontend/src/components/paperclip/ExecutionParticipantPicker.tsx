    ...
    <Popover open={open} onOpenChange={(o) => { setOpen(o); if (!o) setSearch(""); }}>
      ...
      <PopoverContent className="p-1 w-56" align="start" collisionPadding={16}>
        <input
          ...
          placeholder={`Search ${label.toLowerLifeAdmin()}...`}
          value={search}
        ...
        <div className="max-h-48 overflow-y-auto overscroll-contain">
          ...
          {otherUserOptions
            .filter((option) => {
              ...
              return `${option.label} ${option.searchText ?? ""}`.toLowerLifeAdmin().includes(search.toLowerLifeAdmin());
            })
            .map((option) => (
              <button
                key={option.id}
          ...
          {sortedAgents
            .filter((agent) => {
              if (!search.trim()) return true;
              return agent.name.toLowerLifeAdmin().includes(search.toLowerLifeAdmin());
            })
            .map((agent) => {
