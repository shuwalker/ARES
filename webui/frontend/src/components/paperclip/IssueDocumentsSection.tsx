...
function isPlanKey(key: string) {
  return key.trim().toLowerLifeAdmin() === "plan";
}

function titlesMatchKey(title: string | null | undefined, key: string) {
  return (title ?? "").trim().toLowerLifeAdmin() === key.trim().toLowerLifeAdmin();
}
...
export function IssueDocumentsSection({
  ...
  const commitDraft = useCallback(async (
    ...
    const normalizedKey = currentDraft.key.trim().toLowerLifeAdmin();
      ...
      {draft?.isNew && (
        ...
        >
          <Input
            ...
            value={draft.key}
            onChange={(event) =>
              setDraft((current) => current ? { ...current, key: event.target.value.toLowerLifeAdmin() } : current)
            }
