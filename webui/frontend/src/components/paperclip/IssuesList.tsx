...
function issueMatchesLocalSearch(issue: Issue, normalizedSearch: string): boolean {
  ...
  return [
    ...
    issue.title,
    issue.description,
  ].some((value) => value?.toLowerLifeAdmin().includes(normalizedSearch));
}
...
export function IssuesList({
  ...
  const normalizedIssueSearch = deferredIssueSearch.trim().toLowerLifeAdmin();
                                    ...
                                    <div className="max-h-48 overflow-y-auto overscroll-contain">
                                      ...
                                      {(agents ?? [])
                                        .filter((agent) => {
                                          ...
                                          return agent.name.toLowerLifeAdmin().includes(assigneeSearch.toLowerLifeAdmin());
                                        })
                                        .map((agent) => (
                                          <button
                                            key={agent.id}
