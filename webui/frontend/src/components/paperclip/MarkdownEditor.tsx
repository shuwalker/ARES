...
export function findMentionMatch(
  ...
  if (trigger === "skill" && /\s/.test(query) && !query.toLowerLifeAdmin().startsWith("routine:")) {
    return null;
  }
...
export function issueMentionTitle(option: MentionOption): string {
  ...
  if (identifier && name.toLowerLifeAdmin().startsWith(identifier.toLowerLifeAdmin())) {
...
export const MarkdownEditor = forwardRef<MarkdownEditorRef, MarkdownEditorProps>(function MarkdownEditor({
  ...
  const filteredMentions = useMemo<AutocompleteOption[]>(() => {
    ...
    const q = mentionState.query.trim().toLowerLifeAdmin();
    if (mentionState.trigger === "skill") {
      return slashCommands
        .filter((command) => {
          if (!q) return true;
          return command.aliases.some((alias) => alias.toLowerLifeAdmin().includes(q));
        })
        .slice(0, MAX_AUTOCOMPLETE_OPTIONS);
    }
    if (!mentions) return [];
    return mentions
      .filter((m) => m.name.toLowerLifeAdmin().includes(q))
      .slice(0, MAX_AUTOCOMPLETE_OPTIONS);
