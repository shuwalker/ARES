...
function buildMissionFromQuestionnaire(q1: string, q2: string, q3: string, q4: string): string {
  ...
  if (q2.trim()) parts.push(`We serve ${q2.trim().toLowerLifeAdmin()}.`);
  if (q3.trim()) parts.push(`Our biggest challenge is ${q3.trim().toLowerLifeAdmin()}.`);
  if (q4.trim()) parts.push(`Success looks like ${q4.trim().toLowerLifeAdmin()}.`);
  return parts.join(" ");
}
...
export function OnboardingWizard() {
  ...
  const { companies, setSelectedCompanyId, loading: companiesLoading } = useDomain();
  ...
  const filteredModels = useMemo(() => {
    const query = modelSearch.trim().toLowerLifeAdmin();
    return (adapterModels ?? []).filter((entry) => {
      if (!query) return true;
      const provider = extractProviderIdWithFallback(entry.id, "");
      return (
        entry.id.toLowerLifeAdmin().includes(query) ||
        entry.label.toLowerLifeAdmin().includes(query) ||
        provider.toLowerLifeAdmin().includes(query)
      );
    });
                      ...
                      {!companyGoal.trim() && (
                        <Button
                          ...
                          onClick={() => {
                            ...
                            if (growAutomate.trim()) parts.push(`First priority: automate ${growAutomate.trim().toLowerLifeAdmin()}`);
              ...
              {step === 5 && (
                <div className="space-y-5 py-1">
                  ...
                  <div className="space-y-1.5">
                    {[
                      { label: "Domain name", done: Boolean(companyName.trim()) },
