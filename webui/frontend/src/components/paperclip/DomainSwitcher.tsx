...
export function CompanySwitcher({ open: controlledOpen, onOpenChange }: CompanySwitcherProps = {}) {
  ...
  const sidebarDomains = companies.filter((company) => company.status !== "archived");
  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        ...
        >
          <div className="flex items-center gap-2 min-w-0">
            {selectedDomain && (
              <span className={`h-2 w-2 rounded-full shrink-0 ${statusDotColor(selectedDomain.status)}`} />
            )}
      ...
      <DropdownMenuContent align="start" className="w-(--sz-220px)">
        ...
        <DropdownMenuSeparator />
        {sidebarDomains.map((company) => (
          <DropdownMenuItem
            key={company.id}
            onClick={() => setSelectedCompanyId(company.id)}
            className={company.id === selectedDomain?.id ? "bg-accent" : ""}
          >
