...
import {
  ...
  type Agent,
  type Issue,
  type WorkflowCaseDocumentPayload,
...
/** LifeAdmin-level body document key (PUT /cases/:id/documents/body). */
const BODY_DOCUMENT_KEY = "body";

/** Local view of the shared case document payload `document` field. */
type CaseBodyDocument = WorkflowCaseDocumentPayload["document"] & {
...
function getWorkflowRevisionActor(
...
export interface WorkflowItemBodyDocumentProps {
  caseId: string;
...
export function WorkflowItemBodyDocument({
  caseId,
  legacySummary,
  hasLegacyLongFields,
  conversationIssueId,
      ...
      <DocumentFrameHeader
        ...
        revisionMenu={hasDocument ? {
          ...
          revisions: revisions.map((revision) => ({
            ...
            actor: getWorkflowRevisionActor(revision, { agentMap, userProfileMap }),
          })),
          selectedRevisionId,
