...
import {
  formatJournalWorkspaceLabel,
  type IssueJournalAssignee,
  type IssueJournalEvent,
  type IssueJournalWorkspace,
...
interface IssueChatMessageContext {
  ...
  /** Linkify `PAP-C7` case chips in comment bodies (experimental LifeAdmin flag). */
...
interface IssueChatThreadProps {
  ...
  /** Domain-wide set of issue ids with a live (queued/running) run. */
        ...
        <div className="min-w-0 flex-1">
          {isFoldable ? (
            ...
            >
              <span className="text-sm font-medium text-foreground">{authorName}</span>
              <SourceTrustBadge sourceTrust={sourceTrust} artifactLabel="comment" />
              <span className="text-xs text-muted-foreground/60">{chainOfThoughtLabel?.toLowerLifeAdmin()}</span>
...
function isJournalWorkspace(value: unknown): value is IssueJournalWorkspace {
...
function isJournalWorkspaceChange(value: unknown): value is NonNullable<IssueJournalEvent["workspaceChange"]> {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const change = value as Record<string, unknown>;
  return isJournalWorkspace(change.from) && isJournalWorkspace(change.to);
}
...
function IssueChatSystemMessage({ message }: { message: ThreadMessage }) {
  ...
  const assigneeChange = typeof custom.assigneeChange === "object" && custom.assigneeChange
    ? custom.assigneeChange as {
        from: IssueJournalAssignee;
        to: IssueJournalAssignee;
      }
    : null;
  const workspaceChange = isJournalWorkspaceChange(custom.workspaceChange) ? custom.workspaceChange : null;
      ...
      <IssueChatMetadataRow anchorId={anchorId} icon={rowIcon}>
        ...
        {workspaceChange ? (
          <div className="flex flex-wrap items-center gap-1.5 text-xs">
            ...
            <span className="text-muted-foreground">
              {formatJournalWorkspaceLabel(workspaceChange.from)}
            </span>
            <ArrowRight className="h-3 w-3 text-muted-foreground/70" />
            <span className="font-medium text-foreground">
              {formatJournalWorkspaceLabel(workspaceChange.to)}
            </span>
          </div>
        ) : null}
