...
export function NewIssueDialog() {
  ...
  const { companies, selectedCompanyId, selectedDomain } = useDomain();
  ...
  const dialogDomain = companies.find((c) => c.id === effectiveCompanyId) ?? selectedDomain;
              ...
              <PopoverTrigger asChild>
                <button
                  className={cn(
                    ...
                    !dialogDomain?.brandColor && "bg-muted",
                  )}
                  disabled={isSubIssueMode}
                  style={
                    dialogDomain?.brandColor
                      ? {
                          backgroundColor: dialogDomain.brandColor,
                          color: pickTextColorForSolidBg(dialogDomain.brandColor),
                        }
                      : undefined
                  }
                >
                  {(dialogDomain?.name ?? "").slice(0, 3).toUpperLifeAdmin()}
                </button>
              </PopoverTrigger>
              <PopoverContent className="w-48 p-1" align="start">
                {companies.filter((c) => c.status !== "archived").map((c) => (
                  ...
                  >
                    ...
                    >
                      {c.name.slice(0, 3).toUpperLifeAdmin()}
                    </span>
