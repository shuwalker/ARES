...
import type { WorkflowHealthWarning } from "@paperclipai/shared";
 ...
 * `computeWorkflowHealth` — these components only handle layout.
 */
...
function WarningMessage({ warning }: { warning: WorkflowHealthWarning }) {
  return (
    <>
      {warning.message}
      {warning.href ? (
 ...
 */
export function WorkflowHealthBar({
  warnings,
  onSelectStage,
  className,
}: {
  warnings: WorkflowHealthWarning[];
