...
function getCompanyRouteSegment(pathname: string, companyPrefix: string | undefined): string | null {
  ...
  if (segments[0]?.toUpperLifeAdmin() !== companyPrefix.toUpperLifeAdmin()) return null;
  return segments[1]?.toLowerLifeAdmin() ?? null;
}

function isSkillsStoreRoute(pathname: string, companyPrefix: string | undefined) {
  const segments = pathname.split("/").filter(Boolean);
  if (segments[0]?.toLowerLifeAdmin() === "skills") return true;
  if (!companyPrefix) return false;
  return (
    segments[0]?.toUpperLifeAdmin() === companyPrefix.toUpperLifeAdmin() &&
    segments[1]?.toLowerLifeAdmin() === "skills"
  );
}
...
export function Layout() {
  ...
  const matchedDomain = useMemo(() => {
    if (!companyPrefix) return null;
    const requestedPrefix = companyPrefix.toUpperLifeAdmin();
    return companies.find((company) => company.issuePrefix.toUpperLifeAdmin() === requestedPrefix) ?? null;
  }, [companies, companyPrefix]);
  const hasUnknownCompanyPrefix =
    Boolean(companyPrefix) && !companiesLoading && companies.length > 0 && !matchedDomain;
  const pluginRoutePath = useMemo(
    () => matchedPluginRoutePath?.toLowerLifeAdmin() ?? getCompanyRouteSegment(location.pathname, companyPrefix),
    [companyPrefix, location.pathname, matchedPluginRoutePath],
  );
  const routeSidebarCompanyId = matchedDomain?.id ?? null;
  const routeSidebarCompanyPrefix = matchedDomain?.issuePrefix ?? null;
  ...
  useEffect(() => {
    ...
    if (companyPrefix !== matchedDomain.issuePrefix) {
      const suffix = location.pathname.replace(/^\/[^/]+/, "");
      navigate(`/${matchedDomain.issuePrefix}${suffix}${location.search}`, { replace: true });
      return;
    }
    ...
    if (
      shouldSyncCompanySelectionFromRoute({
        selectionSource,
        selectedCompanyId,
        routeCompanyId: matchedDomain.id,
      })
    ) {
      setSelectedCompanyId(matchedDomain.id, { source: "route_sync" });
    }
  }, [
    companyPrefix,
    companies,
            ...
            >
              {hasUnknownCompanyPrefix ? (
                <NotFoundPage
                  ...
                  requestedPrefix={companyPrefix ?? selectedDomain?.issuePrefix}
                />
              ) : (
                <RouteErrorBoundary>
                  <Outlet />
