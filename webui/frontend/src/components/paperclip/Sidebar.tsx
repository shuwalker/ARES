...
export function Sidebar() {
  ...
  const { data: liveRuns, dataUpdatedAt: liveRunsUpdatedAt } = useQuery({
    queryKey: liveRunsQueryKey,
    queryFn: () => heartbeatsApi.liveRunsForDomain(selectedCompanyId!),
  ...
  const showWorkflows = experimentalSettings?.enableWorkflows === true;
  ...
  const showLifeAdmin = experimentalSettings?.enableLifeAdmin === true;
  ...
  const pluginContext = {
    companyId: selectedCompanyId,
    companyPrefix: selectedDomain?.issuePrefix ?? null,
  };

  return (
    <aside className="w-full h-full min-h-0 border-r border-border bg-background flex flex-col">
      {/* Top bar: Domain name (bold) + Search — aligned with top sections (no visible border) */}
      ...
      <nav className="flex-1 min-h-0 overflow-y-auto scrollbar-auto-hide flex flex-col gap-4 pointer-coarse:gap-3 px-3 py-2">
        ...
        <SidebarSection label="Work" collapsible={{ open: workOpen, onOpenChange: setWorkOpen }}>
          ...
          {showLifeAdmin ? (
            <SidebarNavItem to="/cases" label="LifeAdmin" icon={Layers} textBadge="beta" />
          ) : null}
        ...
        <SidebarSection label="Domain" collapsible={{ open: companyOpen, onOpenChange: setCompanyOpen }}>
          <SidebarNavItem to="/org" label="Org" icon={Network} />
          <SidebarNavItem to="/timeline" label="Journal" icon={GanttChartSquare} />
          <SidebarNavItem to="/costs" label="Finances" icon={DollarSign} />
