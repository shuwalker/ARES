...
import { pipelinesApi, type WorkflowDocumentRevision } from "../api/pipelines";
 ...
 */
export function WorkflowStageHistoryPanel({
  pipelineId,
  documentKey,
  currentRevisionId,
  hasDocument,
  ...
  const revisionsQuery = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        if (error instanceof ApiError && error.status === 404) return [] as WorkflowDocumentRevision[];
        throw error;
      }
    },
