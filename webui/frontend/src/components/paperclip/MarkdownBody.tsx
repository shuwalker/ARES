...
import { useOptionalDomain } from "../context/CompanyContext";
...
function caseIdentifierFromHref(href: string | undefined): string | null {
  ...
  return match ? match[1]!.toUpperLifeAdmin() : null;
}
...
interface MarkdownBodyProps {
   ...
   * default; enabled on surfaces behind the experimental LifeAdmin flag (PAP-12969).
   */
...
function MarkdownCaseLink({
  ...
  // LifeAdmin resolve via the get-by-identifier route; navigate there on click.
  ...
  return (
    <Link
      ...
      aria-label={`LifeAdmin ${identifier}`}
    >
      {children}
    </Link>
  );
...
function MarkdownBodyImpl({
  ...
  const company = useOptionalDomain();
