...
function WorkspaceIcon({ company }: { company: Domain }) {
  return (
    <CompanyPatternIcon
...
export function SidebarCompanyMenu({ open: controlledOpen, onOpenChange }: SidebarCompanyMenuProps = {}) {
  ...
  const { companies, selectedDomain, setSelectedCompanyId } = useDomain();
  ...
  const { orderedDomains, persistOrder } = useCompanyOrder({
    companies: sidebarDomains,
  ...
  function selectDomain(company: Domain) {
    const pathPrefix = location.pathname.split("/")[1]?.toUpperLifeAdmin();
    const isCompanyRoute = sidebarDomains.some((sidebarDomain) => (
      sidebarDomain.issuePrefix.toUpperLifeAdmin() === pathPrefix
    ));
    const shouldLeaveCurrentRoute = company.id !== selectedDomain?.id
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      const ids = orderedDomains.map((company) => company.id);
    ...
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          aria-label={selectedDomain ? `Open ${selectedDomain.name} company switcher` : "Open company switcher"}
        >
          <span className="flex min-w-0 flex-1 items-center gap-2">
            {selectedDomain ? <WorkspaceIcon company={selectedDomain} /> : null}
            <span className={cn("truncate text-sm font-bold text-foreground", rail && SIDEBAR_RAIL_HIDDEN_LABEL)}>
              {selectedDomain?.name ?? "Select company"}
            </span>
          </span>
      ...
      <DropdownMenuContent align="start" sideOffset={8} className="w-64 p-1">
        ...
        <div className="max-h-96 overflow-y-auto">
          ...
          >
            <SortableContext
              items={orderedDomains.map((company) => company.id)}
              strategy={verticalListSortingStrategy}
            >
              {orderedDomains.map((company) => (
                <SortableCompanyItem
                  key={company.id}
                  company={company}
                  isEditing={isEditingOrder}
                  isSelected={company.id === selectedDomain?.id}
        ...
        <DropdownMenuItem asChild disabled={isEditingOrder}>
          ...
          >
            ...
            <span className="truncate">
              {selectedDomain ? `Invite people to ${selectedDomain.name}` : "Invite people"}
            </span>
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild disabled={isEditingOrder}>
