...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  function getCheapModelTestLifeAdmin(adapterConfigPatch?: Record<string, unknown>): { model: string; adapterConfig: Record<string, unknown> } | null {
  ...
  function prefixEnvironmentTestChecks(
    ...
    return {
      ...
      checks: [
        {
          code: `${label.toLowerLifeAdmin().replace(/[^a-z0-9]+/g, "_")}_test_started`,
          level: "info",
  ...
  async function runEnvironmentTestLifeAdmin(
    label: string,
    model: string | null,
  ...
  const testEnvironment = useMutation({
    mutationFn: async () => {
      ...
      const cheapTestLifeAdmin = getCheapModelTestLifeAdmin(adapterConfigPatch);
      ...
      const testResults: Array<{ label: string; model: string | null; result: AdapterEnvironmentTestResult }> = [
        {
          label: "Primary model",
          model: primaryModel,
          result: await runEnvironmentTestLifeAdmin(
            "Primary model",
            primaryModel,
      ...
      if (cheapTestLifeAdmin) {
        testResults.push({
          label: "Cheap model",
          model: cheapTestLifeAdmin.model,
          result: await runEnvironmentTestLifeAdmin(
            "Cheap model",
            cheapTestLifeAdmin.model,
            cheapTestLifeAdmin.adapterConfig,
            environmentId,
          ),
        });
      }
...
export function ModelDropdown({
  ...
  const canCreateManualModel = Boolean(
    creatable &&
      manualModel &&
      !models.some((m) => m.id.toLowerLifeAdmin() === manualModel.toLowerLifeAdmin()),
  );
  ...
  const filteredModels = useMemo(() => {
    return models.filter((m) => {
      ...
      const q = modelSearch.toLowerLifeAdmin();
      const provider = extractProviderId(m.id) ?? "";
      return (
        m.id.toLowerLifeAdmin().includes(q) ||
        m.label.toLowerLifeAdmin().includes(q) ||
        provider.toLowerLifeAdmin().includes(q)
      );
    });
