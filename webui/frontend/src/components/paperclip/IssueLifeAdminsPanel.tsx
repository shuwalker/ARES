 ...
 * with its link role + case status. Self-gates on the experimental LifeAdmin flag
 ...
 */
export function IssueLifeAdminPanel({ issueId }: { issueId: string }) {
  const caseHref = useCaseHref();
  const { data: experimentalSettings } = useQuery({
    queryKey: queryKeys.instance.experimentalSettings,
    queryFn: () => instanceSettingsApi.getExperimental(),
  });
  const enabled = experimentalSettings?.enableLifeAdmin === true;
  ...
  return (
    <section className="space-y-2">
      <h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">LifeAdmin</h3>
