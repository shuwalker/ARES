...
export function getProjectOrderStorageKey(domainId: string, userId: string | null | undefined): string {
  return `${PROJECT_ORDER_STORAGE_PREFIX}:${domainId}:${resolveUserId(userId)}`;
}

export function getProjectSortModeStorageKey(domainId: string, userId: string | null | undefined): string {
  return `${PROJECT_SORT_MODE_STORAGE_PREFIX}:${domainId}:${resolveUserId(userId)}`;
}
