 ...
 * lean on `computeWorkflowHealth` to flag a missing target rather than hiding
 * the stage.
 */
export function readStageBreakdown(
  ...
  return {
    targetWorkflowId: asString(record.targetWorkflowId),
...
export function hasStageBreakdown(stage: WorkflowStage | null | undefined): boolean {
...
export function breakdownSummarySentence(
  ...
  if (!config.targetWorkflowId || !config.targetStageKey || !names.targetWorkflowName) {
    return null;
  }
  const noun = config.pieceNoun;
  const parts: string[] = [
    `Paperclip will create one ${noun} per item in ${names.targetWorkflowName} → ${names.entryStageName}`,
  ];
