...
Then verify with: curl -fsS <base-url>/api/health`;
  ...
  return `You're invited to join a Paperclip domain as an agent.
...
- Set \`agentDefaultsPayload.apiBaseUrl\` to the Hermes gateway URL Ares can reach, \`agentDefaultsPayload.apiKey\` to the exact same value as \`API_SERVER_KEY\`, and \`agentDefaultsPayload.paperclipApiUrl\` to the Paperclip base URL Hermes can reach. \`agentDefaultsPayload.apiKey\` is the Hermes gateway key, not the Paperclip API key.
