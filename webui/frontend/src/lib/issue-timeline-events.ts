...
export interface IssueJournalAssignee {
  agentId: string | null;
  userId: string | null;
}

export interface IssueJournalEvent {
  id: string;
  ...
  workspaceChange?: {
    from: IssueJournalWorkspace;
    to: IssueJournalWorkspace;
  };
  commentId?: string | null;
  followUpRequested?: boolean;
}

export interface IssueJournalWorkspace {
  label: string | null;
...
export function formatJournalWorkspaceLabel(workspace: IssueJournalWorkspace) {
...
function sameAssignee(left: IssueJournalAssignee, right: IssueJournalAssignee) {
  return left.agentId === right.agentId && left.userId === right.userId;
}

function sameWorkspace(left: IssueJournalWorkspace, right: IssueJournalWorkspace) {
...
function workspaceFromRecord(value: unknown): IssueJournalWorkspace | null {
...
function sortJournalEvents<T extends { createdAt: Date | string; id: string }>(events: T[]) {
...
export function extractIssueJournalEvents(activity: ActivityEvent[] | null | undefined): IssueJournalEvent[] {
  const events: IssueJournalEvent[] = [];
  ...
  return sortJournalEvents(events);
}
