...
export interface LivenessBannerLink {
  /** Task link target. We only link tasks; life-admin items lack a routable id here. */
  issueId: string;
...
export interface LivenessBannerView {
  reason: WorkflowCaseLiveness["reason"];
...
const AUTO_RETRY_NOTE =
  "Ares retries automatically once the blocker clears — you don't need to move the item by hand.";
...
const NO_ACTION_PATH_BODY =
  "Ares can't see anything to work on next here — no automation, retry, blocker, or review. " +
...
function blockerLinkFromLiveness(liveness: WorkflowCaseLiveness): LivenessBannerLink | null {
...
function automationLinkFromLiveness(liveness: WorkflowCaseLiveness): LivenessBannerLink | null {
 ...
 */
export function deriveWorkflowLivenessBanner(
  liveness: WorkflowCaseLiveness | null | undefined,
  ...
  switch (liveness.reason) {
    ...
    case "permission_preflight_failed":
      return {
        ...
        retryLabel: "",
        helperNote:
          "Grant the access above to the configured responsible, then Ares retries automatically.",
      };
...
function missingPiecesBody(liveness: WorkflowCaseLiveness): string {
