...
type StageLookup = Map<string, string> | Record<string, string> | WorkflowStage[] | undefined;

export interface WorkflowChildRow {
...
interface WorkflowCaseTreeNode {
  id: string;
  caseKey?: string | null;
  title: string;
...
interface WorkflowCaseChildrenTree {
  case?: WorkflowCaseTreeNode | null;
  childGroups?: Array<{ cases?: WorkflowCaseTreeNode[] | null }> | null;
}
...
export function displayWorkflowItemFields(fields: Record<string, unknown> | null | undefined) {
...
export type WorkflowItemDisplayField = ReturnType<typeof displayWorkflowItemFields>[number];

const LONG_FIELD_CHARACTER_THRESHOLD = 180;

export function isLongWorkflowItemField(field: Pick<WorkflowItemDisplayField, "value">) {
  const value = field.value.trim();
  if (!value || value === "None") return false;
  return value.includes("\n") || value.length >= LONG_FIELD_CHARACTER_THRESHOLD;
}

export function splitWorkflowItemFields(fields: WorkflowItemDisplayField[]) {
  const shortFields: WorkflowItemDisplayField[] = [];
  const longFields: WorkflowItemDisplayField[] = [];
  for (const field of fields) {
    if (isLongWorkflowItemField(field)) {
...
type WorkflowConversationAssigneeIssue = Pick<
  Issue,
  "id" | "parentId" | "assigneeAgentId" | "assigneeUserId" | "createdByAgentId"
>;

function sourceIssueAssigneeValue(issue: WorkflowConversationAssigneeIssue | null | undefined) {
  if (!issue) return "";
...
export function normalizeWorkflowChildRows(value: unknown): WorkflowChildRow[] {
  if (Array.isArray(value)) {
  ...
  const tree = readRecord(value) as WorkflowCaseChildrenTree | null;
  if (!tree) return [];

  return (tree.childGroups ?? tree.case?.childGroups ?? [])
    .flatMap((group) => group.cases ?? [])
    .map(treeNodeToChildRow)
    .filter((row): row is WorkflowChildRow => Boolean(row));
}

export function getPendingTransitionBannerState(item: Pick<WorkflowLifeAdmin, "pendingSuggestion" | "fields">, stages?: StageLookup) {
...
export function itemHasChangedNotice(item: Pick<WorkflowLifeAdmin, "fields"> & {
  thisChanged?: unknown;
...
export function eventsHaveUnacknowledgedDrift(events: WorkflowCaseEvent[]) {
...
export function changedNoticeFromEvents(events: WorkflowCaseEvent[]) {
...
function stageName(event: WorkflowCaseEvent, stages: StageLookup, side: "from" | "to") {
...
function automationIssueLabel(event: WorkflowCaseEvent) {
