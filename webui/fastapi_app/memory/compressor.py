...
class ContextCompressor(ContextEngine):
    ...
    def should_defer_preflight_to_real_usage(self, rough_tokens: int) -> bool:
        ...
        overestimates schema-heavy requests so the compressor runs before a
...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
    ...
    def _generate_summary(
        ...
        # Date-only granularity, resolved in the host's local timezone. The
        # compaction summary is a mid-conversation message that is NOT part of
        # the cached prefix, so a date here never affects prompt-cache
        # stability. Resolved defensively — a clock failure must never block
        # compaction.
        try:
            from datetime import datetime

            _today_str = datetime.now().astimezone().strftime("%Y-%m-%d")
