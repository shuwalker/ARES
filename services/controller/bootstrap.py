...
def main() -> int:
    ...
    # Monorepo root hosts core.* and integrations.* (Pass 2 layout).
    monorepo_root = str(REPO_ROOT.parent.parent)
    os.environ["PYTHONPATH"] = os.pathsep.join(
        value
        for value in (str(REPO_ROOT), monorepo_root, existing_pythonpath)
        if value
    )
