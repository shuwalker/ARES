...
Ares WebUI runs well under WSL2, but native Windows login does not automatically start Linux user processes. This guide covers two supported options:
...
```text
scripts/wsl/ares_webui_autostart.sh
```
...
|---|---|---|
| `ARES_WEBUI_REPO` | repo containing the script | WebUI checkout to start |
| `ARES_WEBUI_LOG_DIR` | `$HOME/.ares/webui/logs` | Autostart and WebUI logs |
| `ARES_WEBUI_HOST` | `127.0.0.1` | Host passed through to `start.sh` / `bootstrap.py` |
| `ARES_WEBUI_PORT` | `8787` | WebUI port and health-check port |
| `ARES_WEBUI_HEALTH_URL` | `http://127.0.0.1:$ARES_WEBUI_PORT/health` | URL used to decide whether WebUI is already running |
| `ARES_WEBUI_PID_FILE` | `$ARES_WEBUI_LOG_DIR/ares-webui.pid` | pid file used for duplicate prevention |
| `ARES_WEBUI_REQUIRE_AGENT_PROCESS` | `0` | Optional: set to `1` only if your local setup requires a separate Ares process before WebUI starts |

Make the script executable once inside WSL:

```bash
cd /path/to/ares-webui
chmod +x scripts/wsl/ares_webui_autostart.sh
```
...
```text
$HOME/.ares/webui/logs/webui_autostart.log
$HOME/.ares/webui/logs/ares_webui.log
```
...
```bash
if [ -x "$HOME/ares-webui/scripts/wsl/ares_webui_autostart.sh" ]; then
  ARES_WEBUI_REPO="$HOME/ares-webui" \
    "$HOME/ares-webui/scripts/wsl/ares_webui_autostart.sh" >/dev/null 2>&1 &
fi
```
...
.\scripts\windows\setup_webui_autostart.ps1 `
  -WslScriptPath "/home/your-user/ares-webui/scripts/wsl/ares_webui_autostart.sh" `
  -Distro "Ubuntu"
```
...
- The default task name is `AresWebUIAutoStart`; pass `-TaskName` if you need a different name.
...
```powershell
Get-ScheduledTask -TaskName AresWebUIAutoStart
Unregister-ScheduledTask -TaskName AresWebUIAutoStart -Confirm:$false
```
...
```bash
tail -n 80 "$HOME/.ares/webui/logs/webui_autostart.log"
tail -n 80 "$HOME/.ares/webui/logs/ares_webui.log"
```
...
| Health check fails but pid exists | WebUI is still booting or the port differs | Check `ARES_WEBUI_PORT` and `ares_webui.log` |
