# Ares Run Adapter Contract and Migration Gates
...
- **Created:** 2026-05-11
- **Revised:** 2026-05-23
- **Tracking issue:** [#1925](https://github.com/nesquena/ares-webui/issues/1925)
...
- future Ares runtime APIs cannot be adopted cleanly because WebUI lacks a
...
- #2479 shipped the first Slice 3a implementation in v0.51.86, routing Stop
  ...
  `ARES_WEBUI_RUNTIME_ADAPTER=legacy-journal` is enabled.
...
- #2544 shipped the first Slice 3c implementation in v0.51.91. The goal
  ...
  `ARES_WEBUI_RUNTIME_ADAPTER=legacy-journal` is enabled, while preserving the
...
- Identify future backend mapping: existing Ares runtime API, missing Ares
...
- Do not depend on Ares Agent shipping a WebUI-specific runtime connector before
...
future WebUI-managed runner, or a future Ares `/v1/runs` backend.
...
| queue / continue | append follow-up work according to Ares semantics | runner/runtime control plane |
...
|---|---|---|---|---|---|
| `STREAMS` / `STREAMS_LOCK` | `api.state_sync` process memory | adapter API surface + presentation fan-out | WebUI runner or future Ares run observation API | keep live path; mirror events into journal | Must stop being authoritative for active run existence. |
...
| cached `AIAgent` objects / `AGENT_INSTANCES` | `api/config.py` process memory | runner process | runner-owned Ares integration | unchanged | Moving this is deferred until after journal proof. |
...
| per-request `ARES_HOME` env mutation lock | `api/streaming.py` / config helpers | runner process | runner/profile execution context | unchanged | Long-term runner must isolate profile env without process-global mutation. |
...
| command capability metadata | WebUI command registry + Ares command assumptions | adapter API surface | runtime command/capability metadata | unchanged | Unknown command support should not be guessed by WebUI. |
...
| Multi-profile isolation | profile-specific runs write/read the correct Ares home and memory | protects #2134-family isolation concerns | runner/profile slice |
| Queue/continue | follow-up input during live/resumable work obeys Ares semantics | prevents parallel continuation model | control migration slice |
...
example `ARES_WEBUI_RUNTIME_ADAPTER=legacy-journal` with default
...
2. **Adapter flag is behavior-preserving.** With
   `ARES_WEBUI_RUNTIME_ADAPTER=legacy-journal`, Stop Generation uses
...
- future Ares CLI/Python/local API or `/v1/runs` backends can be evaluated
  behind the adapter.
...
4. **Restart/reattach success criterion.** Start a long-running run, restart only
   `ares-webui.service`, reload the session, rediscover the active or terminal
...
- no dependency on Ares Agent shipping `/v1/runs` before WebUI can validate the
...
- add a concrete runner-client factory behind an explicit mode such as
  `ARES_WEBUI_RUNTIME_ADAPTER=runner-local`, while preserving `legacy-direct`
...
- carry the runtime API gap matrix forward: missing Ares-owned capabilities
...
1. **Route remains default-off.** Unset `ARES_WEBUI_RUNTIME_ADAPTER` and
...
- no permanent WebUI-owned active-run discovery cache when the missing capability
  belongs in a runner or future Ares Runtime API;
...
transport is now implemented behind `ARES_WEBUI_RUNNER_BASE_URL` and remains
...
Ares runtime families such as `token.delta`, `tool.started`, and `done` before
...
- no permanent WebUI-owned active-run discovery cache that duplicates runner or
  future Ares Runtime API responsibility.
...
- keep WebUI as a client of `ARES_WEBUI_RUNNER_BASE_URL`, not the owner of
...
1. **Process ownership moved.** A local runner process, not `ares-webui`, owns
...
2. **Restart/reattach with a real runner.** Start a non-trivial `runner-local`
   run, restart only `ares-webui`, reload the session, rediscover the active or
...
4. **Default-off and reversible.** Unset `ARES_WEBUI_RUNNER_BASE_URL` or switch
...
- no server-side queue scheduler just for adapter symmetry;
- no broad WebUI product-surface migration;
- no claim that this is the canonical Ares Agent Runtime API; if Ares Agent
...
- If Ares Agent later ships a durable `/v1/runs` API, which adapter fields map
