...
def test_advanced_chat_docs_name_gateway_approval_runs_api_opt_in():
    ...
    assert "ARES_WEBUI_CHAT_BACKEND=gateway" in advanced
    assert "ARES_WEBUI_GATEWAY_USE_RUNS_API=true" in advanced
...
def test_docker_docs_name_webui_service_runs_api_opt_in_for_approval_cards():
    ...
    assert "ARES_WEBUI_CHAT_BACKEND=gateway" in docker
    assert "ARES_WEBUI_GATEWAY_BASE_URL=http://ares-agent:8642" in docker
    assert "ARES_WEBUI_GATEWAY_USE_RUNS_API=true" in docker
