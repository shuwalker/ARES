"""Tests for issue #1612: renamed root profile must resolve to ~/.ares,
not ~/.ares/profiles/<name>.

A renamed root/default Ares profile (`is_default=True` on the agent side
but with a display name like `kinni`) was being treated as a named profile
directory under `~/.ares/profiles/kinni`, which doesn't exist. Every
...
def test_is_root_profile_renamed_root_via_list_profiles_api(monkeypatch):
    ...
    monkeypatch.setattr(p, 'list_profiles_api', lambda: [
        {'name': 'kinni', 'is_default': True, 'path': str(p._DEFAULT_ARES_HOME)},
...
def test_is_root_profile_caches_results(monkeypatch):
    ...
    def fake_list():
        calls['n'] += 1
        return [{'name': 'kinni', 'is_default': True, 'path': '/tmp/.ares'}]
...
def test_is_root_profile_invalidation_drops_stale(monkeypatch):
    ...
    seq = [
        [{'name': 'kinni', 'is_default': True, 'path': '/tmp/.ares'}],
        [{'name': 'noblepro', 'is_default': True, 'path': '/tmp/.ares'}],
    ]
...
def test_is_root_profile_handles_list_profiles_failure(monkeypatch):
    ...
    def boom():
        raise RuntimeError("ares_cli explosion")
...
# ── get_active_ares_home: returns _DEFAULT_ARES_HOME for renamed root ──


def test_get_active_ares_home_returns_default_for_renamed_root(tmp_path, monkeypatch):
    """The core bug: a renamed root profile must resolve to _DEFAULT_ARES_HOME,
    not _DEFAULT_ARES_HOME / 'profiles' / <name>."""
    import api.profiles as p
    ...
    result = p.get_active_ares_home()
    assert result == tmp_path, f"Expected {tmp_path}, got {result}"


def test_get_active_ares_home_returns_named_for_real_named_profile(tmp_path, monkeypatch):
...
def test_switch_profile_resolution_renamed_root_picks_default_home(tmp_path, monkeypatch):
    ...
    _DEFAULT_ARES_HOME, not raise 'Profile <name> does not exist.'
...
def test_switch_profile_sticky_marker_renamed_root(tmp_path, monkeypatch):
    ...
    _is_root_profile('default') → resolves to _DEFAULT_ARES_HOME, which
