...
# Monorepo root for core.* / integrations.* packages (Pass 2 layout).
_MONOREPO_ROOT = REPO_ROOT.parent.parent
...
def test_server():
    ...
    # Monorepo root hosts core.* / integrations.* (Pass 2 layout).
    _monorepo_root = str(REPO_ROOT.parent.parent)
    env["PYTHONPATH"] = os.pathsep.join(
        value
        for value in (str(REPO_ROOT), _monorepo_root, env.get("PYTHONPATH", ""))
        if value
    )
