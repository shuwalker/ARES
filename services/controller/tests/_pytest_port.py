...
conftest.py publishes ``ARES_WEBUI_TEST_PORT`` and
``ARES_WEBUI_TEST_STATE_DIR`` to ``os.environ`` at module level
...
TEST_PORT = int(os.environ.get('ARES_WEBUI_TEST_PORT',
                               str(_auto_test_port(_REPO_ROOT))))
BASE = f"http://127.0.0.1:{TEST_PORT}"

# Test state dir: prefer the value conftest.py published to the environment.
# The standalone fallback anchors under the OS temp dir (NOT ~/.ares) so test
# state is never created inside the production Ares home — matching conftest's
# hard-isolation default. (See conftest.py TEST_STATE_DIR.)
import tempfile as _tempfile
_TEST_STATE_ROOT = pathlib.Path(
    os.environ.get('ARES_WEBUI_TEST_STATE_ROOT', _tempfile.gettempdir())
) / 'ares-webui-tests'
...
# ~/.ares either. Same logic: trip only if under literal ~/.ares and the temp
# root isn't itself under production.
_PROD_ARES_HOME = (pathlib.Path.home() / '.ares').resolve()
...
TEST_DEFAULT_MODEL = os.environ.get('ARES_WEBUI_DEFAULT_MODEL', 'openai/gpt-5.4-mini')
