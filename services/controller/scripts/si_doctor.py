...
# Controller package root + monorepo root (core/integrations packages).
CONTROLLER_ROOT = Path(__file__).resolve().parent.parent
MONOREPO_ROOT = CONTROLLER_ROOT.parent.parent
...
def check_bridge_wired():
    """Bridge is wired into the agentic worker path (core.si via api.si shim)."""
    # Implementation lives under integrations/ after reorg (was api/backends/base.py).
    candidates = [
        MONOREPO_ROOT / "integrations" / "providers" / "agentic_backend.py",
        CONTROLLER_ROOT / "api" / "backends" / "base.py",  # legacy path
    ]
    base_py = next((p for p in candidates if p.is_file()), None)
    assert base_py is not None, (
        "SI bridge host file not found "
        f"(tried: {', '.join(str(p) for p in candidates)})"
    )
    src = base_py.read_text()

    assert "from api.si.bridge import si_enabled, si_turn" in src, \
        f"SI bridge import not found in {base_py}"
    assert "if si_enabled():" in src, \
        f"SI enabled check not found in {base_py}"
    assert "result = si_turn(" in src or "si_turn(" in src, \
        f"si_turn call not found in {base_py}"
