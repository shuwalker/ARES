...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --port) PORT="$2"; shift 2 ;;
        --host) HOST="$2"; shift 2 ;;
        --dir) INSTALL_DIR="$2"; WEBUI_DIR="$INSTALL_DIR/services/controller"; shift 2 ;;
...
install_from_source() {
    ...
    if [ ! -f "$source_abs/services/controller/fastapi_app/main.py" ] || \
       [ ! -f "$source_abs/apps/web/package.json" ]; then
    ...
    rsync -a \
        ...
        --exclude '/.build/' \
        --exclude '/attic/' \
        --exclude '/apps/macos/ARES.app/' \
        --exclude '/services/controller/.venv/' \
        --exclude '/services/controller/venv/' \
        --exclude '/services/controller/node_modules/' \
        --exclude '/apps/web/node_modules/' \
        --exclude '/services/controller/tests/' \
        --exclude '/services/controller/docs/' \
        --exclude '**/__pycache__/' \
...
install_deps() {
    ...
    if [ ! -f "$WEBUI_DIR/requirements.txt" ] || [ ! -d "$INSTALL_DIR/apps/web" ]; then
    ...
    if ! (cd "$INSTALL_DIR/apps/web" && npm ci && npm run build); then
